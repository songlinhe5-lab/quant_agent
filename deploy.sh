...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "  overseas  海外节点 (futu + finnhub + yfinance)"
            echo ""
            echo "选项:"
...
run_step "3/7" "📸 记录当前镜像 ID (回滚用)" '
    PREV_IMAGE=$(docker compose ps -q quant-agent 2>/dev/null | head -1 | \
...
run_step "4/7" "🛑 停掉旧容器释放内存" "
    COMPOSE_PROFILES=$COMPOSE_PROFILES docker compose down || true
"
...
run_step "6/7" "🚀 启动服务 (Profile: ${COMPOSE_PROFILES:-default})" "
    COMPOSE_PROFILES=$COMPOSE_PROFILES docker compose up -d --remove-orphans
"
...
if [ "$DRY_RUN" = false ]; then
    echo -e "${CYAN}🐳 容器状态:${NC}"
    COMPOSE_PROFILES=$COMPOSE_PROFILES docker compose ps --format "table {{.Name}}\t{{.Status}}\t{{.Ports}}" 2>/dev/null || \
        docker compose ps
    echo ""
    echo -e "${CYAN}💾 磁盘使用:${NC}"
    df -h / | tail -1
    ...
    echo "  重启服务:        COMPOSE_PROFILES=$COMPOSE_PROFILES docker compose restart quant-agent"
