...
def test_new_indicators():
    ...
    try:
        ...
        print(
            f"   ✓ CCI: {cci_result:.2f} ({'Overbought' if cci_result > 100 else 'Oversold' if cci_result < -100 else 'Neutral'})"
        )
        ...
        from backend.utils.technical_indicators_pro import IndicatorConfig, TechnicalIndicatorsEngine
        ...
        print(
            f"   ✓ Bull Power: {elder_result.get('bull_power'):.2f} ({'Bullish' if elder_result.get('bull_power', 0) > 0 else 'Bearish'})"
        )
        print(
            f"   ✓ Bear Power: {elder_result.get('bear_power'):.2f} ({'Bearish' if elder_result.get('bear_power', 0) < 0 else 'Bullish'})"
        )
