...
async def test_futu_service(close_after: bool = True):
    ...
    if futu_service.status != "CONNECTED":
        ...
        try:
            ...
            if res.get("status") != "success":
                errors.append(f"获取实时快照失败: {res.get('message')}")

            res = await futu_service.get_history("HK.00700", num=10)
            if res.get("status") != "success":
                errors.append(f"获取历史K线失败: {res.get('message')}")

            res = await futu_service.get_fundamental("HK.00700")
            if res.get("status") != "success":
                errors.append(f"获取基本面失败: {res.get('message')}")

            res = await futu_service.get_fund_flow("HK.00700")
            if res.get("status") != "success":
                errors.append(f"获取资金流与席位失败: {res.get('message')}")

            res = await futu_service.get_option_chain("US.AAPL")
            if res.get("status") != "success":
                errors.append(f"获取期权链失败: {res.get('message')}")

            res = await futu_service.get_account_info("HK")
            if res.get("status") != "success":
                errors.append(f"获取账户信息失败: {res.get('message')}")
...
async def main(service: str):
    ...
    try:
        if service in ["all", "yfinance"]:
            await test_yfinance_service()
        if service in ["all", "futu"]:
            await test_futu_service()
        if service in ["all", "notification"]:
            await test_notification_service()
        if service in ["all", "fred"]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-s",
        type=str,
        choices=["all", "yfinance", "futu", "notification", "fred"],
        default="all",
