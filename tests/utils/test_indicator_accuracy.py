...
class TestADXXAccuracy:
    ...
    def test_adx_with_known_data_pattern(self):
        ...
        n = 20
        closes = np.array(
            [100, 102, 104, 106, 108, 110, 112, 114, 116, 118, 120, 118, 116, 114, 112, 110, 108, 106, 104, 102]
        )
...
class TestElderRayAccuracy:
    ...
    def test_ema_basis_reasonable(self):
        ...
        assert min_price <= ema_basis <= max_price, f"EMA Basis ({ema_basis}) 超出价格范围 [{min_price}, {max_price}]"
...
class TestKeltnerChannelsAccuracy:
    ...
    def test_keltner_channel_math(self):
        ...
        # 基本验证
        assert upper is None or middle is None or lower is None or upper > middle > lower, "肯特纳通道上下轨位置错误"
    ...
    def test_keltner_width_volatility_correlation(self):
        ...
        # 计算通道宽度百分比
        width_low = (
            (result_low["upper"] - result_low["lower"]) / result_low["middle"] * 100
            if result_low["upper"] and result_low["middle"]
            else 0
        )
        width_high = (
            (result_high["upper"] - result_high["lower"]) / result_high["middle"] * 100
            if result_high["upper"] and result_high["middle"]
            else 0
        )

        # 高波动的通道应更宽
        if width_low > 0 and width_high > 0:
            assert width_high > width_low, f"高波动数据通道宽度 ({width_high:.2f}%) 应大于低波动 ({width_low:.2f}%)"
