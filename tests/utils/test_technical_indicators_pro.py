...
class TestTechnicalIndicatorsEngine:
    ...
    def test_ma_calculation(self, sample_klines):
        """测试 MA 移动平均线计算"""
        from backend.utils.technical_indicators_pro import DEFAULT_INDICATORS, TechnicalIndicatorsEngine
    ...
    def test_error_isolation(self, sample_klines):
        """测试单个指标失败不影响其他指标"""
        from backend.utils.technical_indicators_pro import DEFAULT_INDICATORS, IndicatorType, TechnicalIndicatorsEngine
...
if __name__ == "__main__":
    pytest.main(
        [
            __file__,
            "-v",
            "--cov=backend.utils.technical_indicators_pro",
