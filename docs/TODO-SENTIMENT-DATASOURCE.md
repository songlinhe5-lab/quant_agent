...
> **⚠️ 生产迁移（必须执行）**：`SentimentRecord` 新增 `retail_heat_change_pct`/`retail_heat_total` 列，但生产 `sentiment_records` 为已存在旧表，SQLAlchemy `create_all` **不会给已有表加列**（否则查询报 `UndefinedColumn`）。Alembic 迁移链历史已损坏（多 None head，`alembic upgrade` 不可靠）——**2026-08-31 已修复**：链并为单 root/单 head（`strat03a → pt01a → ai04rag → fe05b_frontend_logs → sent01 → fin03a`），`sent01` 由 PG 专有 `ADD COLUMN IF NOT EXISTS` 改为方言无关的 inspector 判列加列，`alembic upgrade head` 已实跑验证可用（详见 `backend/alembic/README.md`）。**当时的应急修复**：独立幂等迁移脚本 `backend/scripts/migrate_sentiment_retail_heat.py`（`ALTER TABLE ... ADD COLUMN`，列存在检查兜底，PG/SQLite 均安全，可重复执行）。部署时需执行：`DATABASE_URL=... python -m backend.scripts.migrate_sentiment_retail_heat`。
- [x] **C.2** 热度因子作为独立情绪信号：✅ **2026-08-22**。AGENTS.md §7 多空矩阵为过期引用（当前 AGENTS.md 无此章节）；实质已通过 `get_sentiment_history` 返回独立 `retail_heat_*` 字段 + 前端独立标注完成（热度作为独立散户层，不与机构情绪混淆）。
- [x] **C.3** 「机构 + 散户」双层视图：✅ **2026-08-22**。`macro_app.get_sentiment_history` 每行新增 `retail_heat_change_pct`/`retail_heat_total`（散户热度 A 层），与 `pc_ratio`/`vix`/`credit_spread`（机构情绪 B 层）并排，形成双层视图。
- [x] **C.4** 前端标注（热度 ≠ 情绪）：✅ **2026-08-22**。`market-sentiment.tsx`：`SentimentInfoPanel` 新增「散户热度 (ApeWisdom)」说明项（⚠️ 明确「热度 ≠ 情绪：注意力突变，不代表看多/看空方向」）；`SentimentHistoryChart` 新增「散户热度」柱状线（`retail_heat_change_pct`），与 VIX/P-C 分开显示。
