...
> **⚠️ 生产迁移（必须执行）**：`SentimentRecord` 新增 `retail_heat_change_pct`/`retail_heat_total` 列，但生产 `sentiment_records` 为已存在旧表，SQLAlchemy `create_all` **不会给已有表加列**（否则查询报 `UndefinedColumn`）。Alembic 迁移链历史已损坏（多 None head，`alembic upgrade` 不可靠）。**修复**：独立幂等迁移脚本 `backend/scripts/migrate_sentiment_retail_heat.py`（`ALTER TABLE ... ADD COLUMN`，列存在检查兜底，PG/SQLite 均安全，可重复执行）。部署时需执行：`DATABASE_URL=... python -m backend.scripts.migrate_sentiment_retail_heat`。
