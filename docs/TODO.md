...
- [ ] **[BE-ARCH-07f-2]** **AKShare 连接层下沉远程（从 07f 拆出）**：`backend/services/akshare/{quote.py:58,196,290 / flow.py:48,158,273,413 / calendar.py:55,68,82}` 仍 `import akshare as ak` 且直连 `sina.com.cn` + `jin10.com`；`legacy_market_data` 的 `:26,30` 持有 `akshare_service`、`:444-483` 日历远程失败后**本地 SDK 降级**（`:461-462`）。子服务 `_internal/akshare` 现仅有占位能力，需先补齐南北向资金流 / HSGT 持股 / 行情 / 日历等真实 action（含 Redis 缓存与 mock 降级语义对齐），再切主服务。另有 daemon 直调：`backend/workers/macro/alert_daemon.py:34-36`
