...
- [ ] **[BE-ARCH-07b]** **删除 `market_engine` 的 Futu 死门控**（Bug，非重构）：`backend/services/market_engine.py:302` `futu_connected = _get_futu_service().status == "CONNECTED"` 读取的是**主服务本地** Futu 状态，而 `backend/bootstrap/lifecycle.py:108-110` 已明确主服务不再 connect OpenD ⇒ 该值恒为 `False` ⇒ 把 `:343`（技术指标历史）/`:386`（资金流轮询）/`:427`（账户快照）/`:440`（Futu 快照补漏）四段**本身已合规**的 `data_source_router.fetch_futu` 调用全部废掉。改法：门控换为 Router 节点健康度（`data_source_router.get_health_status()`）或直接移除（Router 自带熔断，无需这层"防 CPU 空转"），顺带清理 `:34-38` 的 `_get_futu_service`。验收：四段逻辑在主服务无本地 OpenD 时可正常执行
- [ ] **[BE-ARCH-07c]** **主服务卸载 Futu SDK 连接层**：`backend/services/futu/` 仍完整持有 SDK —— `connection_manager.py:15-22`（`from futu import OpenQuoteContext`）、`data_source.py:53-68`（`LocalDataSource`）+ `source_router.py:13,30`、`watchdog.py:182,258,282`、`push_handler.py`（与 `data_subservice/futu_src/push_handler.py` 重复实现）；`lifecycle.py:394-397` shutdown 仍 `futu_service.close()`。另有两处仅取枚举的 SDK 依赖需替换为本地常量：`backend/engine/gateway.py:23`、`backend/workers/oms/algo_engine.py:655`（`from futu import TrdMarket, TrdSide`）。Kill Switch 残留本地 `trade_ctx`：`backend/services/adapters/legacy_broker.py:85-100`（同文件 `:50-75` 下单已合规走 `fetch_futu`，照抄即可）。**依赖 07a/07b 先完成**

#### P1 · 路由层与采集层直连外网

- [ ] **[BE-ARCH-07d]** **`routers/search` 切 search 适配器**：`backend/routers/search.py:6,22` → `backend/services/search/service.py` 直连 `api.tavily.com`（`:32`）、`api.bochaai.com`（`:67`）、`duckduckgo_search` SDK（`:101-117`）。合规通道**早已就位却闲置**：`backend/services/datasource/adapters/search.py:9`（文件头注释即写明"主服务不再直接 httpx 外部 API"）+ `:114-132` 三源注册 + `router.py:907` `fetch_search`。注意 Hermes `web_search_tool.py:52-65` 走的是后端 `/search/web`，所以本任务同时修掉 Agent 侧的实际外网出口。验收：`services/search/service.py` 无外部域名 + `pytest backend/tests/routers/test_datasource_search_adapters.py`
- [ ] **[BE-ARCH-07e]** **`routers/calendars` 切 finnhub 适配器**：`backend/routers/calendars.py:495-496`（`https://finnhub.io/api/v1/calendar/dividend`）、`:553-554`（`.../calendar/ipo`）在**路由函数内**裸 httpx 打外网，违反"Router 只做参数校验与转发"。改走 `data_source_router.fetch_finnhub`（`router.py:789` + action 映射 `:107-115`）或 Facade；子服务侧 handler 已支持（`data_subservice/finnhub_worker.py`）
- [ ] **[BE-ARCH-07f]** **`legacy_market_data` 的 AKShare + 宏观三源切远程**：`:26,30` 持有 `akshare_service`；`:49-72` 懒加载 `dbnomics_service`/`fred_service`/`rbi_service`；`:444-483` 日历远程失败后**本地 SDK 降级**（`:461-462`）；`:517-544` 宏观直调本地 service。本地连接层同步下沉/删除：`backend/services/macro/fred_service.py:25-27`（`api.stlouisfed.org`）、`macro/dbnomics.py:11,66`（`api.db.nomics.world`）、`macro/rbi.py:20,41-43`（`api.worldbank.org`）、`services/akshare/{quote.py:58,196,290 / flow.py:48,158,273,413 / calendar.py:55,68,82}`（后者还直连 `sina.com.cn` + `jin10.com`）。另有 daemon 直调：`backend/workers/macro/alert_daemon.py:34-36`（`fred_service.get_economic_calendar`）。合规通道已有：`adapters/akshare.py`、`adapters/macro.py`（fred/dbnomics/rbi）
- [ ] **[BE-ARCH-07g]** **AKShare Collector 去本地 SDK**：`backend/workers/akshare_collector.py:154-163` 强制 `AKSHARE_MODE=direct` 并实例化本地 `AKShareService`，与 CN 节点 `data_subservice`（`DS_CAPABILITIES=akshare`）平行采集，违反 `AGENTS.md` §9.4"限流敏感源优先独立辅节点出口"。改走 `data_source_router.fetch_akshare`（参照已合规的 `backend/workers/collectors/yfinance.py:46` 与 `collectors/fmp.py:77`）
- [ ] **[BE-ARCH-07h]** **长连接链路缺口修复**（当前仅 `Futu push → quant:quotes:stream → /market/quotes/ws` 一条真通）：① 宏观新闻频道名错配 —— 发 `live_news_channel`（`backend/workers/market/daemon.py:248`）而 WS 订 `macro_news`（`backend/routers/macro.py:181`）；② `macro_alerts` 有订阅无发布（`routers/macro.py:245`）；③ `oms:trades:new` 有订阅无发布（`backend/routers/oms.py:408`）；④ `quant:tick:{SYMBOL}` 订阅空转 —— `backend/services/datasource/subscription.py:164` 订阅，但子服务已移除 Finnhub WS（`data_subservice/_internal/finnhub/__init__.py:9` 注释说明），无任何 publisher；⑤ `QuotePublisher` 从未挂载 —— `backend/workers/quote_publisher.py:208` 仅 `__main__`，`worker.py`/`lifecycle.py` 均不启动；⑥ `AlertEngine`/`CEPEngine` 订阅代码在但未接入 lifecycle/worker，告警规则的行情触发链路断；⑦ 中间件 WS 白名单列了三个**不存在**的路径 `backend/middleware/stack.py:68-70`（`/api/v1/macro/quotes/ws`、`/api/v1/oms/quotes/ws`、`/api/v1/sse/`，真实为 `macro/news/ws`、`oms/ws`）；⑧ `futu:push:{ticker,broker,kline}:*` 子服务在发、主服务无消费者（`data_subservice/futu_src/push_handler.py:283,330,374`）
- [ ] **[BE-ARCH-07i]** **Facade action 与 capabilities 对齐 + Registry 回退收紧**：Facade 域方法用的 action 在对应适配器 `capabilities` 里**未声明** —— `WARRANT_CHAIN`（`facade.py:180`）/ `SCREEN_STOCKS`（`:189`）未在 `adapters/futu.py:53-60` 声明，`HSGT_HOLDERS`（`:198`）未在 `adapters/akshare.py:40-44` 声明，`INFO`/`FUNDAMENTAL` 与 `adapters/fmp.py:44-45` 的 `profile`/`income_statement` 命名不一致，yfinance 的 `fund_flow`/`option_chain` 有 Router 映射（`adapters/legacy_yfinance.py:27-46`）但 `:71-72` 未声明。叠加 `backend/services/datasource/source_registry.py:106-109` 在能力不匹配时**仍回退返回首个可用实例**，导致"按 action 选源"语义失效，只能靠 fetch 失败逐源重试兜底。改法：补齐 capabilities 声明 + 回退行为改为显式告警（或按 env 开关）

#### P2 · 散点下沉、死代码清理与守门测试

- [ ] **[BE-ARCH-07j]** **剩余业务模块直连下沉**：`backend/services/fund_flow/hk_sector.py:50`、`fund_flow/a_share_sector.py:48`、`services/margin/a_share.py:49`（均 `import akshare as ak`）；监管源直连 `services/margin/sources/finra.py:26,49`（`api.finra.org`）、`sources/hkex.py:56`、`sources/sfc.py:40`（外部 CSV，env 注入 URL）；`services/tushare/service.py:86,90-95`（`import tushare` + `tushare.pro`）与 `services/tushare/adapter.py:47-49,145-191`（**已注册进 Registry 却调本地 SDK 而非 Router**，且 `:215-217` 主节点无包时静默跳过注册）；`backend/core/yahoo_news.py:19,32`（`query2.finance.yahoo.com`，被 `services/akshare/quote.py:71-79` 新闻降级调用）
- [ ] **[BE-ARCH-07k]** **连接层死代码清理 + `health_check_url` 死配置**：`backend/services/finnhub/service.py:27,477`（完整 REST 客户端，生产已改走 Router，仅测试 import）；`services/fmp/service.py:19,28-41,76` 的 `_local_get` 本地兜底（任何人 import `fmp_service` 即触发外网直连，是定时炸弹）；`services/yfinance/search.py:60`、`yfinance/technical.py:33`（无生产 import 的死 mixin）。另：`backend/services/datasource/router.py` 11 处赋值 `health_check_url=f"{url}/api/v1/health"`（`:176,186,196,206,228,241,254,276,284,303`）**全仓零消费**（无主动探活），且与子服务实际路径 `/health`（`data_subservice/main.py:72`）不一致 → 要么接上探活并统一路径，要么删字段，勿留半成品
- [ ] **[BE-ARCH-07l]** **子服务配置陷阱修复**：① `data_subservice/nodeinfo.py:17-28` 解析 `DS_CAPABILITIES` **漏了 `.split(",")`**，逗号字符串会按字符迭代，导致注册进 Redis 的 capabilities 与 `main.py:82-84` 实际门控不一致；② 默认能力集 `main.py:83-84` 仅 `yfinance,akshare,tushare,fmp,futu`，**不含 finnhub/fred/dbnomics/rbi/search**，未显式声明则全部 503（`main.py:102-107`），需在部署文档与 `.env.example` 显著标注；③ `ENABLE_REDIS_HEARTBEAT=true` 时仅启动 `register` 一次（`main.py:173-180`），`_internal/service_registry.py:129` 的 `heartbeat()` 无周期调用循环，而 TTL 仅 30s（`:39`）→ 节点约 30s 后可能被判 dead；④ 逐笔频道注释与实现不一致（`futu_src/push_handler.py:10` 写 `quant:trades:stream:` vs `:283-285` 实发 `futu:push:ticker:{ticker}`）
- [ ] **[BE-ARCH-07m]** **Hermes Tools 外网直连收口**：`hermes_agent/tools/web_scrape_tool.py:125-135,185`（直连 `r.jina.ai` + 任意 URL httpx）、`insider_tool.py:123-126`（直连 `www1.hkexnews.hk`；同文件 `:112-115` 走后端 `/search/web` 是合规范例）、`download_report_tool.py:280`（直下 `sec.gov`/`hkexnews` PDF）。改法：走后端代理 —— jina 抓取经 `adapters/search.py` 的 `jina` 源（`router.py:907` `fetch_search`），或新增后端 `/search/scrape` + `/reports/download` 端点。注意 `tools/secure_client.py:8-30` 的白名单机制本身是合规防护层，不要误删
- [ ] **[BE-ARCH-07n]** **架构守门测试扩面**（防回归，建议与 07a 同批落地）：现有 `backend/tests/test_be_arch01_router_boundary.py` 只禁 **Router 层** 的数据源 import，未覆盖 `services/` 层，本次审计的绝大多数违规因此逃过门禁。新增守门测试：禁止 `backend/`、`hermes_agent/` 下出现 `import yfinance|akshare|tushare|finnhub|from futu import|from fredapi`，禁止外部数据源域名字面量（`finnhub.io`/`stlouisfed`/`db.nomics`/`tavily`/`bochaai`/`r.jina.ai`/`finance.yahoo.com`/`tushare.pro`/`finra.org` 等）；豁免清单：`data_subservice/**`、`backend/tests/**`、`scripts/**`、`adapters/*.py` 中的 `provider` 标识字符串
