...
### ❌ **Mock 数据填充逻辑 Anti-Pattern (非单测禁止)** ⭐⭐⭐⭐⭐

> **规则**：除**单元测试**（`tests/` 下的测试文件与 fixtures）外，**任何生产路径、路由端点、前端页面或演示代码，严禁用 mock / 假数据填充业务逻辑或喂给用户可见的结果**。

🚫 **BAD — 这些是禁止的**:
```python
# 后端端点未连真实数据源时，返回 MockProvider 编造的行情/期权/财务数据给前端渲染
# FutuAdapter._fetch_option_chain 在 dev 模式直接返回 Mock 假链
# 前端在无真实数据时用本地写死的假数组渲染图表/表格
# 用 mock 兜底绕过"数据源不可用"的错误展示，掩盖真实故障
```

✅ **GOOD — 唯一允许的例外是单测**:
```python
# 数据源不可用时：显式返回错误或空状态 + 明确告警（如 "数据源已死，无法分析"）
# 演示/Demo 用途如需假数据：必须置于独立 demo 模块，文件/函数前缀标注 `MOCK_DEMO`，且绝不进入主链路
# 单测中可用 mock/fixtures 隔离外部依赖（✅ 唯一合规的 mock 使用场景）
```

**Rationale**:
- Mock 填充会让"看起来能用"掩盖"实际没接真数据"，直接破坏零幻觉原则。
- 即使在沙箱演练中，用户也应基于真实行情/链路推演，否则结论无意义、不可信。
