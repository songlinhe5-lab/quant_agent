...
- [x] **P2.1** 核查消费场景：✅ 2026-08-22 完成。全库（排除 node_modules）**无任何 SG/MY/JP 策略线/研报/前端消费场景** → 结论：**不接，无增量价值**。同时核查底层：`quote_handler`（市场校验含 SG/JP）+ `screener_handler`（ScrMarket JP/SG）+ `format_ticker`（识别 JP./SG. 前缀）**已能处理 SG/JP 代码**——能取数但无消费场景。
- [x] **P2.2** 需求触发路径（无需现在执行）：✅ 因无需求不扩展；已确认若未来有需求，行情走 `quote_handler`（市场映射已含 SG/JP）、交易走 `trade_handler`、选股走 `screener_handler`（ScrMarket JP/SG 已映射），无需新增代码。待真实需求再启。
- [x] **P2.3** 记录决策到 `MEMORY.md`：✅ 新增 §11：新马日为长尾、无消费场景、暂不投入；并同步沉淀本次组合期权 P0/P0.5/P1 接口族清单（含 SDK 命名/枚举修正、沙箱约束）。

### 阶段 P3：文档与宣传对齐

- [x] **P3.1** 同步 `AGENTS.md` §2 期权工具描述：✅ 2026-08-22 核查后**无需改动**——当前 `AGENTS.md` 无期权工具描述（文档所引 §2 描述为历史/过期版本），不存在「单腿 vs 组合策略」宣传鸿沟对象。能力说明已由 docs/14 + MEMORY §11 + 本文档承担。
- [x] **P3.2** `MEMORY.md` 沉淀：✅ 已由 P2.3 完成（§11）：组合期权是期权能力质变（单腿→策略+损益）、交易类受沙箱约束（REAL_TRADE_EXECUTE+force_real）、新马日暂缓。
- [x] **P3.3** `DEPLOYMENT_CHECKLIST.md` + `docs/14` 补接入说明：✅ 2026-08-22 完成——`docs/14` L31 能力表扩展为「期权链+组合策略+期权全维」、L131 capabilities 表补 12 个新增 action + 追加组合期权接入说明（权限门槛 + 沙箱）；`DEPLOYMENT_CHECKLIST` Futu 表补 `REAL_TRADE_EXECUTE` 变量 + 期权接入说明（US/HK 期权可通无额外权限卡、组合交易默认 SIMULATE）。
