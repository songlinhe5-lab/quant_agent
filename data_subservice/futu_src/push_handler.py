...
async def _publish_quote_to_redis(ticker: str, quote_data: dict):
    """
    子服务本地的行情写入逻辑（替代 backend.services.market_engine.update_quote_to_redis）。

    仅做"数据写入"：将 QuoteData 序列化为 Protobuf 后：
      1. HSET 到 quant:quotes:latest（最新快照）
      2. PUBLISH 到 quant:quotes:stream（实时流）
    保持与主服务消费端 (market_engine WebSocket) 完全一致的序列化格式，使其零改动。
    报警检测等业务逻辑留在主服务侧基于 Redis 流触发，不污染数据服务。
    """
    try:
        from .proto.market_pb2 import Order, QuoteData
        ...
        quote_msg = QuoteData(
            ...
            source=str(quote_data.get("source", "futu")),
        )
        ...
        await rds.hset("quant:quotes:latest", ticker, payload_bytes)
        await rds.publish("quant:quotes:stream", payload_bytes)
    except Exception as e:  # noqa: BLE001
        logger.warning(f"[PushHandler] 写入 Redis 行情失败: {e}")
            ...
            try:
                for _, row in data.iterrows():
                    ...
                    async def _publish(qd=quote_dict, tk=ticker):
                        await _publish_quote_to_redis(tk, qd)
