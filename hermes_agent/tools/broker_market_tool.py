...
class BrokerMarketTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            "action": {
                "type": "string",
                "enum": ["QUOTE", "HISTORY", "OPTION_CHAIN", "FUND_FLOW", "CAPITAL_DISTRIBUTION", "WARRANT_CHAIN"],
                "description": "必须指定要获取的数据类型。QUOTE(实时快照), HISTORY(历史K线), OPTION_CHAIN(期权链), FUND_FLOW(主力资金流与席位), CAPITAL_DISTRIBUTION(主力筹码分层与背离信号), WARRANT_CHAIN(港股窝轮/牛熊证链，用于市场多空情绪分析)",
            },
    ...
    async def run(
        self, action: str, ticker: str, ktype: str = "K_DAY", num: int = 60, expiration_date: str = ""
    ) -> Dict[str, Any]:
        ...
        if action == "QUOTE":
            ...
            url, method, kwargs = (
                f"{backend_url}/market/history",
                "GET",
                {"params": {"ticker": ticker, "ktype": ktype, "num": num}},
            )
        elif action == "OPTION_CHAIN":
            url, method, kwargs = (
                f"{backend_url}/market/option-chain",
                "GET",
                {"params": {"ticker": ticker, "expiration_date": expiration_date}},
            )
            ...
            url, method, kwargs = f"{backend_url}/market/capital-distribution/{ticker}", "GET", {}
