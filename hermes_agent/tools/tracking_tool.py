...
class StockTrackingInput(BaseModel):
    ticker: str = Field(
        default="", description="股票代码，例如 US.AAPL, HK.00700。如果 action 是 list，可以传空字符串。"
    )
    action: Literal["add", "remove", "list"] = Field(
        ..., description="操作类型：add (加入长期监控), remove (取消监控), list (列出所有正在监控的股票)"
    )
    ...
    pct_change_threshold: Optional[float] = Field(
        default=None,
        description="可选：当日涨跌幅的异动报警阈值（纯数字，如 5 表示涨幅大于 5% 或跌幅小于 -5% 均触发报警）",
    )
...
class StockTrackingTool:
    ...
    async def run(
        ...
        pct_change_threshold: Optional[float] = None,
        user_id: str = "admin",
    ) -> str:
        try:
            # 1. 用户的个人监控池
            ...
            if action == "add":
                ...
                if upper_threshold is not None or lower_threshold is not None or pct_change_threshold is not None:
                    rules = {}
                    if upper_threshold:
                        rules["upper"] = upper_threshold
                    if lower_threshold:
                        rules["lower"] = lower_threshold
                    if pct_change_threshold:
                        rules["pct_change"] = pct_change_threshold
