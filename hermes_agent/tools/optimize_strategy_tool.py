...
class OptimizeStrategyTool(BaseTool):
    ...
    parameters = {
        "type": "object",
        "properties": {
            "source_code": {"type": "string", "description": "完整的 Python 策略类源码"},
            "class_name": {"type": "string", "description": "源码中的策略类名，例如 MACrossStrategy"},
            "param_grid": {
                "type": "object",
    ...
    async def run(
        self, source_code: str, class_name: str, param_grid: dict, target_metric: str = "sharpe_ratio"
    ) -> Dict[str, Any]:
        ...
        try:
            async with SecureAsyncClient(timeout=30.0) as client:
                resp = await client.post(f"{backend_url}/strategy/optimize-sandbox", json=payload)
                return resp.json()
