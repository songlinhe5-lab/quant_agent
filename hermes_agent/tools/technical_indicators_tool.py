...
class TechnicalIndicatorsTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            "ticker": {
                "type": "string",
                "description": "股票标准代码，例如 AAPL, 0700.HK"
            },
            "ma_periods": {
                "type": "array",
                "items": {"type": "integer"},
            ...
            "rsi_period": {
                "type": "integer",
                "description": "可选：需要计算的 RSI 周期，默认为 14"
            },
            "include_macd": {
                "type": "boolean",
                "description": "可选：是否计算 MACD (12, 26, 9) 指标，默认为 true"
            },
            "include_kdj": {
                "type": "boolean",
                "description": "可选：是否计算 KDJ (9, 3, 3) 随机指标，默认为 true"
            },
            "atr_period": {
                "type": "integer",
                "description": "可选：需要计算的 ATR 真实波幅周期，默认为 14"
            },
            ...
            "bbands_std_dev": {
                "type": "number",
                "description": "可选：布林带的标准差倍数，默认为 2.0"
            }
        },
        "required": ["ticker"]
    }
    ...
    async def run(self, ticker: str = "", ma_periods: Optional[List[int]] = None, rsi_period: int = 14, include_macd: bool = True, include_kdj: bool = True, atr_period: int = 14, stop_loss_multiplier: float = 2.0, take_profit_multiplier: float = 3.0, lookback_days: int = 1, bbands_period: int = 20, bbands_std_dev: float = 2.0) -> Dict[str, Any]:
        if not ticker:
