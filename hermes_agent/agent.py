...
class HermesAgent:
    ...
    """
    def __init__(self, tool_registry, system_prompt_path: str, session_id: str = "default", llm_client: Optional[AsyncOpenAI] = None, redis_client: Optional[redis.Redis] = None):
        ...
        for i in range(1, len(self.messages) - 4): # 避开最新的 4 条记录，保证当轮推理完整
            ...
            if msg.get("role") == "tool" and isinstance(msg.get("content"), str):
                if len(msg["content"]) > max_tool_len:
                    ...
                    msg["content"] = safe_truncate(msg["content"], max_tool_len, suffix="\n... [老旧数据已被系统折叠，省略 {omitted} 字符以释放内存] ...")

        # 2. 滑动窗口：如果消息数依然超过最大阈值，安全剥离最老的记录
        if len(self.messages) > max_messages:
            self.console.print(f"[dim yellow]🗜️ [Memory] 上下文达 {len(self.messages)} 条，触发滑动窗口自动瘦身...[/dim yellow]")
                ...
                for m in messages:
                    if m.get("role") == "user":
                        ...
                        if isinstance(c, str):
                            ...
                            user_content = next((item.get("text", "") for item in c if item.get("type") == "text"), "").strip()
                        if user_content:
                            break
                ...
                if user_content:
                    try:
                        ...
                        response = await self.client.chat.completions.create(
                            model=self.model,
                            messages=[
                                {"role": "system", "content": "你是一个标题生成器。请用极简、专业的中文（不超过3个词或10个汉字）精准总结用户的提问作为标题。严禁输出任何标点符号、引号或其他解释性文字。"},
        ...
        for i in range(max_iterations):
            ...
            try:
                ...
                if self.debug_mode:
                    ...
                    self.console.print(f"[dim]Messages: {json.dumps(request_kwargs['messages'], ensure_ascii=False, indent=2, default=str)}[/dim]")
                ...
                if self.debug_mode:
                    ...
                    self.console.print(f"[dim]{json.dumps(msg.model_dump(exclude_none=True), ensure_ascii=False, indent=2, default=str)}[/dim]")
        ...
        for i in range(max_iterations):
            ...
            try:
                ...
                if self.debug_mode:
                    ...
                    self.console.print(f"[dim]{json.dumps(request_kwargs['messages'], ensure_ascii=False, indent=2, default=str)}[/dim]")
                    self.console.print("[dim cyan]----------------------------------------------[/dim cyan]\n")
                        ...
                        for tc_chunk in delta.tool_calls:
                            ...
                            if idx not in tool_calls_dict:
                                tool_calls_dict[idx] = {
                                    "id": tc_chunk.id,
                                    "type": "function",
                                    "function": {"name": tc_chunk.function.name or "", "arguments": tc_chunk.function.arguments or ""}
                                }
                            else:
                ...
                if tool_calls_dict:
                    msg_dict["tool_calls"] = [{"id": tc["id"], "type": "function", "function": {"name": tc["function"]["name"], "arguments": tc["function"]["arguments"]}} for idx, tc in sorted(tool_calls_dict.items())]
                ...
                if tool_calls_dict:
                    for tc in msg_dict["tool_calls"]:
                        yield {"type": "tool_start", "name": tc["function"]["name"], "input": tc["function"]["arguments"]}

                    async def safe_execute(tc):
                        try:
                            # 💡 核心修复：execute 是 async 函数，必须 await
                            return await self.tool_registry.execute(tc["function"]["name"], **json.loads(tc["function"]["arguments"]))
                        except Exception as e:
                            return {"status": "error", "message": f"工具执行异常: {str(e)}"}

                    results = await asyncio.gather(*[safe_execute(tc) for tc in msg_dict["tool_calls"]], return_exceptions=True)
                    for tc, res in zip(msg_dict["tool_calls"], results):
                        final_res = {"status": "error", "message": str(res)} if isinstance(res, Exception) else res
                        self.messages.append({"role": "tool", "tool_call_id": tc["id"], "name": tc["function"]["name"], "content": json.dumps(final_res, ensure_ascii=False)})
                        # 抛出执行结果给前端或 CLI 终端展示
                    ...
                    if collected_content:
                        ...
                        is_correction_turn = len(self.messages) >= 2 and "系统校验拦截" in str(self.messages[-2].get("content", ""))
