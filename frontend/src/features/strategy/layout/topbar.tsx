...
import { useStrategyStore } from '@/features/strategy/stores/useStrategyStore'

export function Topbar() {
  const { isDirty, formSchema, activeStrategy } = useStrategyStore()
  ...
  return (
    <div className="h-12 border-b border-border/40 bg-secondary/20 flex items-center justify-between px-4 shrink-0 transition-colors duration-300">
      ...
      <div className="flex items-center gap-2">
        <Button size="sm" variant="ghost" className="h-7 text-xs gap-1.5 text-muted-foreground hover:text-foreground"><Save className="h-3.5 w-3.5"/> 保存</Button>
        <Button size="sm" variant="outline" className="h-7 text-xs gap-1.5 border-emerald-500/30 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10"><Play className="h-3.5 w-3.5"/> 运行沙箱</Button>
        <Button size="sm" className="h-7 text-xs gap-1.5 bg-primary/10 text-primary shadow-none hover:bg-primary/20"><Rocket className="h-3.5 w-3.5"/> 部署至 OMS</Button>
      </div>
    </div>
  )
}