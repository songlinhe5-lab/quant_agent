...
export function ScreenerResultsTable() {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[minmax(280px,360px)_1fr] gap-4">
      {/* ① 实时行情卡 */}
      <section className="glass-card rounded-[var(--radius-card)] overflow-hidden border border-border/40 shadow-sm flex flex-col">
        <div className="px-4 py-2.5 border-b border-border/30 flex items-center justify-between bg-secondary/30 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">实时行情</span>
            <span className="ml-1 bg-primary/10 text-primary px-1.5 py-0.5 rounded-md font-mono text-[10px]">{realDataLength}</span>
          </div>
          {wsStatus.connected ? (
            ...
            <span className="text-[10px] text-amber-500 bg-amber-500/10 px-2 py-0.5 rounded-full font-mono">STALE</span>
          )}
        </div>
        <DataState
          status={viewStatus}
        ...
        >
          <div className="overflow-auto flex-1 custom-scrollbar" style={{ maxHeight: 460 }}>
            <table className="w-full text-xs" role="table" aria-label="实时行情">
              <thead className="sticky top-0 z-10 bg-slate-50/90 dark:bg-zinc-900/90 backdrop-blur-md">
                <tr className="border-b border-border/40">
                  <SortableTh label="代码" k="symbol" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} align="left" className="min-w-[110px] pl-4" />
                  <SortableTh label="现价" k="price" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} className="min-w-[88px]" />
                  <SortableTh label="涨跌幅" k="chg" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} className="min-w-[88px]" />
                  <th scope="col" className="min-w-[120px] px-3 py-2 pr-4 text-right text-muted-foreground font-medium whitespace-nowrap">操作</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border/15">
                {paginatedData.map((r) => (
                  <ScreenerQuoteRow key={r.symbol} r={r} dynamicCols={dynamicCols} onPreview={setPreviewData} onSendToCopilot={handleSendToCopilot} onSendToBacktest={handleSendToBacktest} handleAddSingle={handleAddSingle} />
                ))}
              </tbody>
            </table>
          </div>
      ...
      <section className="glass-card rounded-[var(--radius-card)] overflow-hidden border border-border/40 shadow-sm flex flex-col">
        <div className="px-4 py-2.5 border-b border-border/30 flex items-center justify-between bg-secondary/30 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">量化指标</span>
            {useAgGrid && <span className="text-[10px] text-muted-foreground font-mono">AG Grid</span>}
          </div>
          <div className="flex items-center gap-2">
        ...
        {useAgGrid ? (
          ...
          >
            <div className="overflow-auto flex-1 custom-scrollbar" style={{ maxHeight: 460 }}>
              <table className="w-full text-xs" role="table" aria-label="量化指标">
