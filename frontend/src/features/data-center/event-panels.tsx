      ...
      <div className="w-full max-w-3xl h-full max-h-[85vh] bg-card border border-border/40 rounded-lg overflow-hidden flex flex-col shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-3 py-2 border-b border-border/30 bg-secondary/20">
          <h3 className="text-xs font-bold flex items-center gap-1.5 truncate pr-2">
            <AlertTriangle className={cn("h-3 w-3 shrink-0", isHigh ? "text-[hsl(var(--bear))] dark:text-[hsl(var(--bear))]" : isLow ? "text-sky-500 dark:text-sky-400" : "text-amber-500")} />
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded", isHigh ? "bg-[hsl(var(--bear))]/15 text-[hsl(var(--bear))] dark:text-[hsl(var(--bear))]" : "bg-secondary text-muted-foreground")}>{event.country}</span>
            ...
            <span className={cn('inline-flex px-1.5 py-0.5 rounded text-[10px] font-bold tracking-widest', isHigh ? 'text-[hsl(var(--bear))] dark:text-[hsl(var(--bear))]' : isLow ? 'text-sky-600/70 dark:text-sky-400/70' : 'text-amber-500/70 dark:text-amber-400/70')}>{isHigh ? '●●●' : isLow ? '●' : '●●'}</span>
          </div>
