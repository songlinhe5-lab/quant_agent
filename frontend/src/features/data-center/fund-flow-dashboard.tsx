    ...
    <div className="glass-card rounded-lg p-4 flex flex-col gap-3">
      ...
      <div className="flex items-end justify-between gap-3">
        <div className="flex flex-col">
          ...
          <span className={cn('text-2xl font-bold font-mono tabular-nums', positive ? 'text-[hsl(var(--bull))] dark:text-[hsl(var(--bull))]' : 'text-[hsl(var(--bear))] dark:text-[hsl(var(--bear))]')}>
            {fmt(raw)}
        ...
        {spark && spark.length >= 2 && (
          <div className="opacity-80">
            {positive ? <TrendingUp className="h-4 w-4 text-[hsl(var(--bull))]" /> : <TrendingDown className="h-4 w-4 text-[hsl(var(--bear))]" />}
    ...
    <div className="glass-card rounded-lg p-4 flex flex-col gap-3">
      ...
      <div className="flex items-end justify-between gap-3">
        <div className="flex flex-col">
          ...
          <span className={cn('text-2xl font-bold font-mono tabular-nums', positive ? 'text-[hsl(var(--bull))]' : 'text-[hsl(var(--bear))]')}>
            {fmt(total)}
            <span className="text-xs ml-1 opacity-70">{data.unit || '亿元'}</span>
          </span>
        </div>
        {positive ? <TrendingUp className="h-5 w-5 text-[hsl(var(--bull))]" /> : <TrendingDown className="h-5 w-5 text-[hsl(var(--bear))]" />}
      </div>
          ...
          return (
            <div key={c.board} className="flex items-center justify-between gap-2 text-[11px]">
              ...
              <span className="flex items-center gap-2">
                <span className={cn('font-mono tabular-nums', pos ? 'text-[hsl(var(--bull))]' : 'text-[hsl(var(--bear))]')}>{fmt(c.net_buy)}</span>
                <span className="text-muted-foreground/50 font-mono">
                  涨 {c.up ?? 0} / 跌 {c.down ?? 0}
                </span>
                {c.index_chg !== null && c.index_chg !== undefined && (
                  <span className={cn('font-mono', (c.index_chg ?? 0) >= 0 ? 'text-[hsl(var(--bull))]' : 'text-[hsl(var(--bear))]')}>
                    {(c.index_chg ?? 0) >= 0 ? '▲' : '▼'}
        ...
        {data.breakdown.slice(0, 5).map((b) => {
          ...
          return (
            <div key={b.ticker} className="flex items-center justify-between text-[11px]">
              ...
              <span className={cn('font-mono tabular-nums', pos ? 'text-[hsl(var(--bull))]' : 'text-[hsl(var(--bear))]')}>{fmt(b.net_inflow)}</span>
            </div>
          )
        })}
      </div>
        ...
        <>
          {tab === 'a' && (
            <div className="flex flex-col gap-3">
              <FlowCard title="北向资金 (外资买 A 股)" point={data.northbound} accent="hsl(var(--bull))" />
          ...
          {tab === 'hk' && (
            <div className="flex flex-col gap-3">
              <FlowCard title="南向资金 (港股通净买入)" point={data.southbound} accent="hsl(var(--bear))" />
          ...
          {tab === 'us' && (
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <FlowCard title="北向资金 (参照)" point={data.northbound} accent="hsl(var(--bull))" />
                <FlowCard title="南向资金 (参照)" point={data.southbound} accent="hsl(var(--bear))" />
              </div>
            </div>
          )}
        </>
