...
export function FlowItem({ item, onClick }: { item: CapitalFlowItem; onClick?: () => void }) {
  ...
  return (
    <div onClick={onClick} role="button" tabIndex={0} className={cn('flex flex-col justify-center gap-1 px-2.5 py-1.5 rounded-lg border border-border/20 overflow-hidden transition-colors cursor-pointer', 'bg-slate-50 dark:bg-secondary/10 hover:bg-slate-100 dark:hover:bg-secondary/30 focus:outline-none focus:ring-1 focus:ring-primary/50', 'border-l-2', item.market === 'HK' ? 'border-l-[hsl(var(--bear))]/50' : item.market === 'CN' ? 'border-l-amber-500/50' : 'border-l-blue-500/50', flash === 'up' && 'animate-flash-green', flash === 'down' && 'animate-flash-red')}>
      <div className="flex items-center gap-1.5 w-full">
        ...
        {hasAmount ? (
          <span className={cn('text-xs font-bold font-mono tabular-nums whitespace-nowrap transition-colors duration-500', inflow ? 'text-[hsl(var(--bull))] dark:text-[hsl(var(--bull))]' : 'text-[hsl(var(--bear))] dark:text-[hsl(var(--bear))]')}>{inflow ? '+' : ''}{item.amount.toFixed(1)}<span className="text-[8px] ml-0.5 opacity-60">{item.unit}</span></span>
        ) : (
          <span className="text-xs font-bold font-mono tabular-nums whitespace-nowrap text-muted-foreground">--</span>
        )}
        <span className={cn('text-[9px] font-mono font-bold px-1.5 py-0.5 rounded flex-shrink-0 transition-colors duration-300 ml-auto', hasAmount ? (inflow ? 'bg-[hsl(var(--bull))]/15 text-[hsl(var(--bull))] dark:text-[hsl(var(--bull))]' : 'bg-[hsl(var(--bear))]/15 text-[hsl(var(--bear))] dark:text-[hsl(var(--bear))]') : 'bg-secondary/20 text-muted-foreground')}>{statusBadge}</span>
        <svg width="36" height="14" viewBox="0 0 36 14" aria-hidden="true" className="flex-shrink-0 opacity-60 ml-1.5">
          ...
          {item.sparkDirs && item.sparkDirs.length >= 2 && (
            <path
              ...
              fill="none" stroke={!hasAmount ? '#64748b' : inflow ? (isDark ? 'hsl(var(--bull))' : 'hsl(var(--bull))') : (isDark ? 'hsl(var(--bear))' : 'hsl(var(--bear))')} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
            />
          )}
        </svg>
      </div>
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1.5 custom-scrollbar">
            {history.slice().reverse().map((h, i) => (
              <div key={i} className="flex justify-between items-center text-xs px-3 py-2 rounded-md bg-secondary/10 hover:bg-secondary/30 transition-colors border border-border/50">
                ...
                <span className={cn("font-bold font-mono tabular-nums", h.dir > 0 ? "text-[hsl(var(--bull))] dark:text-[hsl(var(--bull))]" : "text-[hsl(var(--bear))] dark:text-[hsl(var(--bear))]")}>{h.dir > 0 ? '+' : '-'}{h.val.toFixed(2)} {flow.unit}</span>
              </div>
            ))}
          </div>
        ) : (
