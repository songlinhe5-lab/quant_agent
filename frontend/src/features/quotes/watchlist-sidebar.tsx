...
const SortableWatchlistItem = React.memo(function SortableWatchlistItem({ item, isSelected, theme, allowDrag, onSelect, onRemove }: {
  ...
  return (
    <SymbolContextMenu symbol={item.symbol} onRemove={onRemove} onSelect={() => onSelect()}>
      ...
      >
        <div className="flex items-center justify-between gap-2">
          ...
          <button onClick={(e) => { e.stopPropagation(); onRemove(item.symbol) }} className="p-0.5 text-muted-foreground hover:text-red-500 dark:hover:text-red-400 hover:bg-red-500/10 dark:hover:bg-red-400/10 rounded transition-colors z-10" title="移除自选" aria-label={`移除 ${item.symbol}`}><X className="h-3 w-3" /></button>
        </div>
...
export const WatchlistSidebar = React.memo(function WatchlistSidebar({ watchlist, selectedSymbol, setSelectedSymbol, theme, toggleWatchlist, addTicker, removeTicker, reorderWatchlist, latestStatsRef }: { watchlist: WatchlistItem[]; selectedSymbol: string; setSelectedSymbol: (sym: string) => void; theme?: string; toggleWatchlist: () => void; addTicker: (sym: string) => void; removeTicker: (sym: string) => void; reorderWatchlist: (o: number, n: number) => void; latestStatsRef: React.MutableRefObject<Record<string, {change: number, vol: number}>> }) {
  ...
  const listStatus = resolveDataStatus({ empty: displayWatchlist.length === 0 })
  const useVirtual = displayWatchlist.length > 60 && sortMode !== 'manual'

  return (
    <Panel defaultSize={20} minSize={15} maxSize={30} className="flex flex-col bg-background/50 glass-card rounded-xl shadow-sm border-border/40 overflow-hidden relative">
      <div className="px-3 py-2.5 border-b border-border/40 bg-secondary/20 flex items-center justify-between shrink-0"><span className="text-[11px] font-bold tracking-widest uppercase text-muted-foreground flex items-center gap-1.5"><Eye className="w-3.5 h-3.5" /> 自选池</span><div className="flex items-center gap-1"><button onClick={() => { setSortMode(prev => prev === 'change_desc' ? 'change_asc' : 'change_desc'); setSortTick(t => t + 1) }} className={cn("p-1 rounded transition-colors text-muted-foreground", sortMode.includes('change') ? "bg-primary/20 text-primary" : "hover:bg-secondary/80")} title="涨跌幅排序" aria-label="按涨跌幅排序"><ArrowUpDown className="w-3.5 h-3.5" /></button><Button variant="ghost" size="icon" onClick={toggleWatchlist} className="h-6 w-6 rounded-md hover:bg-secondary/80" title="收起列表" aria-label="收起自选列表"><ChevronLeft className="w-4 h-4" /></Button></div></div>
      <div className="px-2 py-2 border-b border-border/20 bg-background/50 shrink-0"><div className="relative"><Search className="absolute left-2 top-1/2 -translate-y-1/2 h-3 w-3 text-muted-foreground" /><input type="text" placeholder="搜索或添加标的..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && searchQuery) { addTicker(searchQuery.toUpperCase()); setSearchQuery('') } }} className="w-full h-7 bg-secondary/30 border border-border/50 rounded-md pl-7 pr-2 text-[11px] font-mono focus:outline-none focus:border-primary/50 transition-colors" aria-label="搜索或添加自选标的" /></div></div>
      <div className="flex-1 overflow-y-auto custom-scrollbar relative">
        <DataState status={listStatus} emptyTitle="自选为空" emptyDescription="搜索添加标的，或右键行情列表加入自选" skeletonRows={4}>
          {useVirtual ? (
            <VirtualList
            ...
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
              <SortableContext items={displayWatchlist.map(w => w.symbol)} strategy={verticalListSortingStrategy}>
                {displayWatchlist.map(item => (
                  <SortableWatchlistItem
                    ...
                    isSelected={selectedSymbol === item.symbol}
                    theme={theme}
                    allowDrag={sortMode === 'manual' && !searchQuery}
                    onSelect={() => setSelectedSymbol(item.symbol)}
                    onRemove={removeTicker}
                  />
                ))}
