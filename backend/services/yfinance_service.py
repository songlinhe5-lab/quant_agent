...
class YFinanceService:
    ...
    async def fetch_yf_data(self, ticker: str, fetch_type: str, ttl: int, persist: bool = False, **kwargs) -> Tuple[bool, Any, str]:  # noqa: E501
        ...
        cache_key = f"yf_{fetch_type}_{yf_ticker}" + ("_" + "_".join([f"{k}_{v}" for k, v in kwargs.items()]) if kwargs else "")  # noqa: E501
        ...
        if cache_key in self._error_cache:
            if now - self._error_cache[cache_key] < 300:
                return False, None, f"数据源限流冷却中：{yf_ticker} 请求过于频繁，请等待几分钟后重试"  # noqa: E501
            else:
        ...
        if cache_key in self._cache:
            ...
            if now - ts < ttl:
                if not (fetch_type == "history" and getattr(data, "empty", False)) and not (fetch_type == "info" and len(data) <= 1):  # noqa: E501
                    return True, data, ""
        ...
        try:
            def _do_fetch():
                ...
                res = yf.Ticker(yf_ticker, session=self.session).info if fetch_type == "info" else yf.download(yf_ticker, **kwargs)  # noqa: E501
                ...
                if yf_shared is not None:
                    ...
                    if errs:
                        ...
                        if "429" in err_str or "Rate limit" in err_str or "Too Many Requests" in err_str or "YFRateLimitError" in err_str:  # noqa: E501
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    print(f"⚠️ [YFinance API] 第 {attempt+1} 次获取到空数据 (疑似软限流/Cookie失效) -> Ticker: {yf_ticker}")  # noqa: E501
                    ...
                    if "429" in err_str or "Rate limit" in err_str or "Too Many Requests" in err_str or "YFRateLimitError" in err_str:  # noqa: E501
                        ...
                        return False, None, "限流冷却中：yfinance 触发了 429 限流保护。已开启全局熔断，请等待 60 秒后重试。"  # noqa: E501
            ...
            return False, None, f"重试 {max_retries} 次后仍然失败，获取 {yf_ticker} 数据无效"  # noqa: E501
        except Exception as e:
    ...
    async def get_batched_quote(self, ticker: str, req_type: str = "quote", **kwargs) -> Dict[str, Any]:  # noqa: E501
        ...
        kwargs_str = "_".join([f"{k}_{str(v).replace(' ','')}" for k, v in sorted(kwargs.items())]) if kwargs else "default"  # noqa: E501
        ...
        async with self._batch_lock:
            ...
            self._batch_queue[yf_ticker].append({"fut": fut, "req_type": req_type, "kwargs": kwargs, "cache_key": cache_key})  # noqa: E501
        ...
        try:
                    ...
                    try:
                        ...
                        if isinstance(local_df.columns, pd.MultiIndex):
                            if t in local_df.columns.levels[0] or t in local_df:
                                ...
                                ticker_df = local_df.xs(t, axis=1, level=1).dropna(how="all") if t in local_df.columns.get_level_values(1) else pd.DataFrame()  # noqa: E501
                            else:
            ...
            for t, reqs in batch.items():
                try:
                    ...
                    for req in reqs:
                        fut = req["fut"]
                        if fut.done(): continue  # noqa: E701
                        ...
                        if req["req_type"] == "quote":
                            ...
                            prev_close = float(ticker_df['Close'].iloc[-2]) if len(ticker_df) > 1 else float(ticker_df['Open'].iloc[-1])  # noqa: E501
                            change_pct = ((last_price - prev_close) / prev_close) * 100 if prev_close else 0.0  # noqa: E501
                            res = {"status": "success", "ticker": t, "last_price": round(last_price, 4), "change_pct": f"{change_pct:+.2f}%", "source": "yfinance_batch"}  # noqa: E501
                        elif req["req_type"] == "tech":
                            # 0 网络延迟回调！直接将切片好的 DataFrame 数据喂给技术面计算引擎  # noqa: E501
                            res = await self.get_tech_indicators(ticker=t, pre_fetched_df=cast(pd.DataFrame, ticker_df), **req["kwargs"])  # noqa: E501
                        ...
                        if res and res.get("status") == "success":
                            ...
                            if not res: res = {"status": "error", "message": "处理失败"}  # noqa: E701
    ...
    async def get_tech_indicators(self, ticker: str, ma_periods: Optional[List[int]] = None, rsi_period: int = 14, include_macd: bool = True, include_kdj: bool = True, atr_period: int = 14, stop_loss_multiplier: float = 2.0, take_profit_multiplier: float = 3.0, lookback_days: int = 1, bbands_period: int = 20, bbands_std_dev: float = 2.0, pre_fetched_df: Optional[pd.DataFrame] = None) -> Dict[str, Any]:  # noqa: E501
        if ma_periods is None: ma_periods = [10, 20]  # noqa: E701
        lookback_days = max(1, min(lookback_days, 30))

        df = pre_fetched_df
        if df is None:
            success, df, msg = await self.fetch_yf_data(ticker, "history", ttl=3600, persist=False, period="6mo", progress=False)  # noqa: E501
            if not success:
                # 将降级条件放宽，一旦获取失败就使用 mock 数据进行界面展示
                if msg == "development_mock" or "数据无效" in msg or "限流冷却" in msg:
                    fallback_data = self._mock_tech_data(ticker, ma_periods, rsi_period, include_macd, atr_period, stop_loss_multiplier, take_profit_multiplier, lookback_days, bbands_period, bbands_std_dev)  # noqa: E501
        ...
        try:
            ...
            def _compute_tech(local_df: pd.DataFrame):
                ...
                open_series = cast(pd.Series, local_df['Open'].squeeze() if isinstance(local_df.columns, pd.MultiIndex) else local_df['Open'])  # noqa: E501
                close_series = cast(pd.Series, local_df['Close'].squeeze() if isinstance(local_df.columns, pd.MultiIndex) else local_df['Close'])  # noqa: E501
                high_series = cast(pd.Series, local_df['High'].squeeze() if isinstance(local_df.columns, pd.MultiIndex) else local_df['High'])  # noqa: E501
                low_series = cast(pd.Series, local_df['Low'].squeeze() if isinstance(local_df.columns, pd.MultiIndex) else local_df['Low'])  # noqa: E501
                volume_series = cast(pd.Series, local_df['Volume'].squeeze() if isinstance(local_df.columns, pd.MultiIndex) else local_df['Volume'])  # noqa: E501

                ma_dict = {p: close_series.rolling(window=p).mean() for p in ma_periods}
                rsi_series = None
                if rsi_period and rsi_period > 0:
                    delta = close_series.diff()
                    gain, loss = delta.where(delta > 0, 0.0), -delta.where(delta < 0, 0.0)  # noqa: E501
                    rs = gain.ewm(alpha=1/rsi_period, adjust=False).mean() / loss.ewm(alpha=1/rsi_period, adjust=False).mean()  # noqa: E501
                    rsi_series = 100 - (100 / (1 + rs))

                macd_hist, macd_line, signal_line = None, None, None
                if include_macd:
                    macd_line = close_series.ewm(span=12, adjust=False).mean() - close_series.ewm(span=26, adjust=False).mean()  # noqa: E501
                ...
                if atr_period and atr_period > 0:
                    tr = pd.concat([high_series - low_series, (high_series - close_series.shift(1)).abs(), (low_series - close_series.shift(1)).abs()], axis=1).max(axis=1)  # noqa: E501
                ...
                if bbands_period and bbands_period > 0:
                    ...
                    bb_upper, bb_lower = bb_middle + bbands_std_dev * bb_std, bb_middle - bbands_std_dev * bb_std  # noqa: E501

                trend_data = []
                for i in range(-lookback_days, 0):
                    if i < -len(close_series): continue  # noqa: E701
                    day_res = {"date": str(close_series.index[i].date()), "open": round(float(open_series.iloc[i]), 2), "high": round(float(high_series.iloc[i]), 2), "low": round(float(low_series.iloc[i]), 2), "close": round(float(close_series.iloc[i]), 2), "volume": int(volume_series.iloc[i])}  # noqa: E501
                    for p in ma_periods: day_res[f"MA_{p}"] = round(float(ma_dict[p].iloc[i]), 2)  # noqa: E501, E701
                    if rsi_series is not None: day_res[f"RSI_{rsi_period}"] = round(float(rsi_series.iloc[i]), 2)  # noqa: E501, E701
                    if include_macd and (macd_line is not None) and (signal_line is not None) and (macd_hist is not None): day_res.update({"MACD_line": round(float(macd_line.iloc[i]), 3), "MACD_signal": round(float(signal_line.iloc[i]), 3), "MACD_hist": round(float(macd_hist.iloc[i]), 3)})  # noqa: E501, E701
                    if include_kdj and (k_series is not None) and (d_series is not None) and (j_series is not None): day_res.update({"KDJ_K": round(float(k_series.iloc[i]), 2), "KDJ_D": round(float(d_series.iloc[i]), 2), "KDJ_J": round(float(j_series.iloc[i]), 2)})  # noqa: E501, E701
                    if atr_series is not None:
                        curr_atr = float(atr_series.iloc[i])
                        day_res[f"ATR_{atr_period}"] = round(curr_atr, 3)
                        if ma_periods and day_res.get(f"MA_{ma_periods[0]}"): day_res.update({"trailing_stop_loss": round(day_res[f"MA_{ma_periods[0]}"] - stop_loss_multiplier * curr_atr, 2), "take_profit": round(day_res[f"MA_{ma_periods[0]}"] + take_profit_multiplier * curr_atr, 2)})  # noqa: E501, E701
                    if (bb_middle is not None) and (bb_upper is not None) and (bb_lower is not None): day_res.update({f"BB_middle_{bbands_period}": round(float(bb_middle.iloc[i]), 2), f"BB_upper_{bbands_period}": round(float(bb_upper.iloc[i]), 2), f"BB_lower_{bbands_period}": round(float(bb_lower.iloc[i]), 2)})  # noqa: E501, E701

                    actions = []
                    if i - 1 >= -len(close_series):
                        if include_macd and macd_hist is not None: actions.append("buy (MACD金叉)") if macd_hist.iloc[i] > 0 and macd_hist.iloc[i-1] <= 0 else actions.append("sell (MACD死叉)") if macd_hist.iloc[i] < 0 and macd_hist.iloc[i-1] >= 0 else None  # noqa: E501, E701
                        if include_kdj and (k_series is not None) and (d_series is not None): actions.append("buy (KDJ金叉)") if k_series.iloc[i] > d_series.iloc[i] and k_series.iloc[i-1] <= d_series.iloc[i-1] else actions.append("sell (KDJ死叉)") if k_series.iloc[i] < d_series.iloc[i] and k_series.iloc[i-1] >= d_series.iloc[i-1] else None  # noqa: E501, E701
                        if ma_periods and len(ma_periods) >= 2: actions.append(f"buy (MA{ma_periods[0]}上穿MA{ma_periods[1]})") if ma_dict[ma_periods[0]].iloc[i] > ma_dict[ma_periods[1]].iloc[i] and ma_dict[ma_periods[0]].iloc[i-1] <= ma_dict[ma_periods[1]].iloc[i-1] else actions.append(f"sell (MA{ma_periods[0]}下穿MA{ma_periods[1]})") if ma_dict[ma_periods[0]].iloc[i] < ma_dict[ma_periods[1]].iloc[i] and ma_dict[ma_periods[0]].iloc[i-1] >= ma_dict[ma_periods[1]].iloc[i-1] else None  # noqa: E501, E701
                        if (bb_upper is not None) and (bb_lower is not None): actions.append("buy (突破布林带上轨)") if close_series.iloc[i] > bb_upper.iloc[i] and close_series.iloc[i-1] <= bb_upper.iloc[i-1] else actions.append("sell (跌破布林带下轨)") if close_series.iloc[i] < bb_lower.iloc[i] and close_series.iloc[i-1] >= bb_lower.iloc[i-1] else None  # noqa: E501, E701
                        ...
                        if rsi_series is not None and i - 5 >= -len(close_series):
                            ...
                            if close_series.iloc[i] < close_series.iloc[i-1] and close_series.iloc[i] <= close_series.iloc[i-5:i].min() and rsi_series.iloc[i] > rsi_series.iloc[i-1] and rsi_series.iloc[i] < 40:  # noqa: E501
                                if is_shrink:
                            ...
                            elif close_series.iloc[i] > close_series.iloc[i-1] and close_series.iloc[i] >= close_series.iloc[i-5:i].max() and rsi_series.iloc[i] < rsi_series.iloc[i-1] and rsi_series.iloc[i] > 60:  # noqa: E501
                                if is_shrink:
                ...
                return {"status": "success", "data": {"ticker": ticker, "lookback_days": len(trend_data), "trend": trend_data}}  # noqa: E501
    ...
    def _mock_tech_data(self, ticker: str, ma_periods: List[int], rsi_period: int, include_macd: bool, atr_period: int, stop_loss_multiplier: float, take_profit_multiplier: float, lookback_days: int, bbands_period: int, bbands_std_dev: float) -> Dict[str, Any]:  # noqa: E501
        trend = []
        for i in range(lookback_days):
            day_data = {"date": f"2026-05-{20+i}", "open": 144.5 + i, "high": 146.5 + i, "low": 143.5 + i, "close": 145.5 + i, "volume": 1200000 + i * 50000, "MA_10": 145.5, "MA_20": 142.1, "RSI_14": 65.4, "trend_score": 85, "action": "buy (MACD金叉)" if i == lookback_days - 1 else "hold"}  # noqa: E501
            if include_macd: day_data.update({"MACD_line": 1.25, "MACD_signal": 0.85, "MACD_hist": 0.40})  # noqa: E501, E701
            if atr_period: day_data.update({f"ATR_{atr_period}": 5.94, "trailing_stop_loss": round(145.5 - (stop_loss_multiplier * 5.94), 2), "take_profit": round(145.5 + (take_profit_multiplier * 5.94), 2)})  # noqa: E501, E701
            if bbands_period: day_data.update({f"BB_middle_{bbands_period}": 142.1, f"BB_upper_{bbands_period}": 148.5, f"BB_lower_{bbands_period}": 135.7})  # noqa: E501, E701
            trend.append(day_data)
        return {"status": "success", "message": "未安装依赖，返回 Mock 数据", "data": {"ticker": ticker, "lookback_days": lookback_days, "trend": trend}}  # noqa: E501
    ...
    async def search_tickers(self, query: str) -> Dict[str, Any]:
        ...
        if time.time() < self._circuit_breaker_until:
            return {"status": "warning", "message": "雅虎搜索接口全局限流熔断中，返回空列表", "data": []}  # noqa: E501
            ...
            try:
                ...
                for q in data.get("quotes", []):
                    ...
                    if q.get("quoteType") in ["EQUITY", "ETF", "INDEX"]:
                        results.append({"symbol": q.get("symbol"), "name": q.get("shortname", q.get("longname", "Unknown")), "type": q.get("quoteType")})  # noqa: E501
                        ...
                        results.append({"symbol": futu_sym, "name": q.get("shortname", q.get("longname", "Unknown")), "type": q.get("quoteType")})  # noqa: E501
                ...
                if "429" in str(e) or "Too Many Requests" in str(e):
                    ...
                    self._circuit_breaker_until = time.time() + 60.0
                    return {"status": "warning", "message": "雅虎搜索接口触发限流，返回空列表", "data": []}  # noqa: E501
            ...
            try:
                def _do_batch_fetch():
                    ...
                    res = yf.download(
                        ...
                        threads=False, progress=False, session=self.session # 🚨 核心：必须禁用多线程  # noqa: E501
                    )
                ...
                for attempt in range(max_retries):
                    ...
                    if attempt < max_retries - 1:
                        backoff = random.uniform(2.0, 5.0) * (2 ** attempt)
                        print(f"  🔄 [YF Daemon] 准备第 {attempt+2} 次重试，重置 Session 并退避休眠 {backoff:.1f} 秒...")  # noqa: E501
                        await asyncio.sleep(backoff)
                        self._init_session()
                ...
                if df is not None and not df.empty:
                            ...
                            try:
                                if isinstance(local_df.columns, pd.MultiIndex):
                                    if ticker in local_df.columns.levels[0] or ticker in local_df:  # noqa: E501
                                        ...
                                        ticker_df = local_df.xs(ticker, axis=1, level=1).dropna(how="all") if ticker in local_df.columns.get_level_values(1) else pd.DataFrame()  # noqa: E501
                                    else:
                                        continue
                                else:
                                ...
                                if not ticker_df.empty:
                                    ...
                                    cache_list.append((f"yf_macro_cache_{ticker}", df_reset.to_json(orient="records", date_format="iso")))  # noqa: E501
                                    ...
                                    try:
                                        if 'Close' in ticker_df.columns:
                                            ...
                                            chg_pct = (last_price - prev_price_for_chg) / prev_price_for_chg * 100 if prev_price_for_chg > 0 else 0.0  # noqa: E501
                                            daily_snap[ticker] = {"price": last_price, "change": chg_pct, "date": last_date}  # noqa: E501
                    ...
                    loop = asyncio.get_running_loop()
                    daily_snapshot, cache_updates, alerts = await loop.run_in_executor(self._executor, _process_macro_data, df)  # noqa: E501
                    ...
                    try:
                        ...
                        if est_now.hour == 16 and last_summary_date != current_date and est_now.weekday() < 5 and is_trading_day:  # noqa: E501
                            last_summary_date = current_date
                            core_assets = {
                                "^GSPC": "标普500", "^IXIC": "纳斯达克", "^VIX": "恐慌指数", "DX-Y.NYB": "美元指数", "^TNX": "10年期美债", "GC=F": "黄金", "BTC-USD": "比特币",  # noqa: E501
                                "XLK": "科技板块", "XLF": "金融板块", "XLE": "能源板块", "KWEB": "中概互联"  # noqa: E501
                            }
                            ...
                            # 💡 新增：大模型盘后一句话犀利点评
                            try:
                                market_data_str = ", ".join([f"{name} {daily_snapshot[t]['price']:.2f} ({'+' if daily_snapshot[t]['change'] > 0 else ''}{daily_snapshot[t]['change']:.2f}%)" for t, name in core_assets.items() if t in daily_snapshot])  # noqa: E501
