...
def parse_strategy_parameters(source_code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "message": f"代码缩进错误: {e.msg} (第 {e.lineno} 行)。请检查空格与 Tab。"}
    except SyntaxError as e:
        return {"status": "error", "message": f"代码语法错误: {e.msg} (第 {e.lineno} 行)"}
        ...
        if isinstance(node, ast.ClassDef):
            ...
            for class_body_item in node.body:
                if isinstance(class_body_item, ast.FunctionDef) and class_body_item.name == '__init__':
                    ...
                    docstring = ast.get_docstring(class_body_item) or ast.get_docstring(node) or ""
                    param_descs = {}
                    for line in docstring.split("\n"):
                        ...
                        if m_sphinx:
                            ...
                            continue
                        # 匹配 Google 风格: `fast_ma (int): 快速均线周期` 或 `fast_ma: 快速均线周期`
                        m_google = re.match(r'^(\w+)\s*(?:\([^)]+\))?:\s*(.*)', line)
                        # 排除掉特殊的保留字
                        if m_google and m_google.group(1) not in ("param", "type", "return", "rtype"):
    ...
    if not strategies:
        return {"status": "error", "message": "架构规范缺失: 未检测到继承自 BaseStrategy 的策略类，或缺失带有参数的 __init__ 方法。请将逻辑封装为标准策略 (第 1 行)"}
