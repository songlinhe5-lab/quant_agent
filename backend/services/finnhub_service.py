...
class FinnhubService:
    ...
    async def get_earnings_calendar(self, days_ahead: int = 7, skip_cache: bool = False) -> Dict[str, Any]:  # noqa: E501
        ...
        if not skip_cache:
            try:
                ...
                if cached_data:
                    return {"status": "success", "data": json.loads(cached_data), "source": "redis_cache"}  # noqa: E501
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0, verify=False, proxy=self._get_proxy(), event_hooks={'request': [httpx_log_request], 'response': [httpx_log_response]}) as client:  # noqa: E501
            ...
            return {"status": "error", "message": f"Finnhub 财报日历请求异常: HTTP {e.response.status_code}"}  # noqa: E501
        except Exception as e:
    ...
    async def get_stock_history(self, ticker: str, days_back: int = 365) -> Dict[str, Any]:  # noqa: E501
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0, verify=False, proxy=self._get_proxy(), event_hooks={'request': [httpx_log_request], 'response': [httpx_log_response]}) as client:  # noqa: E501
                response = await client.get(url, params=params)
                response.raise_for_status()
                data = response.json()

                if data.get("s") != "ok":
                    return {"status": "error", "message": f"Finnhub 返回状态: {data.get('s')}"}  # noqa: E501

                df_data = []
                for i in range(len(data.get("t", []))):
                    dt = datetime.fromtimestamp(data["t"][i], timezone.utc).strftime("%Y-%m-%d %H:%M:%S")  # noqa: E501
                    df_data.append({"time": dt, "open": float(data["o"][i]), "high": float(data["h"][i]), "low": float(data["l"][i]), "close": float(data["c"][i]), "volume": float(data["v"][i])})  # noqa: E501

                try:
                    await redis_client.setex(cache_key, 3600, json.dumps(df_data)) # 缓存 1 小时，彻底防范频控  # noqa: E501
                except Exception:
                    pass
    ...
    async def get_insider_transactions(self, ticker: str, limit: int = 30) -> Dict[str, Any]:  # noqa: E501
        ...
        try:
            ...
            return {"status": "error", "message": f"Finnhub 内幕交易请求异常: HTTP {e.response.status_code}"}  # noqa: E501
        except Exception as e:
    ...
    async def get_market_news(self, category: str = "general") -> Dict[str, Any]:
        ...
        try:
            ...
            if e.response.status_code == 429:
                return {"status": "error", "message": "Finnhub 免费版 API 触发 60次/分钟 限流 (429 Too Many Requests)，请稍后重试。"}  # noqa: E501
            return {"status": "error", "message": f"Finnhub 新闻接口请求异常: HTTP {e.response.status_code}"}  # noqa: E501
        except Exception as e:
    ...
    async def get_company_news(self, ticker: str, days_back: int = 3, skip_cache: bool = False) -> Dict[str, Any]:  # noqa: E501
        ...
        if not skip_cache:
            try:
                ...
                if cached_news:
                    return {"status": "success", "data": json.loads(cached_news), "source": "redis_cache"}  # noqa: E501
            except Exception as e:
        ...
        async with self._locks[cache_key]:
            if not skip_cache:
                try:
                    ...
                    if cached_double:
                        return {"status": "success", "data": json.loads(cached_double), "source": "redis_cache"}  # noqa: E501
                except Exception:
                    pass
            ...
            try:
                ...
                if e.response.status_code in (403, 429):
                    ...
                    if fallback_data:
                        ...
                        return {"status": "success", "data": fallback_data, "source": "yahoo_fallback"}  # noqa: E501

                    return {
                        "status": "error",
                        "message": f"{reason} 且 Yahoo 兜底失败。Finnhub 免费版 API 仅支持美股或已达请求上限。请改用网络搜索工具获取。"  # noqa: E501
                    }
                return {"status": "error", "message": f"Finnhub 个股新闻请求异常: HTTP {e.response.status_code}"}  # noqa: E501
            except Exception as e:
                return {"status": "error", "message": f"Finnhub 个股新闻请求异常: {str(e)}"}  # noqa: E501
                ...
                for row in earnings_list:
                    ...
                    if eps_actual is not None or rev_actual is not None:
                        ...
                        if is_new:
                            ...
                            def fmt_num(val):
                                if val in ("--", None, ""): return "--"  # noqa: E701
                                try:
                                    fval = float(val)
                                    if fval >= 1e9: return f"{fval/1e9:.2f}B"  # noqa: E701
                                    if fval >= 1e6: return f"{fval/1e6:.2f}M"  # noqa: E701
                                    return str(val)
                                except Exception:
                                    return str(val)
            ...
            async with httpx.AsyncClient(timeout=10.0, verify=False, proxy=self._get_proxy(), event_hooks={'request': [httpx_log_request], 'response': [httpx_log_response]}) as client:  # noqa: E501
                ...
                for item in news_list:
                    ...
                    formatted_news.append({
                        ...
                        "summary": item.get("publisher", "Yahoo Finance"), # Yahoo搜索通常无长摘要，使用出版方占位  # noqa: E501
            ...
            try:
                ...
                if res.get("status") == "success":
                    ...
                    for news_item in reversed(news_items):
                        headline = news_item.get("headline", "")
                        if not headline: continue  # noqa: E701
                                ...
                                if monitored_tickers:
                                    for t in monitored_tickers:
                                        ...
                                        if symbol.startswith("US."):
                                            symbol = symbol[3:]
                                        if not symbol.startswith("HK.") and not symbol.startswith("SH.") and not symbol.startswith("SZ."):  # noqa: E501
                        ...
                        while True:
                            ...
                            if msg_type == "trade":
                                ...
                                for news_item in news_list:
                                    ...
                                    await redis_client.publish("live_news_channel", json.dumps(news_item, ensure_ascii=False))  # noqa: E501
            ...
            try:
                ...
                for row in events:
                    ...
                    if actual_val is not None:
                        ...
                        event_date = str(row.get("time", "")).split(" ")[0] if " " in str(row.get("time", "")) else "today"  # noqa: E501
    ...
    async def _insider_transactions_marquee_daemon(self) -> None:
        ...
        MAJOR_TICKERS = ["US.AAPL", "US.MSFT", "US.NVDA", "US.GOOG", "US.AMZN", "US.META",  # noqa: E501
                    ...
                    if res.get("status") == "success" and res.get("data"):
                        ...
                        for tx in transactions:
                            ...
                            if transaction_value >= 100000 or abs(tx.get("change", 0)) >= 10000:  # noqa: E501
                                ...
                                tx_hash = hashlib.md5(json.dumps(tx_hash_data, sort_keys=True).encode('utf-8')).hexdigest()  # noqa: E501

                                # 确保是今天或最近的交易
                                tx_date_str = tx.get("date", "")
                                if not tx_date_str: continue  # noqa: E701
                                try:
                                ...
                                except ValueError: continue  # noqa: E701

                                # 使用 Redis set NX (Not Exists) 原子操作进行去重，确保每个交易只入库一次  # noqa: E501
                                is_new = await redis_client.set(f"{DEDUP_KEY}:{tx_hash}", "1", nx=True, ex=86400 * 7) # 7天去重周期  # noqa: E501
                                if is_new:
                                    # 将交易时间戳作为 ZSET 的 Score，便于按时间排序
                                    await redis_client.zadd(MARQUEE_KEY, {json.dumps(tx_hash_data): tx_timestamp})  # noqa: E501
                                new_transactions_count += 1

                if new_transactions_count > 0:
                    print(f"✨ [Finnhub Daemon] 检测到 {new_transactions_count} 条新的显著高管交易，已推送到跑马灯队列。")  # noqa: E501
