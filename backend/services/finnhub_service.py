...
class FinnhubService:
    ...
    async def get_earnings_calendar(self, days_ahead: int = 7, skip_cache: bool = False) -> Dict[str, Any]:
        """获取近期财报日历"""
        ...
        if not skip_cache:
            try:
                ...
                if cached_data:
                    return {"status": "success", "data": json.loads(cached_data), "source": "redis_cache"}
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0, verify=False, proxy=self._get_proxy(), event_hooks={'request': [httpx_log_request], 'response': [httpx_log_response]}) as client:
                ...
                # 💡 强制按日期正序排列 (从今天到未来)，解决默认返回可能把 7 天后排在前面的问题
            ...
            return {"status": "error", "message": f"Finnhub 财报日历请求异常: HTTP {e.response.status_code}"}
        except Exception as e:
    ...
    async def get_stock_history(self, ticker: str, days_back: int = 365) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0, verify=False, proxy=self._get_proxy(), event_hooks={'request': [httpx_log_request], 'response': [httpx_log_response]}) as client:
                response = await client.get(url, params=params)
                response.raise_for_status()
                data = response.json()
                
                if data.get("s") != "ok":
                    return {"status": "error", "message": f"Finnhub 返回状态: {data.get('s')}"}
                    
                df_data = []
                for i in range(len(data.get("t", []))):
                    dt = datetime.fromtimestamp(data["t"][i], timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
                    df_data.append({"time": dt, "open": float(data["o"][i]), "high": float(data["h"][i]), "low": float(data["l"][i]), "close": float(data["c"][i]), "volume": float(data["v"][i])})
                    
                try:
                    await redis_client.setex(cache_key, 3600, json.dumps(df_data)) # 缓存 1 小时，彻底防范频控
                except Exception:
                    pass
    ...
    async def get_insider_transactions(self, ticker: str, limit: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "message": f"Finnhub 内幕交易请求异常: HTTP {e.response.status_code}"}
        except Exception as e:
    ...
    async def get_market_news(self, category: str = "general") -> Dict[str, Any]:
        ...
        try:
            ...
            if e.response.status_code == 429:
                return {"status": "error", "message": "Finnhub 免费版 API 触发 60次/分钟 限流 (429 Too Many Requests)，请稍后重试。"}
            return {"status": "error", "message": f"Finnhub 新闻接口请求异常: HTTP {e.response.status_code}"}
        except Exception as e:
            return {"status": "error", "message": f"Finnhub 新闻接口请求异常: {str(e)}"}

    async def get_company_news(self, ticker: str, days_back: int = 3, skip_cache: bool = False) -> Dict[str, Any]:
        """获取个股专属新闻"""
        api_key = self._get_api_key()
        if not api_key:
            return {"status": "error", "message": "系统未配置 FINNHUB_API_KEY"}
        ...
        if not skip_cache:
            try:
                ...
                if cached_news:
                    return {"status": "success", "data": json.loads(cached_news), "source": "redis_cache"}
            except Exception as e:
        ...
        async with self._locks[cache_key]:
            if not skip_cache:
                try:
                    ...
                    if cached_double:
                        return {"status": "success", "data": json.loads(cached_double), "source": "redis_cache"}
                except Exception:
                    pass
            ...
            try:
                ...
                if e.response.status_code in (403, 429):
                    reason = "403 权限拒绝" if e.response.status_code == 403 else "429 触发限流"
                    print(f"⚠️ [Finnhub] {reason}，正在尝试使用 Yahoo Finance 兜底获取 {symbol} 新闻...")
                    fallback_data = await self._fallback_yahoo_news(symbol)
                    if fallback_data:
                        try:
                            ttl = 86400 + random.randint(600, 3600)
                            await redis_client.set(cache_key, json.dumps(fallback_data), ex=ttl)
                        except Exception as cache_e:
                            print(f"⚠️ [Yahoo Fallback] Redis 缓存写入异常: {cache_e}")
                        return {"status": "success", "data": fallback_data, "source": "yahoo_fallback"}
                        
                    return {
                        "status": "error", 
                        "message": f"{reason} 且 Yahoo 兜底失败。Finnhub 免费版 API 仅支持美股或已达请求上限。请改用网络搜索工具获取。"
                    }
                return {"status": "error", "message": f"Finnhub 个股新闻请求异常: HTTP {e.response.status_code}"}
            except Exception as e:
                return {"status": "error", "message": f"Finnhub 个股新闻请求异常: {str(e)}"}
                    ...
                    if eps_actual is not None or rev_actual is not None:
                        ...
                        if is_new:
                            ...
                            def fmt_num(val):
                                ...
                                try:
                                    fval = float(val)
                                    if fval >= 1e9: return f"{fval/1e9:.2f}B"
                                    if fval >= 1e6: return f"{fval/1e6:.2f}M"
                                    return str(val)
                                except Exception:
                                    return str(val)
                                    
                            msg = f"🚨 [重磅财报出炉]\n\n🏢 巨头: {symbol} (Q{quarter})\n💵 实际 EPS: {eps_actual} (预期: {eps_est})\n💰 实际营收: {fmt_num(rev_actual)} (预期: {fmt_num(rev_est)}){ai_comment}\n\n⚠️ 财报已发布，盘前/盘后价格可能发生剧烈跳空，请注意期权 IV Crush 风险！"
    ...
    async def run_global_daemon(self) -> None:
        ...
        await asyncio.gather(
            ...
            self._insider_transactions_marquee_daemon(), # 新增：高管内幕交易跑马灯守护进程
    ...
    async def _news_stream_daemon(self) -> None:
        ...
        # 启动时先主动拉取一次全量数据，作为冷启动时的初始缓冲，防止 WebSocket 断开期间遗漏新闻
        try:
            print("🔄 [Finnhub Daemon] 正在通过 HTTP 拉取初始新闻快照以填充 ZSET...")
            # 增加 asyncio.wait_for 提供绝对的协程级超时打断，防止 REST 假死阻塞后续 WS 建立
            init_res = await asyncio.wait_for(self.get_market_news("general"), timeout=15.0)
            if init_res.get("status") == "success":
                ...
                if new_items:
                    ...
                    for news_item in scored_items:
                        ...
                        news_item["tags"] = self._generate_news_tags(text_content, rules)
        ...
        while True:
            ...
            try:
                ...
                if res.get("status") == "success":
                    ...
                    # 💡 及时释放大对象：清理数百条新闻列表，防止在下一次循环顶部的 60 秒 sleep 中常驻内存
                    res = None
                    news_items = None
                    new_incoming = None
                    chunk = None
        ...
        while True:
            ...
            try:
                ...
                for raw_ticker in monitored_tickers:
                    ...
                    # 💡 分布式分片 (Sharding) 防御：如果该标的不属于当前节点负责的哈希槽，直接跳过
                    ...
                    if is_asian_stock:
                        ...
                        res = await self.get_company_news(ticker, days_back=3, skip_cache=True)
    ...
    async def _trade_stream_daemon(self) -> None:
        ...
        while True:
            try:
                # ping_interval=20, ping_timeout=20 是保持长连接稳定且防被服务端踢出的关键参数
                            ...
                            try:
                                ...
                                if monitored_tickers:
                                    for t in monitored_tickers:
                                        ...
                                        # 💡 HA 容灾感知：根据最新的存活 Worker 数，动态判断该标的是否归我管
                                        if not is_my_shard(symbol):
                                            continue
                                            
                                        if symbol.startswith("US."):
                                            symbol = symbol[3:]
                                        if not symbol.startswith("HK.") and not symbol.startswith("SH.") and not symbol.startswith("SZ."):
                    ...
                    try:
                        while True:
                            ...
                            if msg_type == "trade":
                                ...
                                # 🚀 兼容 Premium 账户：如果服务端推送了实时新闻，直接广播
                                news_list = data.get("data", [])
                                print(f"🎉 [Finnhub WS] 收到 {len(news_list)} 条 Premium 实时新闻推送！")
                                for news_item in news_list:
                                    # 推送至全局新闻通道，前端 WebSocket 或策略引擎可直接订阅
                                    await redis_client.publish("live_news_channel", json.dumps(news_item, ensure_ascii=False))
                                    
                            elif msg_type == "ping":
                                # 🚨 致命缺陷修复：应用层的心跳机制！
                                # websockets 库底层只会回复协议层的 Ping (Opcode 9)，
                                # 这种封装在 Text Frame (Opcode 1) 里的 JSON Ping 必须手动回复，否则会被 Finnhub 强制踢下线！
    ...
    async def _macro_alert_daemon(self) -> None:
        ...
        while True:
            # 💡 轮询间隔缩短为 60 秒，保证高危数据 (如 LPR/MLF/非农) 在出炉 1 分钟内推送
            await asyncio.sleep(60)
            try:
                from backend.services.akshare_service import akshare_service
                ...
                for row in events:
                    ...
                    if actual_val is not None:
                        ...
                        dedup_key = f"quant:macro:notified:{event_date}:{hashlib.md5(event_name.encode('utf-8')).hexdigest()}"
                        ...
                        if is_new:
                            ...
                            ai_comment = ""
                            try:
                                prompt = f"作为华尔街顶级宏观分析师，请用一句话解读以下刚刚发布的宏观数据：\n事件: {event_name}\n国家: {country}\n公布值: {actual_val}\n预期值: {estimate_val}\n前值: {previous_val}\n\n请直接对比公布值与预期值，判断是超预期还是不及预期，并明确指出对该国股市及货币是利空还是利多，说明理由。字数限制在60字以内，不许输出多余的解释与Markdown格式。"
                                resp = await llm_service.get_client().chat.completions.create(
                                    model=llm_service.get_model(),
                                    temperature=0.4,
                                    messages=[{"role": "user", "content": prompt}]
    ...
    async def _insider_transactions_marquee_daemon(self) -> None:
        ...
        MAJOR_TICKERS = ["US.AAPL", "US.MSFT", "US.NVDA", "US.GOOG", "US.AMZN", "US.META",
        ...
        while True:
            ...
            try:
                ...
                for ticker in MAJOR_TICKERS:
                    # 调用现有的 get_insider_transactions 方法，它自带缓存和 Finnhub 格式转换
                    ...
                    if res.get("status") == "success" and res.get("data"):
                        ...
                        for tx in transactions:
                            ...
                            # 💡 过滤条件：仅关注交易金额大于 $100,000 的大额交易，或者股数大于 10000 股的交易
                            if transaction_value >= 100000 or abs(tx.get("change", 0)) >= 10000:
                                ...
                                try:
                                    ...
                                    if (datetime.now().date() - tx_date).days > 3: # 只保留最近3天的交易
                                        continue
                                    # 💡 修复：将 date 对象转换为 datetime 对象以获取 timestamp
                                    tx_datetime = datetime.combine(tx_date, datetime.min.time())
                                    tx_timestamp = tx_datetime.timestamp()
                                except ValueError: continue
                                
                                # 使用 Redis set NX (Not Exists) 原子操作进行去重，确保每个交易只入库一次
                                is_new = await redis_client.set(f"{DEDUP_KEY}:{tx_hash}", "1", nx=True, ex=86400 * 7) # 7天去重周期
                                if is_new:
                                    # 将交易时间戳作为 ZSET 的 Score，便于按时间排序
                                    await redis_client.zadd(MARQUEE_KEY, {json.dumps(tx_hash_data): tx_timestamp})
