...
import pandas as pd
from futu import (
    RET_OK, TrdEnv, TrdMarket, OrderType, ModifyOrderOp, 
    TrdSide
)
...
class TradeHandler:
    ...
    async def place_order(self, ticker: str, qty: int, price: float, 
                         trd_side: TrdSide, market: TrdMarket,
                         format_ticker_func=None) -> Dict[str, Any]:
        """下单（模拟盘）"""
        if format_ticker_func is None:
            from .utils import format_ticker
            format_ticker_func = format_ticker
            
        trd_ctx = self.conn_mgr.get_trade_context(market=market, trd_env=TrdEnv.SIMULATE)
        ...
        return {"status": "success", "message": f"委托已提交(模拟盘)！订单号: {oid}", "order_id": oid}
    ...
    async def modify_order(self, order_id: str, op: ModifyOrderOp, 
        ...
        return {"status": "success", "message": f"撤单指令已提交(模拟盘)！被撤单号: {order_id}"}
    ...
    async def get_account_info(self, market: str = "HK") -> Dict[str, Any]:
        ...
        # 开发环境 Mock
        if self.conn_mgr.status != "CONNECTED" and os.getenv("QUANT_ENV") == "development":
        ...
        try:
            ...
            ret, data = await asyncio.to_thread(
                trd_ctx.accinfo_query, trd_env=trd_env
            )
            if ret != RET_OK: 
            ...
            if isinstance(data, pd.DataFrame) and not data.empty:
                ...
                if ret_pos == RET_OK and isinstance(data_pos, pd.DataFrame) and not data_pos.empty:
                    display_cols = [
                        'code', 'stock_name', 'position_side', 'qty', 
                        'can_sell_qty', 'cost_price', 'market_val', 'pl_val', 'pl_ratio'
                    ]
                    positions = data_pos[
