...
class QuoteHandler:
    ...
    @with_global_retry
    async def get_quote(self, ticker: str, format_ticker_func, is_unsupported_func) -> Dict[str, Any]:
        """获取实时行情（带L1缓存）"""
        ...
        if topic not in self.cache_mgr.subscribed_topics:
            ret, msg = self.conn_mgr.quote_ctx.subscribe(
                [market_ticker], [SubType.QUOTE], subscribe_push=False, extended_time=True
            )
            if ret != RET_OK: 
    ...
    @with_global_retry
    async def get_history(self, ticker: str, ktype: str = "K_DAY", num: int = 60) -> Dict[str, Any]:
        """获取历史K线数据（带缓存和降级策略）"""
        ...
        if cached and now - cached[0] < 60.0:
            ...
            if data.get("status") == "success" and "data" in data:
                if len(data["data"]) >= num:
                    return {"status": "success", "ticker": market_ticker, "data": data["data"][-num:]}
            else:
                return data
        ...
        if topic not in self.cache_mgr.subscribed_topics:
            sub_ret, _ = await asyncio.to_thread(
                self.conn_mgr.quote_ctx.subscribe, [market_ticker], [st], subscribe_push=False
            )
            if sub_ret == RET_OK:
                self.cache_mgr.subscribed_topics.add(topic)
        ...
        if ret != RET_OK or not isinstance(df, pd.DataFrame) or df.empty:
            ...
            ret, df, page_key = await asyncio.to_thread(
                ...
                market_ticker, start=None, end=None, ktype=kt, autype=AuType.QFQ, max_count=num
            )
    ...
    @with_global_retry
    async def unsubscribe_quote(self, ticker: str, format_ticker_func) -> Dict[str, Any]:
        ...
        try:
            def _do_unsub():
                # 退订高频占用槽位的行情推送类型 (包含基础报价、深度摆盘、逐笔成交、席位等)
                sub_types = [SubType.QUOTE, SubType.ORDER_BOOK, SubType.TICKER, SubType.BROKER, SubType.K_DAY]
                ret, data = self.conn_mgr.quote_ctx.unsubscribe([market_ticker], sub_types)
    ...
    @with_global_retry
    async def get_order_book(self, ticker: str, format_ticker_func, is_unsupported_func) -> Dict[str, Any]:
        """获取实时 Level 2 盘口深度数据"""
        if is_unsupported_func(ticker):
            return {"status": "error", "message": "富途原生不支持该大类资产"}
        ...
        result = {"status": "success", "ticker": market_ticker, "bids": bids, "asks": asks}
