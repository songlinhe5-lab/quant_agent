...
class ScreenerHandler:
    ...
    async def screen_stocks(self, market: str = "HK", 
        ...
        """
        print(f"👉 [ScreenerHandler] screen_stocks 被调用 -> 市场: {market}, 过滤条件: {filters}")
        if self.conn_mgr.status != "CONNECTED" or not self.conn_mgr.quote_ctx:
            return {"status": "error", "message": "FutuService 未连接"}
        ...
        try:
            ...
            from futu.quote.stock_screen_const import (
                ...
                BasicProperty, FeaturedProperty, OptionProperty,
                Indicator, Pattern, KlineShapeProperty,
                BrokerProperty, Position
            )
        except ImportError:
            return {"status": "error", "message": "当前 futu-api 版本不支持 V2 选股接口 (StockScreenRequest) 相关常量，请升级 futu-api。"}
        ...
        has_plate = any(f.get("type") == "plate" and f.get("value") for f in (filters or []))

        # 1. 基础市场条件 (💡 重点：Futu 底层 Market 和 Plate 是“并集”关系，若指定了板块绝不能加 Market，否则会变回全市场)
        # if not has_plate:
        print(f"👉 [ScreenerHandler] add_simple_field: MARKET, [{mkt}]")
        req.add_simple_field(SimpleField.MARKET, [mkt])
    
        # 2. 必须的基础信息取回
        print(f"👉 [ScreenerHandler] add_retrieve_basic: CODE")
        req.add_retrieve_basic(BasicProperty.CODE)
        print(f"👉 [ScreenerHandler] add_retrieve_basic: NAME")
        req.add_retrieve_basic(BasicProperty.NAME)
        print(f"👉 [ScreenerHandler] add_retrieve_basic: INDUSTRY")
        ...
        if filters:
            has_plate = any(f.get("type") in ["plate", "exclude_plate"] for f in filters)
            if has_plate:
                try:
                    ...
                    ret, plate_df = await asyncio.to_thread(self.conn_mgr.quote_ctx.get_plate_list, get_plate_mkt, Plate.ALL)
                    if ret == RET_OK and isinstance(plate_df, pd.DataFrame) and not plate_df.empty:
        ...
        if filters:
            for f in filters:
                ...
                def get_enum(enum_cls, name, default=None):
                    ...
                    # 💡 兼容处理：将 V1 版本的旧名称映射为 V2 版本的标准属性名，防止底层找不到枚举导致条件静默丢失
                    legacy_map = {
                        ...
                        "PROFIT_TO_SHAREHOLDERS_GROWTH_RATE": "SHAREHOLDER_PROFIT_GROWTH",
                        "PROFIT_BEFORE_TAX_GROWTH_RATE": "PROFIT_BEFORE_TAX_GROWTH",
                        "NOCF_PER_SHARE_GROWTH_RATE": "NOCF_PER_SHARE_GROWTH",
                        "OPERATING_PROFIT_TO_TOTAL_PROFIT": "OPERATING_PROFIT_TOTAL_RATIO",
                    }
                    if name_str in legacy_map and hasattr(enum_cls, legacy_map[name_str]):
                    ...
                    if not expected_enum or not get_enum(expected_enum, field_name):
                        for correct_type, enum_cls in type_mapping.items():
                            if get_enum(enum_cls, field_name):
                                print(f"🔧 [ScreenerHandler] 智能纠偏: 字段 {field_name} 的类型从 {f_type} 被自动纠正为 {correct_type}")
                                f_type = correct_type
                                f["type"] = correct_type
                                break

                def format_intervals(raw_intervals, l_val=None, u_val=None):
                    # 严格按照 Futu SDK 文档格式组装: [{'lower': {'value':v,'includes':b}, 'upper':...}]
                    res_list = []
                    if raw_intervals:
                        for intv in raw_intervals:
                            item = {}
                            l = intv.get("lower", intv.get("min"))
                            u = intv.get("upper", intv.get("max"))
                            if l is not None: item["lower"] = {"value": float(l), "includes": True}
                            if u is not None: item["upper"] = {"value": float(u), "includes": True}
                            if item: res_list.append(item)
                    elif l_val is not None or u_val is not None:
                        item = {}
                        if l_val is not None: item["lower"] = {"value": float(l_val), "includes": True}
                        if u_val is not None: item["upper"] = {"value": float(u_val), "includes": True}
                ...
                def get_period(period_str):
                    ...
                    return period_map.get(period_str, getattr(__import__('futu').Period, "DAY", 11))

                try:
                    # 💡 自动兜底格式化：防范 LLM 忘记输出市场前缀 (如把 US.BK2991 输出成了 BK2991)
                    def format_plate(code: str) -> str:
                        # 💡 行业名动态翻译：如果传入的是纯中文行业名，利用字典翻译为 Futu 板块代码
                    ...
                    if f_type == "exclude_plate" and f.get("value"):
                        exclude_plate_codes.extend([format_plate(p) for p in f.get("value")])
                        continue
                    ...
                    if f_type == "simple":
                        ...
                        if prop:
                            print(f"👉 [ScreenerHandler] add_simple_property: {prop}, lower={lower}, upper={upper}")
                    ...
                    # 3. 财务属性
                        ...
                        if prop:
                            ...
                            fin_kwargs = {}
                            for arg_k in ["lower_included", "upper_included", "duration", "continuous_period", "period_average", "future_duration", "unit"]:
                                if arg_k in f and f[arg_k] is not None:
                                    fin_kwargs[arg_k] = f[arg_k]
                            print(f"👉 [ScreenerHandler] add_financial_property: {prop}, term={term}, lower={lower}, upper={upper}, kwargs={fin_kwargs}")
                            req.add_financial_property(prop, term=term, lower=lower, upper=upper, **fin_kwargs)
                            print(f"👉 [ScreenerHandler] add_retrieve_financial: {prop}, term={term}")
                    ...
                    # 4. 累计行情属性
                        ...
                        if prop:
                            days = f.get("days", 1)
                            print(f"👉 [ScreenerHandler] add_cumulative_property: {prop}, days={days}, lower={lower}, upper={upper}")
                            req.add_cumulative_property(prop, days=days, lower=lower, upper=upper)
                            print(f"👉 [ScreenerHandler] add_retrieve_cumulative: {prop}, days={days}")
                    ...
                    # 5. 特色指标
                        ...
                        if prop:
                            fmt_intervals = format_intervals(f.get("intervals"), lower, upper)
                            print(f"👉 [ScreenerHandler] add_featured_property: {prop}, intervals={fmt_intervals}")
                    ...
                    # 6. 技术指标形态
                        ...
                        if prop:
                            ...
                            print(f"👉 [ScreenerHandler] add_indicator_pattern: {prop}, period_type={period_type}")
                            req.add_indicator_pattern(prop, period_type=period_type)
                            
                            # 💡 智能附加回包展示对应的具体指标 (如 RSI_BOTTOM_DIVERGE 截取 RSI)
                            base_indicator_str = field_name.split("_")[0]
                            base_ind = get_enum(Indicator, base_indicator_str)
                            if base_ind:
                                print(f"👉 [ScreenerHandler] add_retrieve_indicator (pattern 附加): {base_ind}, period_type={period_type}")
                    ...
                    # 7. 技术指标位置关系
                        ...
                        if prop1 and pos:
                            period_type = get_period(f.get("period", "K_DAY"))
                            print(f"👉 [ScreenerHandler] add_indicator_positional: pos={pos}, period_type={period_type}, prop1={prop1}, prop2={prop2}")
                            req.add_indicator_positional(pos, period_type, prop1, prop2)
                            print(f"👉 [ScreenerHandler] add_retrieve_indicator: {prop1}, period_type={period_type}")
                            req.add_retrieve_indicator(prop1, period=period_type)
                            user_fields.add(field_name)
                            if prop2:
                                print(f"👉 [ScreenerHandler] add_retrieve_indicator: {prop2}, period_type={period_type}")
                    ...
                    # 8. K线形态
                        ...
                        if prop:
                            ...
                            print(f"👉 [ScreenerHandler] add_kline_shape: {prop}, period={period}")
                            req.add_kline_shape(prop, period=period)
                            print(f"👉 [ScreenerHandler] add_retrieve_kline_shape: {prop}, period={period}")
                    ...
                    # 9. 经纪商持股
                        ...
                        if prop:
                            days = f.get("days", 1)
                            print(f"👉 [ScreenerHandler] add_broker_holdings: {prop}, days={days}")
                            req.add_broker_holdings(prop, days=days)
                            print(f"👉 [ScreenerHandler] add_retrieve_broker: {prop}, days={days}")
                    ...
                    # 10. 期权指标
                        ...
                        if prop:
                            period = f.get("period")
                            fmt_intervals = format_intervals(f.get("intervals"), lower, upper)
                            print(f"👉 [ScreenerHandler] add_option: {prop}, intervals={fmt_intervals}, period={period}")
                            req.add_option(prop, intervals=fmt_intervals, period=period)
                            print(f"👉 [ScreenerHandler] add_retrieve_option: {prop}, period={period}")
                            req.add_retrieve_option(prop, period)
                            
                    else:
                        return {"status": "error", "message": f"大模型虚构了不支持的指标字段: '{field_name}'。"}
        ...
        for f_name, f_type in default_return_fields:
            if f_name not in user_fields:
                if f_type == "accumulate" and hasattr(CumulativeProperty, f_name):
                    ...
                    print(f"👉 [ScreenerHandler] add_retrieve_cumulative (default): {prop}, days=1")
                ...
                if not filters and f_name == "PRICE" and hasattr(SimpleProperty, f_name):
                    prop = getattr(SimpleProperty, f_name)
                    print(f"👉 [ScreenerHandler] add_simple_property (default): {prop}, lower=0.001, upper=None")
        ...
        if hasattr(SimpleProperty, "MARKET_CAP"):
            ...
            print(f"👉 [ScreenerHandler] set_sort: direction=DESC, property_type='simple', property_params={{'name': {int(prop)}}}")
            req.set_sort(direction=ScrSortDir.DESC, property_type='simple', property_params={'name': int(prop)})
        ...
        all_dfs = []
        begin = 0
        num = 2000  # 💡 扩大单次拉取数量，实现“一波流”请求，避免每次 200 条导致的过度碎片化网络通信
        max_total = 10000
            
        try:
            async with self.screen_lock:
                try:
                    # 💡 修复超时陷阱：将 timeout 移到锁内部，只计算真实网络耗时，不计入排队等待时间
                    async with asyncio.timeout(14.0):
                        ...
                        while begin < max_total:
                            ...
                            print(f"👉 [ScreenerHandler] get_stock_screen: 发起请求 (page_from={begin}, page_count={num})")
                            ...
                            if ret != RET_OK:
                                if not all_dfs:
                                    ...
                                    return {"status": "error", "message": f"选股失败: {raw_data}"}
                                break
                                
                            if isinstance(raw_data, tuple):
                                if len(raw_data) >= 3:
                                    is_last_page, all_count, items = raw_data[0], raw_data[1], raw_data[2]
                                elif len(raw_data) == 2:
                            ...
                            try:
                                from futu.quote.stock_screen_const import (
                                    BasicProperty, SimpleProperty, FinancialProperty, CumulativeProperty, FeaturedProperty, 
                                    Pattern, KlineShapeProperty, BrokerProperty, OptionProperty, Indicator
                                )
                                for enum_class in [BasicProperty, SimpleProperty, FinancialProperty, CumulativeProperty, FeaturedProperty, Pattern, KlineShapeProperty, BrokerProperty, OptionProperty, Indicator]:
                            ...
                            if isinstance(items, list) and len(items) > 0:
                                print(f"  -> 📄 [Futu] {market} 市场翻页拉取中 (begin={begin})... 拿到 {len(items)} 条数据")
                                for item in items:
                                    row_dict = {}
                                    for res in item.get('results', []):
                                        prop_val = res.get('property', {}).get('name', '')
                                        
                                        if isinstance(prop_val, int) or (isinstance(prop_val, str) and prop_val.isdigit()):
                                            prop_name = v2_rev_map.get(int(prop_val), str(prop_val))
                                        elif hasattr(prop_val, 'name'):
                                            prop_name = str(getattr(prop_val, 'name')).lower()
                                        else:
                            ...
                            if is_last_page or not items:
                                print(f"  -> ✅ [Futu] {market} 市场数据拉取完毕！(已到底或无更多数据)")
                                break
            ...
            print(f"⚠️ [Futu] 条件选股接口响应超时 (12s)，底层网关忙线。")
            return {"status": "error", "message": "Futu 条件选股接口响应超时，底层网关忙线。"}

        if not all_dfs:
            return {"status": "success", "data": [], "message": "未能匹配到任何符合条件的股票。"}
        ...
        if filters:
            for f in filters:
                if f.get("type") == "indicator_pattern":
                    ...
                    matched_patterns_zh.append(PATTERN_ZH_MAP.get(p_field, f.get("field")))
            ...
            for col in data.columns:
                if col not in ["code", "name"]:
                    ...
                    if pd.notna(val) and val is not None:
                        ...
                        # 💡 强制数值化，防范 numpy scalar 对象或大数科学计数法导致的前端类型逃逸
                        try:
                            item_dict[final_key] = float(val) if not isinstance(val, bool) else val
        ...
        if exclude_plate_codes and results:
            print(f"🛡️ [ScreenerHandler] 触发板块剔除，准备拉取需排除的板块成分股: {exclude_plate_codes}")
            exclude_symbols = set()
            for p_code in exclude_plate_codes:
                try:
                    ret, p_data = await asyncio.to_thread(self.conn_mgr.quote_ctx.get_plate_stock, p_code)
                    if ret == RET_OK and isinstance(p_data, pd.DataFrame) and not p_data.empty:
            ...
            if exclude_symbols:
                ...
                print(f"🛡️ [ScreenerHandler] 成功剔除 {original_len - len(results)} 只属于 {exclude_plate_codes} 板块的股票")
        ...
        for row in results:
            for k in list(row.keys()):
                ...
                if isinstance(v, (int, float)):
                    is_pct = k in pct_keys or k.endswith("_growth") or k.endswith("_growth_rate") or "percentile" in k
                    # 剔除掉绝对倍数指标 (如流动比率等)
                    if is_pct and k not in {"current_ratio", "quick_ratio", "property_ratio"}:
                        # 新增 _fmt 字段，前端表格可直接绑定 field: "roe_fmt"
                        row[f"{k}_fmt"] = f"{v * 100:.2f}%"
                    elif k in {"current_ratio", "quick_ratio", "property_ratio", "volume_ratio"}:
                        row[f"{k}_fmt"] = f"{v:.2f}"
    ...
    async def get_stock_basicinfo(self, market: str, sec_type: str) -> Dict[str, Any]:
        ...
        sec_map = {"STOCK": SecurityType.STOCK, "ETF": SecurityType.ETF, "INDEX": SecurityType.IDX}
        
        mkt = mkt_map.get(market.upper(), Market.HK)
        sec = sec_map.get(sec_type.upper(), SecurityType.STOCK)
        
        ret, data = await asyncio.to_thread(
            self.conn_mgr.quote_ctx.get_stock_basicinfo, mkt, sec
        )
