...
class OptionFundHandler:
    ...
    async def get_option_chain(self, ticker: str, expiration_date: str = "", 
                               format_ticker_func=None, is_unsupported_func=None) -> Dict[str, Any]:
        """获取期权链数据"""
        ...
        # 开发环境 Mock
        if self.conn_mgr.status != "CONNECTED" and __import__('os').getenv("QUANT_ENV") == "development":
        ...
        if not expiration_date:
            ...
            if ret != RET_OK or not isinstance(raw_date_data, pd.DataFrame) or raw_date_data.empty:
                return {"status": "error", "message": f"无法获取到期日列表: {raw_date_data}"}
        ...
        ret, chain_data = await asyncio.to_thread(
            ...
            market_ticker, start=expiration_date, end=expiration_date
        )
        if ret != RET_OK or not isinstance(chain_data, pd.DataFrame) or chain_data.empty:
    ...
    @with_global_retry
    async def get_fund_flow(self, ticker: str, format_ticker_func=None, 
                           is_unsupported_func=None) -> Dict[str, Any]:
        """获取资金流向数据（带熔断机制）"""
        if is_unsupported_func and is_unsupported_func(ticker):
            return {"status": "error", "message": "富途原生不支持该大类资产"}
        ...
        if ret != RET_OK or not isinstance(data, pd.DataFrame) or data.empty:
            if "频率太高" in str(data) or "frequency" in str(data).lower():
                print(f"🚨 [Futu] 资金流向触发限流熔断！接口将强制全局休眠 60 秒以释放压力 ({data})")
        ...
        row = data.iloc[0]
        main_in = safe_float(row.get('capital_in_super', 0)) + safe_float(row.get('capital_in_big', 0))
        main_out = safe_float(row.get('capital_out_super', 0)) + safe_float(row.get('capital_out_big', 0))
        ...
        if market_ticker.startswith("HK."):
            sub_ret, sub_err = await asyncio.to_thread(
                ...
                [market_ticker], [SubType.BROKER, SubType.ORDER_BOOK], subscribe_push=False
            )
            if sub_ret == RET_OK:
                for _ in range(3):
                    ...
                    bid_df, ask_df = (res[1], res[2]) if res and res[0] == RET_OK and len(res) > 2 else (pd.DataFrame(), pd.DataFrame())
                ...
                if (isinstance(bid_df, pd.DataFrame) and not bid_df.empty) or \
                   (isinstance(ask_df, pd.DataFrame) and not ask_df.empty):
                    parse_brokers = lambda df: df[df.columns[1]].dropna().unique().tolist()[:10] \
                        if isinstance(df, pd.DataFrame) and not df.empty else []
                    fmt_q = lambda q: ", ".join(map(str, q)) if q else "暂无"
                ...
                if ret_ob == RET_OK and isinstance(ob_data, dict):
                    ...
                    bid1 = {"price": safe_float(bids[0][0]), "volume": int(bids[0][1])} if bids else None
                    ask1 = {"price": safe_float(asks[0][0]), "volume": int(asks[0][1])} if asks else None
                    order_book_data = {"bid1": bid1, "ask1": ask1}

        def _fmt_money(val: float) -> str:
            if abs(val) >= 1_0000_0000: return f"{val / 1_0000_0000:.2f}亿"
            if abs(val) >= 1_0000: return f"{val / 1_0000:.2f}万"
            return f"{val:.2f}"
    ...
    @with_global_retry
    async def get_fundamental(self, ticker: str, format_ticker_func=None, 
                             is_unsupported_func=None) -> Dict[str, Any]:
        """获取基本面数据"""
        if is_unsupported_func and is_unsupported_func(ticker):
            return {"status": "error", "message": "富途原生不支持该大类资产"}
        ...
        result = {
            ...
            "data": {
                ...
                "trailing_PE": safe_float(row.get('pe_ratio', 0.0)) if safe_float(row.get('pe_ratio', 0.0)) > 0 else None,
                "price_to_book": safe_float(row.get('pb_rate', 0.0)) if safe_float(row.get('pb_rate', 0.0)) > 0 else None,
                "dividend_yield": f"{safe_float(row.get('dividend_yield', 0.0))}%" if safe_float(row.get('dividend_yield', 0.0)) > 0 else None,
                "market_cap": safe_float(row.get('market_val', 0.0)) if safe_float(row.get('market_val', 0.0)) > 0 else None,
            }
        }
