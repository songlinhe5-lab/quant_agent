...
class OptionFundHandler:
    ...
    async def get_option_chain(self, ticker: str, expiration_date: str = "",
                               format_ticker_func=None, is_unsupported_func=None) -> Dict[str, Any]:  # noqa: E501
        """获取期权链数据"""
        ...
        if not expiration_date:
            ...
            if ret != RET_OK or not isinstance(raw_date_data, pd.DataFrame) or raw_date_data.empty:  # noqa: E501
                return {"status": "error", "message": f"无法获取到期日列表: {raw_date_data}"}  # noqa: E501
    ...
    async def get_fund_flow(self, ticker: str, format_ticker_func=None,
        ...
        if market_ticker.startswith("HK."):
            sub_ret, sub_err = await asyncio.to_thread(
                ...
                [market_ticker], [SubType.BROKER, SubType.ORDER_BOOK], subscribe_push=False  # noqa: E501
            )
            if sub_ret == RET_OK:
                for _ in range(3):
                    ...
                    bid_df, ask_df = (res[1], res[2]) if res and res[0] == RET_OK and len(res) > 2 else (pd.DataFrame(), pd.DataFrame())  # noqa: E501
                ...
                if (isinstance(bid_df, pd.DataFrame) and not bid_df.empty) or \
                   (isinstance(ask_df, pd.DataFrame) and not ask_df.empty):
                    def parse_brokers(df):
                        return df[df.columns[1]].dropna().unique().tolist()[:10] \
                                            if isinstance(df, pd.DataFrame) and not df.empty else []  # noqa: E501
                    def fmt_q(q):
        ...
        def _fmt_money(val: float) -> str:
            if abs(val) >= 1_0000_0000: return f"{val / 1_0000_0000:.2f}亿"  # noqa: E701
            if abs(val) >= 1_0000: return f"{val / 1_0000:.2f}万"  # noqa: E701
            return f"{val:.2f}"
    ...
    async def get_fundamental(self, ticker: str, format_ticker_func=None,
        ...
        result = {
            ...
            "data": {
                ...
                "trailing_PE": safe_float(row.get('pe_ratio', 0.0)) if safe_float(row.get('pe_ratio', 0.0)) > 0 else None,  # noqa: E501
                "price_to_book": safe_float(row.get('pb_rate', 0.0)) if safe_float(row.get('pb_rate', 0.0)) > 0 else None,  # noqa: E501
                "dividend_yield": f"{safe_float(row.get('dividend_yield', 0.0))}%" if safe_float(row.get('dividend_yield', 0.0)) > 0 else None,  # noqa: E501
                "market_cap": safe_float(row.get('market_val', 0.0)) if safe_float(row.get('market_val', 0.0)) > 0 else None,  # noqa: E501
            }
        }
