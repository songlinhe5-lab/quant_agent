...
DATA_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "data", "kline_warehouse"))
...
class KlineWarehouse:
    ...
    async def get_history(self, ticker: str, ktype: str = "K_DAY", num: int = 252) -> Optional[pd.DataFrame]:
    ...
    async def update_ticker(self, ticker: str, ktype: str = "K_DAY", force_full: bool = False):
        ...
        async with self._locks[ticker]:
            ...
            if last_date is None:
                num_to_fetch = 10000  # 首次冷启动，拉取过去 40 年全量数据 (确保能覆盖至少 20 年)
            else:
            ...
            # 1. 优先使用富途拉取高质量前复权数据
            futu_res = await futu_service.get_history(ticker, ktype=ktype, num=num_to_fetch)
            if futu_res.get("status") == "success" and futu_res.get("data"):
                new_data = futu_res["data"]
                # 💡 强制长线保障：如果我们需要拉取超大跨度数据(>2000)，但富途分页受限返回的数据过少，废弃富途数据交由雅虎财经进行全量深度拉取
                if num_to_fetch > 2000 and len(new_data) < 2000:
                    print(f"⚠️ [Kline Warehouse] 富途仅返回 {len(new_data)} 条历史数据，不足以支撑超长线回测，降级至雅虎财经获取全量历史...")
                    new_data = None
            ...
            if not new_data:
                ...
                success, yf_df, _ = await yf_service.fetch_yf_data(yf_ticker, "history", ttl=0, period=period, interval=yf_interval)
                ...
                if success and yf_df is not None and not yf_df.empty:
                    ...
                    # 兼容不同雅虎版本返回的日期列名
                    date_col = 'Date' if 'Date' in yf_df.columns else 'Datetime' if 'Datetime' in yf_df.columns else None
                    if date_col:
                        yf_df = yf_df.rename(columns={date_col: "time", "Open": "open", "High": "high", "Low": "low", "Close": "close", "Volume": "volume"})
                        yf_df['time'] = yf_df['time'].astype(str).str.split('+').str[0].str.replace('T', ' ')
                        new_data = yf_df[['time', 'open', 'high', 'low', 'close', 'volume']].to_dict(orient="records")

            if not new_data:
                print(f"❌ [Kline Warehouse] {ticker} 增量更新失败：所有数据源均无法获取。")
                return False
            ...
            def _merge_and_save():
                ...
                # 💡 核心策略：按时间去重，且 keep='last'。这能完美让今天拉取的最新复权K线，覆盖掉前几天存入的旧复权数据，保证精度！
            ...
            try:
                ...
                print(f"✅ [Kline Warehouse] {ticker} ({ktype}) 增量同步入库成功 | 抓取 {fetched_count} 条 | 现存 {total_count} 条。")
                return True
            except Exception as e:
            ...
            try:
                ...
                if now.hour == 3 and now.minute == 0:
                    ...
                    if await redis_client.set(lock_key, "1", nx=True, ex=7200):
                        ...
                        tickers_raw = await redis_client.hkeys("quant:settings:monitored_refcounts")
                        tickers = [t.decode('utf-8') if isinstance(t, bytes) else str(t) for t in tickers_raw]
                        default_tickers = ["US.SPY", "US.QQQ", "HK.800000", "SH.000001", "US.AAPL", "US.NVDA"]
