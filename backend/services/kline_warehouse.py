        ...
        async with self._locks[ticker]:
            ...
            if futu_res.get("status") == "success" and futu_res.get("data"):
                ...
                if num_to_fetch > 2000 and len(new_data) < 2000:
                    print(f"⚠️ [Kline Warehouse] 富途仅返回 {len(new_data)} 条历史数据，不足以支撑超长线回测，降级至雅虎财经获取全量历史...")  # noqa: E501
                    new_data = None
            ...
            if not new_data:
                ...
                success, yf_df, _ = await yf_service.fetch_yf_data(yf_ticker, "history", ttl=0, period=period, interval=yf_interval)  # noqa: E501
                ...
                if success and yf_df is not None and not yf_df.empty:
                    ...
                    # 兼容不同雅虎版本返回的日期列名
                    date_col = 'Date' if 'Date' in yf_df.columns else 'Datetime' if 'Datetime' in yf_df.columns else None  # noqa: E501
                    if date_col:
                        yf_df = yf_df.rename(columns={date_col: "time", "Open": "open", "High": "high", "Low": "low", "Close": "close", "Volume": "volume"})  # noqa: E501
            ...
            try:
                ...
                print(f"✅ [Kline Warehouse] {ticker} ({ktype}) 增量同步入库成功 | 抓取 {fetched_count} 条 | 现存 {total_count} 条。")  # noqa: E501
                return True
            except Exception as e:
            ...
            try:
                ...
                if now.hour == 3 and now.minute == 0:
                    ...
                    if await redis_client.set(lock_key, "1", nx=True, ex=7200):
                        ...
                        default_tickers = ["US.SPY", "US.QQQ", "HK.800000", "SH.000001", "US.AAPL", "US.NVDA"]  # noqa: E501
