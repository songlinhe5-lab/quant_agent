...
class SearchService:
    ...
    """
    async def web_search(self, query: str, max_results: int = 5, include_domains: Optional[List[str]] = None, exclude_domains: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if tavily_api_key and not results:
            try:
                ...
                async with httpx.AsyncClient(timeout=30.0, event_hooks={'request': [httpx_log_request], 'response': [httpx_log_response]}) as client:
                    resp = await client.post(url, json=payload)
                    resp.raise_for_status()
                    for item in resp.json().get("results", []):
                        results.append({"title": item.get("title"), "href": item.get("url"), "body": item.get("content")})
            except Exception as e:
                print(f"⚠️ [SearchService] Tavily 搜索失败，尝试降级: {repr(e)}")

        # 💡 优先级 2：博查 Bocha API (国内大模型 RAG 专属搜索，聚合百度/微信/搜狗，中文效果极佳)
        if bocha_api_key and not results:
            try:
                ...
                async with httpx.AsyncClient(timeout=10.0, event_hooks={'request': [httpx_log_request], 'response': [httpx_log_response]}) as client:
                    resp = await client.post(url, headers=headers, json=payload)
                    resp.raise_for_status()
                    for item in resp.json().get("data", {}).get("webPages", {}).get("value", []):
                        results.append({"title": item.get("name"), "href": item.get("url"), "body": item.get("snippet")})
            except Exception as e:
        ...
        if not results:
            ...
            def _do_duckduckgo_search():
                ...
                with DDGS(proxy=proxy, timeout=20) as ddgs:
                    res = list(ddgs.text(query, max_results=max_results, region='cn-zh'))
                    if res: return res
                    raise ValueError("DuckDuckGo 返回空数据")
            results = await asyncio.to_thread(_do_duckduckgo_search)
            
        return {"status": "success", "data": results} if results else {"status": "success", "data": [], "message": "未找到相关结果。请尝试简化搜索词。"}
