...
class TickerService:
    ...
    async def search_tickers(self, query: str) -> Dict[str, Any]:
        ...
        # 💡 修复脏数据漏洞：对任意用户输入进行 Hash，彻底阻断由于空格、冒号、换行符等引发的 Redis 命名空间污染
            ...
            async with self._search_locks[cache_key]:
                ...
                def _db_query():
                    ...
                    with SessionLocal() as db:
                        ...
                        if is_pg:
                            # 1. 生产环境 PostgreSQL: 开启 pg_trgm 相似度评分 (容忍拼写错误)
                            ...
                            rows = db.query(TickerItem).filter(
                                or_(
                                    ...
                                    sim_symbol > 0.15,  # 相似度 > 0.15 即被召回 (容忍 APPL -> AAPL)
                                    sim_name > 0.15
                                )
                            ).order_by(
                                # 排序权重: 绝对等于排最前 > 代码相似度 > 名称相似度 > 字符串越短越精准
                            ...
                            rows = db.query(TickerItem).filter(
                                or_(TickerItem.symbol.ilike(f"%{query_upper}%"), TickerItem.name.ilike(f"%{query_upper}%"))
                            ).order_by(
                                case((TickerItem.symbol == query_upper, 0), else_=1),
                                case((TickerItem.symbol.ilike(f"{query_upper}%"), 0), else_=1),
                                func.length(TickerItem.symbol)
                            ).limit(10).all()
                            
                        return [{"symbol": r.symbol, "name": r.name, "type": r.type} for r in rows]
    ...
    async def sync_tickers_daemon(self):
        ...
        while True:
            try:
                ...
                base_tickers = await asyncio.to_thread(self._write_base_tickers)
                
                if futu_service.status == "CONNECTED":
                    futu_tickers = await self._fetch_and_save_from_futu()
                    print("✅ [Ticker Sync] 股票词库同步完成，Redis 缓存 + DB 持久化就绪！")
                else:
    ...
    def _write_base_tickers(self) -> List[Dict[str, Any]]:
        base_tickers = [
            {"symbol": "HK.800000", "name": "恒生指数", "market": "HK", "type": "INDEX"},
            {"symbol": "HK.800700", "name": "恒生科技指数", "market": "HK", "type": "INDEX"},
            {"symbol": "US.SPX", "name": "标普500", "market": "US", "type": "INDEX"},
            {"symbol": "US.NDX", "name": "纳斯达克100", "market": "US", "type": "INDEX"},
            {"symbol": "US.AAPL", "name": "Apple Inc.", "market": "US", "type": "EQUITY"},
            {"symbol": "US.TSLA", "name": "Tesla Inc.", "market": "US", "type": "EQUITY"},
            {"symbol": "US.NVDA", "name": "NVIDIA Corporation", "market": "US", "type": "EQUITY"},
            {"symbol": "HK.00700", "name": "腾讯控股", "market": "HK", "type": "EQUITY"},
        ]
        with SessionLocal() as db:
                ...
                if res.get("status") == "success":
                    for row in res.get("data", []):
                        ...
                        tickers_to_insert.append({
                            ...
                            "market": market, "type": "EQUITY" if sec_type == "STOCK" else "ETF"
                        })
