...
async def _llm_analyze(
    ...
    """调用 LLM 生成结构化分析"""
    prompt = _build_analysis_prompt(market, date, indices, capital, news, sentiment)
    try:
...
async def generate_market_review(
    ...
    indices, capital, news, sentiment = await asyncio.gather(
        indices_task,
        capital_task,
        news_task,
        sentiment_task,
    ...
    # ── Phase 2: LLM 结构化分析 ──
    analysis = await _llm_analyze(market, date, indices, capital, news, sentiment)
