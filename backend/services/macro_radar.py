...
class MacroRadarEngine:
    ...
    def calculate_adaptive_score(
        ...
        history_closes: Optional[pd.Series],
    ...
    def calculate_spread_score(
        current_diff: float,
        history_series: Optional[pd.Series],
        multiplier: float = 1.5,
        inverse: bool = False,
    ) -> float:  # noqa: E501
        """
    ...
    async def generate_radar_data(self) -> list[dict]:
        ...
        liq_score = avg(
            get_score("USD/JPY", inverse=True),
            get_score("VIX", inverse=True),
            get_spread_score("HY_SPREAD", inverse=True),
        )  # noqa: E501

        vix_series = data_store.get("VIX")
        vol_score = (
            max(0, min(100, 100 - (float(vix_series.iloc[-1]) - 10) * 2.5))
