...
class AKShareService:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": "circuit_open"
            if is_open
            else ("warning" if self._error_count > 0 else "healthy"),  # noqa: E501
            ...
            if is_open
                ...
                f"已连续报错 {self._error_count} 次，接近熔断阈值"
    ...
    async def _acquire_lock_with_timeout(
        self, acquire_timeout: float = 5.0, exec_timeout: float = 15.0
    ):  # noqa: E501
        ...
        try:
            ...
            raise TimeoutError(
                f"AKShare 接口调用排队超时 ({acquire_timeout}s)，分布式锁获取失败。"
            )  # noqa: E501
        except (
            asyncio.TimeoutError
        ):  # This might be raised by the business logic inside the lock  # noqa: E501
            raise TimeoutError(
                f"AKShare 接口执行超时 ({exec_timeout}s)，底层数据源无响应。"
            )  # noqa: E501
    ...
    async def get_southbound_flow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                not isinstance(hist_df, BaseException)
            ...
            if net_inflow >= 800.0:
                raise ValueError(
                    "AKShare 返回了总额度而非净流入，且无法用历史数据拯救，判定为接口异常"
                )  # noqa: E501

            # 状态判定：3 为已收盘
            is_closed = (
                int(south_df["交易状态"].iloc[0]) == 3
                if "交易状态" in south_df.columns
                else False
            )  # noqa: E501
    ...
    async def get_hsgt_top_holders(self, symbol: str = "00700") -> Dict[str, Any]:
        ...
        try:
            ...
            prev_map = (
                {
                    str(row.get("机构名称", "")): float(row.get("持股数量", 0) or 0)
            ...
            prev_southbound_total = (
                float(prev_df["持股数量"].sum())
                if prev_df is not None and not prev_df.empty
                else southbound_total
            )  # noqa: E501
            ...
            for _, row in latest_df.head(20).iterrows():
                ...
                pct = float(
                    row.get("持股数量占A股百分比", row.get("占已发行股份百分比", 0))
                    or 0
                )  # noqa: E501
        ...
        if result.get("status") == "success":
            ...
            await redis_client.set(
                cache_key, json.dumps(result), ex=60
            )  # 错误状态仅做短时防穿透  # noqa: E501
        return result
    ...
    async def get_company_news(self, ticker: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._error_count >= self._max_errors:
                print(
                    f"🚨 [AKShare] 连续报错 {self._error_count} 次，触发个股新闻熔断休眠 60 秒！"
                )  # noqa: E501
    ...
    async def get_stock_quote(self, ticker: str) -> Dict[str, Any]:
        ...
        try:
            ...
            latest = df.iloc[-1]
            prev_close = (
                float(df.iloc[-2]["收盘"]) if len(df) > 1 else float(latest["开盘"])
            )  # noqa: E501
            ...
            result = {
                ...
                "data": {
                    ...
                    if vol > 1_000_000
                    else f"{vol / 1_000:.2f}K",  # noqa: E501
                },
            ...
            if self._error_count >= self._max_errors:
                print(
                    f"🚨 [AKShare] 连续报错 {self._error_count} 次，触发实时行情熔断休眠 60 秒！"
                )  # noqa: E501
                self._circuit_breaker_until = time.time() + 60.0
    ...
    async def get_stock_history(self, ticker: str, num: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._error_count >= self._max_errors:
                print(
                    f"🚨 [AKShare] 连续报错 {self._error_count} 次，触发 K 线接口熔断休眠 60 秒！"
                )  # noqa: E501
                self._circuit_breaker_until = time.time() + 60.0
        ...
        async def _fetch_date(date_str: str, date_compact: str):
            ...
            try:
                ...
                if hasattr(ak, "news_economic_baidu"):
                    df = await asyncio.to_thread(
                        ak.news_economic_baidu, date=date_compact
                    )  # noqa: E501
        ...
        try:
            ...
            for date_idx, res in enumerate(results):
                ...
                for item in res:
                    ...
                    country = str(
                        item.get("地区", item.get("country", item.get("国家", "")))
                    )  # noqa: E501
                    event_name = str(
                        item.get(
                            "事件",
                            item.get(
                                "event", item.get("指标名称", item.get("name", ""))
                            ),
                        )
                    ).strip()  # noqa: E501
                    if not event_name:
                    ...
                    # 星级/重要性
                    star = str(
                        item.get("重要性", item.get("importance", item.get("star", "")))
                    )  # noqa: E501
                    impact = (
                        "high"
                        if "高" in star or "3" in star
                        else ("medium" if "中" in star or "2" in star else "low")
                    )  # noqa: E501
                    ...
                    pub_time = str(
                        item.get(
                            "公布时间",
                            item.get(
                                "时间", item.get("time", item.get("pub_time", ""))
                            ),
                        )
                    )  # noqa: E501
                    ...
                    events.append(
                        {
                            ...
                            "estimate": str(
                                item.get(
                                    "预测值",
                                    item.get(
                                        "predicted_value", item.get("consensus", "")
                                    ),
                                )
                            ),  # noqa: E501
                            "actual": str(
