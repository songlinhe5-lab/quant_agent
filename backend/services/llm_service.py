...
class LLMService:
    ...
    def __init__(self):
        ...
        self.client = AsyncOpenAI(
            ...
            # 若大模型供应商发生静默挂起，后台所有高频的情感分析和选股协程将被卡死 10 分钟。
            timeout=30.0,
            max_retries=2,
            http_client=httpx.AsyncClient(event_hooks={'request': [httpx_log_request], 'response': [httpx_log_response]})
        )
    ...
    async def generate_pydantic(self, prompt: str, response_model: Type[T], system_prompt: str = "You are a helpful assistant.", **kwargs) -> T:
        """
        通用结构化输出提取工具函数。
