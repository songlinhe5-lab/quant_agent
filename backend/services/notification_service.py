...
class NotificationService:
    """后端原生通知服务，负责将系统级报警广播给前端或外部渠道 (支持 DingTalk/WeCom/Feishu/Telegram 等)"""
    
    async def send_alert(self, message: str):
        logger.info(f"🔔 [System Notification] {message}")
        try:
            # 1. 推送到现有的 WebSocket Redis PubSub 通道 (前端会在全局监听到)
            payload = json.dumps({"type": "notification", "message": message, "channel": "system_alerts"})
    ...
    async def _send_to_feishu(self, message: str):
        ...
        if not webhook_url.startswith(("http://", "https://")):
            logger.error(f"飞书推送失败: 无效的 Webhook URL (必须以 http:// 或 https:// 开头): {webhook_url}")
            return
