            ...
            try:
                ...
                if vix_cache:
                    ...
                    if records and len(records) > 0:
                        ...
                        if v_val is None:
                            v_val = next((v for k, v in records[-1].items() if str(k).startswith("('Close'")), None)
                        if v_val:
                ...
                if cpc_cache:
                    ...
                    if records and len(records) > 0:
                        ...
                        if c_val is None:
                            c_val = next((v for k, v in records[-1].items() if str(k).startswith("('Close'")), None)
                        if c_val:
                            cpc_val = round(float(c_val), 2)

                # 3. 拟合 Credit Spread (基于 VIX)
                credit_spread = round(2.0 + (vix_val / 10.0), 2) if vix_val is not None else None
                    
                # 4. 存入关系型数据库做持久化
                def save_to_db():
                    with SessionLocal() as db:
                        record = models.SentimentRecord(vix_value=vix_val, pc_ratio=cpc_val, credit_spread=credit_spread)
                        db.add(record)
                        db.commit()
                await asyncio.to_thread(save_to_db) # 数据库是同步 IO，必须用 to_thread 防止阻塞网关
                print(f"📈 [Sentiment Tracker] 数据打点成功: VIX={vix_val}, P/C={cpc_val}, Spread={credit_spread}")
