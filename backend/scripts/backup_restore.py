...
def restore_postgres(backup_path: str) -> None:
    ...
    print("[2/3] 重建数据库...")
    subprocess.run(
        ["dropdb", "-h", DB_HOST, "-p", str(DB_PORT), "-U", DB_USER, "--if-exists", DB_NAME],
        capture_output=True, text=True,
        env={"PGPASSWORD": DB_PASSWORD}
    )
    subprocess.run(
    ...
    if str(path).endswith(".gz"):
        with gzip.open(path, "rb") as f:
            result = subprocess.run(
                ["psql", "-h", DB_HOST, "-p", str(DB_PORT), "-U", DB_USER, "-d", DB_NAME, "-q"],
...
def restore_redis(backup_path: str) -> None:
    ...
    try:
        info = subprocess.run(
            ["redis-cli", "-h", REDIS_HOST, "-p", str(REDIS_PORT), "-a", REDIS_PASSWORD, "DBSIZE"],
        ...
        if pg_backups:
            ...
            try:
                with gzip.open(latest, "rb") as f:
                    # 读取前 1KB 验证 gzip 完整性
                    f.read(1024)
                results.append(("PostgreSQL", "✅", f"{size/1024/1024:.1f}MB", f"{age_hours:.1f}h前"))
    ...
    if redis_dir.exists():
        ...
        if redis_backups:
            ...
            try:
                with gzip.open(latest, "rb") as f:
                    f.read(1024)
                results.append(("Redis", "✅", f"{size/1024/1024:.1f}MB", f"{age_hours:.1f}h前"))
            except Exception as e:
