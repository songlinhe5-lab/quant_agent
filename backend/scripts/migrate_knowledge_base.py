...
class KnowledgeBaseMigrator:
    ...
    async def import_from_file(
        ...
        logger.info(f"[KB导入] 开始从 {input_path} 导入到 {table} 表 (冲突策略: {on_conflict})")
            ...
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    if len(batch_records) >= batch_size:
                        result = await self._batch_insert(
                            batch_records, model_class, table, on_conflict
                        )
...
async def main():
    ...
    export_parser.add_argument("--table", "-t", default="webpage", choices=["webpage", "screener_rule"])
    ...
    import_parser.add_argument("--table", "-t", default="webpage", choices=["webpage", "screener_rule"])
    import_parser.add_argument("--on-conflict", default="skip", choices=["skip", "update", "overwrite"])
    
    # cleanup 子命令
    cleanup_parser = subparsers.add_parser("cleanup", help="清理旧数据")
    cleanup_parser.add_argument("--days", "-d", type=int, default=90, help="保留天数")
    cleanup_parser.add_argument("--table", "-t", default="webpage", choices=["webpage", "screener_rule"])
