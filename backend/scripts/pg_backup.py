...
class PostgresBackup:
    ...
    async def _upload_to_r2(self, file_path: Path, timestamp: str) -> str:
        ...
        if not all([self.r2_endpoint, self.r2_access_key, self.r2_secret_key, self.r2_bucket]):
            raise ValueError("R2 配置不完整，请检查 R2_ENDPOINT/R2_ACCESS_KEY/R2_SECRET_KEY/R2_BUCKET")
...
async def main():
    ...
    parser.add_argument("--cleanup-days", type=int, default=7, help="清理多少天前的本地备份（0=不清理）")
