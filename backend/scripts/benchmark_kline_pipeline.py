...
class PipelineBenchmark:
    ...
    async def benchmark_redis(self) -> Dict[str, Any]:
        ...
        for i in range(self.iterations):
            ...
            # 读取最新数据
            result = await redis_client.xrevrange(test_key, count=1)
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for stage_name, stage_result in stages.items():
            if stage_result.get("status") == "skipped":
                report_lines.append(f"[{stage_name}] 跳过: {stage_result.get('reason')}")
                continue
        ...
        report_lines.append(f"总结: {overall} ({summary['stages_passed']}/{summary['stages_total']} 阶段通过)")
        report_lines.append(f"目标: P99 < 50ms")
        report_lines.append("=" * 60)
