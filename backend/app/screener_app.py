"""选股编排层 (Screener Orchestration)。

将 routers/screener.py 中的用例编排逻辑（DSL 转译、在线扫盘、缓存防频控、历史/字典/
订阅/横截面/组合回测/CEP 异动筛选等）收口到本模块，使 router 仅承担「请求校验 +
HTTP 映射 + 鉴权注入」。

所有下游依赖（screener_service / market_data / redis_client）均在此处显式导入，
便于单测对 `backend.app.screener_app.<dep>` 进行 patch。
`get_current_user` / `get_db` 等 HTTP 依赖与 `get_subscription` 依赖仍保留在 router 层。
"""
...
async def get_screener_suggestions(limit: int = 6, db: Session = None):
    """获取随机选股灵感提示词"""
...
async def run_screener(req: ScreenerRequest):
    ...
    async with _screener_locks[cache_key]:
        ...
        try:
            ...
            raise AppError(status_code=400, detail=str(ve))
        except Exception as e:
            raise AppError(status_code=500, detail=f"选股器执行失败: {str(e)}")


async def get_screener_history(current_user):
    """获取用户的云端选股历史记录"""
    try:
...
async def save_screener_history(req, current_user):
    """保存用户的选股历史记录到云端"""
    try:
        key = f"quant:screener:history:{current_user.id}"
...
async def reload_indicators():
    ...
    try:
        ...
        raise AppError(status_code=500, detail=f"热更新失败: {str(e)}")


async def get_dictionary(current_user):
    """获取当前用户的私有选股规则列表"""
    data = await screener_service.get_custom_rules(user_id=current_user.id)
    return {"status": "success", "data": data}


async def add_dictionary_item(item, current_user):
    """添加私有 RAG 选股规则"""
    res = await screener_service.add_custom_rule(desc_text=item.desc, rule_text=item.rule, user_id=current_user.id)
    if res.get("status") == "error":
        raise AppError(status_code=500, detail=res.get("message"))
    return res


async def delete_dictionary_item(item, db, current_user):
    ...
    if not rule:
        raise AppError(status_code=404, detail="规则不存在或您无权删除")

    success = await screener_service.delete_custom_rule(rule.id, current_user.id)
    if not success:
        raise AppError(status_code=500, detail="底层删除失败")
    return {"status": "success", "message": "规则已成功删除"}


async def add_dictionary_batch(req, current_user):
    ...
    if success_count == 0 and errors:
        raise AppError(status_code=500, detail="批量导入失败: " + ", ".join(errors[:3]))  # noqa: E501

    return {"status": "success", "message": f"成功批量导入 {success_count} 条规则"}


async def subscribe_screener(req, db, current_user):
    ...
    if not re.match(r"^\d{2}:\d{2}$", trigger_time_str):
        raise AppError(status_code=400, detail="触发时间格式不正确，必须为 HH:MM 格式。")  # noqa: E501
    ...
    try:
        ...
        db.rollback()
        raise AppError(status_code=500, detail=f"订阅失败: {str(e)}")


async def get_subscriptions(db, current_user):
    """获取当前用户所有的每日选股订阅任务"""
    subs = (
...
async def update_subscription_time(req, db, sub):
    """更新订阅任务的触发时间"""
...
async def delete_subscription(db, sub):
    """彻底删除某个订阅任务"""
    db.delete(sub)
    db.commit()
...
async def toggle_subscription(db, sub):
    """切换订阅任务的 启动/暂停 状态"""
...
async def summarize_screener_results(payload):
...
async def cross_sectional_screen(req):
    ...
    try:
        ...
        raise AppError(status_code=500, detail=f"横截面筛选失败: {str(e)}")
...
async def portfolio_backtest(req):
    ...
    try:
        ...
        raise AppError(status_code=500, detail=f"组合回测失败: {str(e)}")
