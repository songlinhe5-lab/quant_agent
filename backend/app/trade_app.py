"""交易编排层 (Trade Orchestration)。

将 routers/trade.py 中的用例编排逻辑（杠杆风控偏好读取、账户信息 Redis 缓存防频控、
基于 ATR 的动态波动率风控、资金敞口校验、下单分发、OMS 持久化与留痕）收口到本模块，
使 router 仅承担「请求校验 + HTTP 映射 + 鉴权注入」。

所有下游依赖（broker / market_data / oms_service / redis_client / models /
_to_yf_ticker）均在此处显式导入，便于单测对 `backend.app.trade_app.<dep>` 进行 patch。
"""
...
def get_trades(limit: int = 100, db: Session = None):
