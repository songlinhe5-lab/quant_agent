...
import numpy as np
import pandas as pd
from typing import Dict, Any, Optional, Literal, List, Tuple, Union, Set, Callable, Mapping, Sequence
import itertools
...
def _safe_import(name, globals=None, locals=None, fromlist=(), level=0):
    dangerous_modules = {'os', 'sys', 'subprocess', 'shutil', 'socket', 'urllib', 'requests', 'pathlib', 'pty', 'multiprocessing', 'threading', 'concurrent', '_thread'}
...
def _safe_open(file, mode='r', buffering=-1, encoding=None, errors=None, newline=None, closefd=True, opener=None):
    ...
    if not abs_path.startswith(_SECURE_PREFIX):
        raise PermissionError(f"🚨 安全风控拦截：沙箱环境严禁进行目录穿越 (Directory Traversal) 访问越权文件！(尝试访问: {file})")
        
    return open(abs_path, mode, buffering, encoding, errors, newline, closefd, opener)

# 剥离读写文件 (open)、直接执行代码 (eval/exec) 及反射内存的高危内建权限
SAFE_BUILTINS = {k: v for k, v in builtins.__dict__.items() if k not in ('eval', 'exec', 'open', 'compile', 'globals', 'locals')}
...
class SandboxSecurityVisitor(ast.NodeVisitor):
    def __init__(self):
        ...
        self.forbidden_names = {
            ...
            'compile', '__import__', 'sys', 'os'
        }
        # 禁用魔术属性访问，切断对象树遍历的逃逸路径
        self.forbidden_attrs = {
            '__class__', '__subclasses__', '__base__', '__bases__',
            '__mro__', '__dict__', '__builtins__', '__globals__',
            '__getattribute__', '__code__', '__closure__', '__func__', '__self__',
            # 💡 性能风控：彻底封杀 Pandas 极度低效的行级遍历迭代器
            'iterrows', 'itertuples', 'iteritems'
        }
        # 💡 数据科学与量化白名单：允许大模型在代码中导入这些绝对安全的计算库
        self.allowed_modules = {
            'numpy', 'pandas', 'math', 'datetime', 'collections', 'itertools',
            'scipy', 'statsmodels', 'sklearn', 'lightgbm', 'xgboost', 'typing',
            'typing_extensions'
        }
    ...
    def visit_FunctionDef(self, node):
        # 💡 安全风控：彻底封杀所有函数装饰器，防止通过闭包和高阶函数篡改执行流或绕过递归检测
        if node.decorator_list:
            dec_name = getattr(node.decorator_list[0], 'id', getattr(getattr(node.decorator_list[0], 'func', None), 'id', '...'))
            raise ValueError(f"🚨 安全风控拦截：沙箱内严禁使用函数装饰器 (如 @{dec_name})！请保持策略代码极简。")
        ...
        if isinstance(iter_node, ast.Call) and isinstance(iter_node.func, ast.Name) and iter_node.func.id == 'range':
            for arg in iter_node.args:
                ...
                if isinstance(arg, ast.Call) and isinstance(arg.func, ast.Name) and arg.func.id == 'len':
                    raise ValueError(f"🚨 性能风控拦截：严禁在 {context_name} 中使用 `range(len(...))` 遍历数据！请务必使用 Pandas 的矢量化运算。")
                # 2. 封堵硬编码超大常数 (如 range(10000000))
                elif isinstance(arg, ast.Constant) and isinstance(arg.value, int) and arg.value > 10000:
                    raise ValueError(f"🚨 内存风控拦截：严禁在 {context_name} 中使用超大范围 `range({arg.value})`，存在瞬间内存溢出 (OOM) 风险！")
                # 3. 封堵幂运算或乘法 (如 range(10**9) 或 range(1000 * 1000))
                elif isinstance(arg, ast.BinOp):
                    raise ValueError(f"🚨 内存风控拦截：严禁在 {context_name} 的 range() 内进行数学表达式运算，存在 OOM 风险！")
    ...
    def visit_ListComp(self, node):
        for comp in node.generators: self._check_range_iter(comp.iter, "列表推导式 (List Comprehension)")
        self.generic_visit(node)

    def visit_DictComp(self, node):
        for comp in node.generators: self._check_range_iter(comp.iter, "字典推导式 (Dict Comprehension)")
        self.generic_visit(node)
        
    def visit_SetComp(self, node):
        for comp in node.generators: self._check_range_iter(comp.iter, "集合推导式 (Set Comprehension)")
        self.generic_visit(node)
        
    def visit_While(self, node):
        # 💡 性能与安全双重风控：彻底封杀 while 循环，防止大模型幻觉导致无限死循环或极度低效的参数逼近
        raise ValueError("🚨 性能风控拦截：沙箱内严禁使用 `while` 循环！请务必使用 Numpy/Pandas 的矢量化矩阵运算来替代，以防死循环彻底耗尽系统算力。")
    ...
    def visit_Call(self, node):
        ...
        if self.current_funcs:
            ...
            if isinstance(node.func, ast.Name) and node.func.id == current_func:
                raise ValueError(f"🚨 性能风控拦截：沙箱内严禁使用递归调用 `{current_func}()`！请务必使用 Numpy/Pandas 的矢量化矩阵运算来替代，以防爆栈 (Stack Overflow)。")
            ...
            if isinstance(node.func, ast.Attribute) and isinstance(node.func.value, ast.Name) and node.func.value.id == 'self':
                if node.func.attr == current_func:
                    raise ValueError(f"🚨 性能风控拦截：沙箱内严禁使用类方法递归 `self.{current_func}()`！请务必使用 Numpy/Pandas 的矢量化矩阵运算来替代，以防爆栈。")
...
def _verify_safe_code(source_code: str):
    ...
    try:
        ...
        raise ValueError(f"策略源码存在缩进错误 (IndentationError): {e.msg} (第 {e.lineno} 行)。请严格检查代码的空格与缩进对齐。")
            ...
            if self.call_count % 5000 == 0:
                try:
                    ...
                    if current_memory - self.start_memory > self.max_memory_mb:
                        raise SandboxMemoryException(f"沙箱内存溢出熔断！在执行期间增量消耗超过了 {self.max_memory_mb} MB，已强制打断防 OOM。")
                except Exception:
                    pass
...
class EventDrivenBacktestEngine:
    ...
    """
    def __init__(self, strategy_instance, df: pd.DataFrame, initial_capital: float = 100000.0, commission_pct: float = 0.0005, slippage_pct: float = 0.001, debug_mode: bool = False):
    ...
    def _execute_buy(self, base_price: float, date_str: str, stop_loss: Optional[float] = None):
        ...
        if shares > 0:
            ...
            self.trades.append({
                "date": date_str, "action": "BUY", "price": round(exec_price, 2), "shares": shares, "profit": 0.0
            })
    ...
    def _execute_sell(self, base_price: float, date_str: str):
        ...
        self.trades.append({
            "date": date_str, "action": "SELL", "price": round(exec_price, 2), "shares": self.position, "profit": round(profit, 2)
        })
    ...
    def run(self) -> dict:
        ...
        for i in range(10, len(df)):
            ...
            if hasattr(self.strategy, '_position_data') and self.position > 0:
                ...
                # 💡 优化：根据最低价 current_low 判定是否触发止损，而非基于收盘价，大幅提高防回撤精度
            ...
            if signal and isinstance(signal, dict):
                ...
                if action == 'cancel':
                    ...
                    if limit_price:
                        self.pending_orders.append({"action": "buy", "limit_price": float(limit_price), "stop_loss": signal.get("stop_loss")})
                    else:
            ...
            if self.debug_mode:
                ...
                log_line = f"[{date_str}] P:{current_price:.2f} | Pos:{self.position} | Cash:{self.cash:.2f} | Eq:{current_equity:.2f} | Sig:{sig_str} | Pending:{len(self.pending_orders)}"
            ...
            self.equity_curve.append({
                ...
                "benchmark": round(self.initial_capital * (current_price / benchmark_start_price), 2),
        ...
        if len(self.equity_curve) > 0:
            ...
            sharpe_ratio = (daily_returns.mean() / daily_returns.std()) * np.sqrt(252) if len(daily_returns) > 0 and daily_returns.std() != 0 else 0.0
...
def run_dynamic_sandbox_backtest(source_code: str, class_name: str, params: dict, df: pd.DataFrame, initial_capital: float = 100000.0, debug_mode: bool = False) -> dict:
    """
    ...
    # 💡 [极速架构]: 侦测大模型生成的代码是否符合 Numba 矢量化规范，如果符合，无缝挂载到底层 C 级引擎极速运行
    # 💡 如果开启了 debug_mode，主动降级回高保真事件驱动引擎以捕获逐 K 线内部状态
    if not debug_mode and hasattr(strategy_instance, '_calculate_indicators') and hasattr(strategy_instance, '_generate_signals'):
        ...
        # 智能推导大模型可能使用的止损乘数变量名
        atr_multi = params.get('atr_multiplier', params.get('stop_loss_atr_multiple', params.get('sl_multiplier', 2.0)))
        sl_trail_pct = (res_df['atr'] * float(atr_multi)) / res_df['Close']
        
        pf = vbt.Portfolio.from_signals(
            close=res_df['Close'], open=res_df['Open'], high=res_df['High'], low=res_df['Low'],
            entries=entries, exits=exits, short_entries=short_entries, short_exits=short_exits,
            init_cash=float(initial_capital), fees=0.0005, slippage=0.001, sl_trail=sl_trail_pct,
            upon_long_conflict='reverse', upon_short_conflict='reverse', freq='1D'
        )
    ...
    # =========================================================================
    if debug_mode:
        print(f"🐛 [Backtest Engine] 调试模式已开启，强制降级至高保真事件驱动引擎以捕获逐 K 线状态！")
...
class DivergenceResonanceStrategy:
    ...
    def _generate_signals(self):
        ...
        # 💡 成交量过滤：只有在“缩量（动能衰竭）”或“放量（资金介入/出逃）”时，背离信号才算有效，过滤掉大量无量横盘的假信号
    ...
    def run(self) -> Dict[str, Any]:
        ...
        pf = vbt.Portfolio.from_signals(
            ...
            entries=entries, exits=exits, short_entries=short_entries, short_exits=short_exits,
            init_cash=float(self.initial_capital), fees=self.commission_pct, slippage=self.slippage_pct, sl_trail=sl_trail_pct,
            upon_long_conflict='reverse', upon_short_conflict='reverse', freq='1D'
        )
        ...
        equity_curve = df_chart[['date', 'equity', 'benchmark', 'price']].iloc[::max(1, len(df_chart)//200)].to_dict(orient='records')
...
def run_grid_search_backtest(source_code: str, class_name: str, param_grid: dict, df: pd.DataFrame, initial_capital: float = 100000.0, target_metric: str = "sharpe_ratio") -> list:
    """
    ...
    # 💡 核心：构建所有参数组合的笛卡尔积 (例如: fast_ma=[10,20], slow_ma=[30,40] -> 4 种组合)
    ...
    for combo in combinations:
        ...
        try:
            with SandboxTimeoutTracer(timeout_seconds=3.0):
                ...
                # 💡 性能防线：对于网格搜索，强烈建议只运行 Numba 矢量化的策略，拒绝运行老旧的 for 循环引擎
                if not (hasattr(strategy_instance, '_calculate_indicators') and hasattr(strategy_instance, '_generate_signals')):
                    raise ValueError("Grid Search 仅支持 Numba 矢量化策略。请让大模型实现 _calculate_indicators 等函数。")
            ...
            res_df = strategy_instance.df
            if 'signal' not in res_df.columns: res_df['signal'] = 0
            if 'atr' not in res_df.columns: res_df['atr'] = res_df['Close'].diff().abs().rolling(14).mean().fillna(res_df['Close']*0.01)
            ...
            results.append({ "params": params, "raw_metrics": { "total_return": total_return_val, "sharpe_ratio": sharpe_ratio, "max_drawdown": max_drawdown, "win_rate": win_rate, "total_trades": total_trades } })
    ...
    if not results: 
        if last_error is not None:
            ...
            raise ValueError(f"全部参数组合均执行失败，未产生有效交易。\n诊断信息: {type(last_error).__name__}: {last_error}")
        return []
    ...
    # 截取 Top 10 并格式化输出给前端
    return [{"params": r["params"], "metrics": {"total_return": f"{r['raw_metrics']['total_return'] * 100:.2f}%", "sharpe_ratio": f"{r['raw_metrics']['sharpe_ratio']:.2f}", "max_drawdown": f"{r['raw_metrics']['max_drawdown'] * 100:.2f}%", "win_rate": f"{r['raw_metrics']['win_rate'] * 100:.2f}%", "total_trades": r['raw_metrics']['total_trades']}} for r in results[:10]]


def run_monte_carlo_stress_test(source_code: str, class_name: str, params: dict, df: pd.DataFrame, initial_capital: float = 100000.0, iterations: int = 100, noise_level: float = 1.0, noise_distribution: str = "laplace", stock_features: Optional[dict] = None) -> dict:
    """
    ...
    global_scope = {
        "__builtins__": SAFE_BUILTINS,
        "np": np, "pd": pd, "Dict": Dict, "Optional": Optional, "List": List, "Any": Any, "Literal": Literal,
        "Tuple": Tuple, "Union": Union, "Set": Set, "Callable": Callable, "Mapping": Mapping, "Sequence": Sequence,
        "collections": collections, "datetime": datetime, "math": math, "random": random, "itertools": itertools,
        "DataFrame": pd.DataFrame, "Series": pd.Series, "BaseStrategy": BaseStrategySandbox,
    }
    
    # 💡 终极类型防线：强制开启注解延迟解析，彻底免疫大模型幻觉生成的 Any[str] 等非法类型导致的沙箱崩溃
    ...
    if stock_features:
        ...
        if market_cap is not None and market_cap < 2_000_000_000.0:
            ...
            print(f"📊 [Monte Carlo] 属性感知: 检测到小盘股 (市值 < 20亿)，环境噪音自动翻倍。")
            
        beta = stock_features.get("beta")
        if beta is not None and beta > 1.5:
            dynamic_noise_multiplier *= 1.5
            print(f"📊 [Monte Carlo] 属性感知: 检测到高波动标的 (Beta > 1.5)，环境噪音放大 50%。")
    ...
    for i in range(iterations):
        ...
        if noise_distribution == "laplace":
            # 拉普拉斯分布 (双指数分布)：尖峰胖尾，方差为 2 * scale^2。为了对齐目标波动率，scale = std / sqrt(2)
            scale = target_std / np.sqrt(2)
            noise = np.random.laplace(0, scale, len(noisy_df))
        elif noise_distribution == "t":
            # 学生 t-分布 (df=3)：具有极强的肥尾特性，方差为 df / (df - 2) = 3。对齐波动率 scale = std / sqrt(3)
        ...
        # 设定成交量的波动率倍数大于价格波动率，为了保持期望值为 1.0（长期平均成交量不变），mean 设为 -0.5 * sigma^2
        ...
        res_df = res_df.dropna().copy()
        if len(res_df) < 10: continue
            
        atr_multi = float(params.get('atr_multiplier', params.get('stop_loss_atr_multiple', params.get('sl_multiplier', 2.0))))
        ...
        pf = vbt.Portfolio.from_signals(
            close=res_df['Close'], open=res_df['Open'], high=res_df['High'], low=res_df['Low'],
            entries=entries, exits=exits, short_entries=short_entries, short_exits=exits,
            init_cash=float(initial_capital), fees=0.0005, slippage=0.001, sl_trail=sl_trail_pct,
            upon_long_conflict='reverse', upon_short_conflict='reverse', freq='1D'
        )
    ...
    if not results: raise ValueError("蒙特卡洛测试失败，所有模拟均未产生有效数据。")
    ...
    return {
        ...
        "win_rate_of_simulations": f"{(np.sum(returns_arr > 0) / len(returns_arr))*100:.2f}%",
        ...
        "raw_returns": returns_arr.tolist(), # Raw data available for plotting the bell curve on the frontend!
        "raw_max_drawdowns": mdds_arr.tolist() # Raw data available for plotting the box plot on the frontend!
    }

def run_batch_sandbox_backtest(source_code: str, class_name: str, params: dict, dfs: Dict[str, pd.DataFrame], initial_capital: float = 100000.0) -> dict:
    """
    ...
    # 将标的矩阵对其，由于交易日历可能略有差异，合并为一个包含全部日期的 Multi-Column DataFrame
    ...
    pf = vbt.Portfolio.from_signals(
        ...
        entries=entries_df, exits=exits_df, short_entries=short_entries_df, short_exits=short_exits_df,
        init_cash=per_asset_capital, fees=0.0005, slippage=0.001, sl_trail=sl_trail_df,
        upon_long_conflict='reverse', upon_short_conflict='reverse', freq='1D',
        group_by=True
    )
