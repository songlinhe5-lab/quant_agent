...
class MarketSession:
    ...
    SESSIONS = {
        # 美股 (NYSE/NASDAQ)
        "US": {
            "pre_market": ("04:00", "09:30"),      # 盘前 04:00-09:30 ET = 08:00-13:30 UTC
            "regular": ("09:30", "16:00"),          # 常规 09:30-16:00 ET = 13:30-20:00 UTC
            "after_hours": ("16:00", "20:00"),      # 盘后 16:00-20:00 ET = 20:00-00:00 UTC
    ...
    def get_market(cls, symbol: str) -> str:
        ...
        if symbol.startswith("US.") or symbol.startswith("US_"):
            ...
            return "HK"
        elif symbol.startswith("SH.") or symbol.startswith("SZ.") or symbol.startswith("CN."):
            return "CN"
        else:
            return "US"  # 默认美股
    ...
    def is_trading_hours(cls, symbol: str, dt: Optional[datetime] = None) -> bool:
        ...
        # 简单判断：工作日 + 交易时段
        local_weekday = (dt.weekday() + (1 if offset < 0 and dt.hour + offset < 0 else 0)) % 7
...
def detect_suspension(kline_df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
    ...
    if kline_df is None or kline_df.empty:
        return {"status": "delisted", "suspension_days": [], "last_trade_date": None, "confidence": 0.9}
    ...
    if len(df) < 5:
        return {"status": "normal", "suspension_days": [], "last_trade_date": None, "confidence": 0.5}
    ...
    return {
        ...
        "last_trade_date": last_date.strftime("%Y-%m-%d") if pd.notna(last_date) else None,
...
class DataQualityChecker:
    ...
    def check(self, kline_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if total_rows == 0:
            ...
            critical_count = sum(1 for a in self.anomalies if a.get("severity") == "critical")
