...
class WebhookAlertHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord):
        try:
            ...
            # 如果是 Telegram，只需修改 payload 结构为 {"chat_id": "你的ID", "text": "..."}  # noqa: E501
            payload = {
                "msgtype": "text",
                "text": {
            ...
            # 🛡️ 发送 HTTP 请求：因为运行在 QueueListener 的独立守护线程中，网络 I/O 绝对不会阻塞行情网关  # noqa: E501
...
def configure_logging(level: int = logging.INFO) -> logging.Logger:
    ...
    def _create_file_handler(filename: str, levels: list) -> TimedRotatingFileHandler:
        ...
        handler.suffix = "%Y-%m-%d"  # 切割后的历史文件名将带有日期后缀 (如 info.log.2026-06-11)  # noqa: E501
