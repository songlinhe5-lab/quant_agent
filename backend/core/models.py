...
class SentimentRecord(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, index=True)
    timestamp: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), server_default=func.now(), index=True)  # noqa: E501
...
class WebpageKnowledgeBase(Base):
    ...
    user_id: Mapped[Optional[int]] = mapped_column(Integer, index=True, nullable=True) # 标量：多租户隔离 (NULL 为系统全局公共库)  # noqa: E501
    ...
    __table_args__ = (
        ...
        Index(
            ...
            postgresql_with={'m': 16, 'ef_construction': 64},      # HNSW 超参数 (平衡召回率与内存)  # noqa: E501
...
class ScreenerRule(Base):
    ...
    rule_type: Mapped[Optional[str]] = mapped_column(String, index=True, nullable=True) # 标量：用于类别过滤  # noqa: E501
