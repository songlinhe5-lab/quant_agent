import os
from sqlalchemy import JSON, ForeignKey, Float, DateTime, String, Text, Integer, Index
...
class User(Base):
    ...
    # 关联配置表
    preferences: Mapped[Optional["UserPreference"]] = relationship(back_populates="owner", uselist=False)
...
class UserPreference(Base):
    ...
    macro_symbols: Mapped[List[str]] = mapped_column(JSON, default=lambda: ["SPY", "QQQ", "VIX", "TNX"])
...
class SentimentRecord(Base):
    ...
    vix_value: Mapped[Optional[float]] = mapped_column(Float, nullable=True)          # 恐慌指数 VIX
    pc_ratio: Mapped[Optional[float]] = mapped_column(Float, nullable=True)           # 期权多空比 Put/Call Ratio
    credit_spread: Mapped[Optional[float]] = mapped_column(Float, nullable=True)      # 高收益债利差
    fear_greed_score: Mapped[Optional[int]] = mapped_column(nullable=True)            # 贪婪恐惧指数
...
class PerformanceLog(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, index=True)
    timestamp: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), server_default=func.now(), index=True)
    log_type: Mapped[str] = mapped_column(String(50), index=True) # 'event_loop_block' 或 'slow_request'
    duration_ms: Mapped[float] = mapped_column(Float)             # 耗时或卡顿延迟 (毫秒)
    endpoint: Mapped[Optional[str]] = mapped_column(String(255), nullable=True) # 发生慢请求的 API 路径
    details: Mapped[Optional[str]] = mapped_column(String, nullable=True)       # 附加的详情描述
...
class WebpageKnowledgeBase(Base):
    ...
    user_id: Mapped[Optional[int]] = mapped_column(Integer, index=True, nullable=True) # 标量：多租户隔离 (NULL 为系统全局公共库)
    ...
    __table_args__ = (
        ...
        Index(
            ...
            postgresql_with={'m': 16, 'ef_construction': 64},      # HNSW 超参数 (平衡召回率与内存)
            postgresql_ops={'embedding': 'vector_cosine_ops'}      # 指定使用余弦距离运算类
        ),
    )
...
class ScreenerRule(Base):
    ...
    rule_type: Mapped[Optional[str]] = mapped_column(String, index=True, nullable=True) # 标量：用于类别过滤
    user_id: Mapped[Optional[int]] = mapped_column(Integer, index=True, nullable=True)  # 标量：多租户隔离
