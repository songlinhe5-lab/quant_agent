...
# 引入编译好的 Protobuf 模块
from backend.core.proto.market_pb2 import QuoteData, Order  # type: ignore
...
from backend.core.utils import safe_float, safe_divide
...
async def update_quote_to_redis(ticker: str, quote_data: dict):
    ...
    try:
        ...
        for b in quote_data.get("bids", []):
            quote_msg.bids.append(Order(price=float(b.get("price", 0.0)), size=float(b.get("size", 0.0))))
        for a in quote_data.get("asks", []):
            quote_msg.asks.append(Order(price=float(a.get("price", 0.0)), size=float(a.get("size", 0.0))))
            ...
            if user_alerts:
                for user_id_bytes, rules_str in user_alerts.items():
                    ...
                    if upper is not None and last_price >= upper:
                        triggered_msg = f"🚀 [专属报警] {ticker} 最新价 {last_price} 已向上突破您设定的上限 {upper}！"
                    elif lower is not None and last_price <= lower:
                        triggered_msg = f"🩸 [专属报警] {ticker} 最新价 {last_price} 已向下跌破您设定的下限 {lower}！"
                    elif pct_change is not None:
                        try:
                            current_pct = float(change_pct_str.replace('%', '').replace('+', ''))
                            if abs(current_pct) >= pct_change:
                                direction = "暴涨" if current_pct > 0 else "暴跌"
                                triggered_msg = f"💥 [异动报警] {ticker} 发生 {direction}，当前涨跌幅 {change_pct_str}，已触及您设定的 ±{pct_change}% 阈值！"
                        except ValueError:
                            pass
                        
                    if triggered_msg:
                        # 触发后仅删除该用户的一次回调报警，防止震荡轰炸
                        await redis_client.hdel(f"quant:alerts:by_ticker:{ticker}", user_id)
                        # 可进一步修改 notification_service 发送给指定 userid 绑定的飞书/钉钉
                        asyncio.create_task(notification_service.send_alert(f"[To User: {user_id}] " + triggered_msg))
...
async def update_trade_to_redis(ticker: str, trade_data: bytes):
    ...
    try:
        ...
        # 1. 写入 Redis Stream，maxlen=5000 保证内存不会无限膨胀，保留最近 5000 条用于断线追补
...
class ConnectionManager:
    ...
    async def _catch_up_or_snapshot(self, websocket: WebSocket, tickers: list[str], last_ids: dict):
        try:
            for t in tickers:
                last_id = last_ids.get(t)
                if last_id:
                    # 💡 核心操作：通过 XRANGE 获取 (last_id, +无穷大] 区间错过的所有包
                    missed_messages = await self.raw_redis.xrange(f"quant:trades:stream:{t}", min=f"({last_id}", max="+")
                    if missed_messages:
                        ...
                        if len(payloads) > 100:
                            # 💡 高频优化：超过 100 条消息，组合成单一的二进制 Batch 帧一次性发送
                            # 格式: [0x01(表示zlib压缩模式)] + zlib_compressed([4字节包数量] + 循环([4字节单包长度] + [单包二进制数据]))
                            raw_buffer = bytearray()
                            raw_buffer.extend(struct.pack('<I', len(payloads))) # Little-endian Unsigned Int
                            for p in payloads:
                    ...
                    if isinstance(cached_bytes, bytes) and websocket in self.active_connections:
    ...
    def get_all_subscribed_tickers(self) -> set[str]:
        ...
        # 💡 将宏观指数替换为富途格式 (如 US.VIX, US.SPX) 以优先利用富途快速获取，并保留比特币与日元汇率
        # 💡 加入 6 大核心资本板块代理 ETF，确保后台守护进程高频拉取其资金流，解耦耗时的外部 API 轮询
        all_tickers = {"GC=F", "CL=F", "^TNX", "ES=F", "US.VIX", "US.SPX", "US.IXIC", "^FVX", "DX-Y.NYB", "JPY=X", "BTC-USD", "US.SPY", "US.QQQ", "US.SOXX", "US.TLT", "US.KWEB", "SH.510300"}
        for tickers in self.subscriptions.values():
            all_tickers.update(tickers)
        return all_tickers
        
    async def redis_pubsub_listener(self) -> None:
        """[Consumer 职责]：持续监听 Redis 的 Pub/Sub 并定向推送给本机连接的前端客户端"""
    ...
    async def broadcast_loop(self) -> None:
        """背景任务持续拉取技术指标缓存与 YFinance 轮询兜底 (即使没有前端连接也保持更新 Redis)"""
        while True:
            try:
                all_tickers = list(self.get_all_subscribed_tickers())
                if all_tickers:
                    # 💡 动态读取全局 YFinance 开关状态
                    yf_enabled_val = await l1_cached_redis.get("quant:settings:yfinance_enabled")
                    is_yf_enabled = yf_enabled_val != "0"

                    # 💡 [额度释放 GC 机制]：对比当前真实需要的标的与已订阅的标的，自动剔除无人观看的废弃订阅
                    current_futu_needs = {t for t in all_tickers if not futu_service.is_futu_unsupported(t)}
                    stale_subs = self._futu_active_subs - current_futu_needs
                    
                    if stale_subs:
                        print(f"🧹 [Futu GC] 检测到 {len(stale_subs)} 只标的已无活跃监听，正在向富途发起退订以释放额度...")
                    ...
                    # 💡 优化 1: 定时刷新技术面指标缓存
                    # 技术指标(日线级)无需每 3 秒全量并发刷新。设定为 1 小时更新一次，或发现新标的时触发
                    current_time = time.time()
                    need_global_tech_update = current_time - getattr(self, "last_tech_update", 0) > 3600
                    tickers_to_update = [t for t in all_tickers if t not in self.tech_cache or need_global_tech_update]
                    ...
                    if tickers_to_update:
                        ...
                        for t in tickers_to_update:
                            try:
                                # 优先尝试利用 Futu 获取 120 日历史，完全免除外部网络请求
                                futu_res = await futu_service.get_history(t, ktype="K_DAY", num=120)
                                if futu_res.get("status") == "success" and futu_res.get("data"):
                                    ...
                                    if len(df) >= 30:
                                        ...
                                        res = await yf_service.get_tech_indicators(ticker=t, lookback_days=6, pre_fetched_df=df)
                                        if res.get("status") == "success":
                                            self.tech_cache[t] = res.get("data", {}).get("trend", [])
                                            continue
                                            
                                # 如果 Futu 失败（加密货币/外汇等），且开启了 YF 兜底，则串行请求 YF
                                if is_yf_enabled:
                                    res = await yf_service.get_tech_indicators(ticker=t, lookback_days=6)
                                    if res.get("status") == "success":
                                        self.tech_cache[t] = res.get("data", {}).get("trend", [])
                            except Exception as e:
                                print(f"⚠️ [Tech Cache] 更新 {t} 指标异常: {e}")
                            
                            # 🚨 核心防封控：必须错峰串行！杜绝高并发导致触发雅虎 429 封IP
                            await asyncio.sleep(1.0)
                            
                        if need_global_tech_update:
                            self.last_tech_update = current_time

                    # 💡 定时拉取资金流与席位 (Futu 内部已有 0.6s 排队锁，使用 gather 并发安全)
                    flow_tasks = [futu_service.get_fund_flow(ticker=t) for t in all_tickers]
                    ...
                    for t in all_tickers:
                        ...
                        if t in monitor_targets:
                            ...
                            if flow_data:
                                ...
                                OUTFLOW_THRESHOLD = -500_000_000  # 阈值：主力净流出超过 5 亿
                                if net_inflow < OUTFLOW_THRESHOLD:
                                    # 💡 升级为分布式防抖锁，防止集群多台服务器同时发出报警轰炸
                                    lock_key = f"quant:lock:outflow_alert:{t}:{int(current_time / 3600)}"
                                    if await redis_client.set(lock_key, "1", nx=True, ex=3600):
                                        alert_msg = (
                    ...
                    if current_time - getattr(self, "last_acc_update", 0) > 10:
                        ...
                        try:
                            ...
                            if acc_res.get("status") == "success":
                                ...
                                self.last_account_summary = f"总资产: {total_assets:,.2f} | 浮动盈亏: {total_pnl:+,.2f}"
                        except Exception:
                            pass
                    ...
                    if futu_check_tickers:
                                        ...
                                        if isinstance(fund_data, dict):
                                            ...
                                            if isinstance(bq, dict):
                                                if bq.get("bid_brokers_queue_str"):
                                                    res["bid_brokers_queue_str"] = bq.get("bid_brokers_queue_str")
                                                if bq.get("ask_brokers_queue_str"):
                                                    res["ask_brokers_queue_str"] = bq.get("ask_brokers_queue_str")
                                            if fund_data.get("main_fund_net_inflow_str"):
                                                res["main_fund_net_inflow_str"] = fund_data.get("main_fund_net_inflow_str")
                                            
                                    self.last_futu_update[t] = time.time()
                                    await update_quote_to_redis(t, res)
                                else:
                                    last_futu_error = res.get("message", "API 返回错误状态")
                                    yf_candidates.append(t)
                                    # 💡 失败时也更新其访问时间，防止下个循环立刻重试引发连环报错
                                    self.last_futu_update[t] = time.time()
                            except Exception as e:
                        ...
                        # 🚨 运行时富途断连报警 (在批处理循环外执行，真正解决防抖失效引发的连环轰炸)
                        if batch_success:
                    ...
                    if yf_candidates and is_yf_enabled:
                        # 💡 无论从哪进来的获取请求，统统汇入微批队列，1 秒后打包为 1 个请求发车
                        quote_tasks = [yf_service.get_batched_quote(t) for t in yf_candidates]
                        ...
                        for ticker, q in zip(yf_candidates, quote_results):
                            if isinstance(q, dict) and q.get("status") == "success":
                                q["ticker"] = ticker # 覆盖被 YF 格式化去掉了前缀的 Ticker，恢复为内部标准
