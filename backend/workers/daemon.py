...
TRADE_LOG_DB = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "trade_logs.db")
...
class QuoteMonitorHandler(StockQuoteHandlerBase):
    ...
    async def execute_trade(self, ticker: str, action: str, qty: int, price: float):
        ...
        async with httpx.AsyncClient() as client:
            payload = {"action": action, "ticker": ticker, "qty": qty, "price": price, "order_id": ""}
            try:
                resp = await client.post(f"{backend_url}/api/trade/order", json=payload, timeout=10.0)
    ...
    def on_recv_rsp(self, rsp_pb):
        ...
        if ticker == TARGET_TICKER and last_price <= TARGET_BUY_PRICE and not self.has_traded:
            ...
            result = asyncio.run(self.execute_trade(ticker=ticker, action=action_type, qty=trade_qty, price=last_price))
            
            log_trade(ticker=ticker, action=action_type, price=last_price, qty=trade_qty, status=result.get("status", "unknown"), message=result.get("message", ""))
            
            if result.get("status") == "success":
                print(f"✅ [执行成功] {result.get('message')}\n")
                asyncio.run(notification_service.send_alert(f"✅ [Quant Agent] 交易成功！\n\n标的: {ticker}\n价格: {last_price}\n动作: 买入 1 股\n回执: {result.get('message')}"))
            else:
                print(f"❌ [执行失败] {result.get('message')}\n")
                asyncio.run(notification_service.send_alert(f"❌ [Quant Agent] 交易受阻！\n\n标的: {ticker}\n当前价格: {last_price}\n失败原因: {result.get('message')}"))
