...
class QuotePublisher:
    """
    行情生产者任务 (Publisher)
    负责后台独立轮询外部 API，清洗数据后推送到 Redis 消息总线，彻底与 WebSocket 前端解耦。
    """
    ...
    async def _fetch_futu_data(self, ticker: str) -> dict[str, Any]:
        ...
        if isinstance(quote_result, str):
            try:
                ...
                raise ValueError(f"FutuTool 返回了非法的报价 JSON: {quote_result}")
        ...
        return {
            ...
            "volume_str": quote_result.get("volume_str", "--"), # 补充成交量
            "bids": bids,
            "asks": asks,
            "source": "futu"
        }
    ...
    def _get_mock_data(self, ticker: str) -> dict[str, Any]:
        ...
        return {"ticker": ticker, "last_price": 100.00, "change_pct": "0.0%", "volume_str": "0", "bids": [], "asks": [], "source": "mock"}
    ...
    async def poll_and_publish(self, ticker: str):
        ...
        try:
            ...
            err_msg = "拉取超时" if isinstance(e, asyncio.TimeoutError) else f"拉取异常 {type(e).__name__}: {e}"
        ...
        if data:
            ...
            if data.get('bids') or data.get('asks'):
                log_msg += f" | 盘口: {len(data.get('bids', []))}/{len(data.get('asks', []))}"
    ...
    async def run_daemon(self, tickers: list[str], interval: float = 1.0):
        ...
        async def _bounded_poll(t: str):
            ...
            # 否则 0.5 秒内这 2 个并发槽位会被死死霸占，导致 20 只股票的更新被硬生生拉长到 5 秒，完全失去高频意义。
        ...
        try:
            while self.is_running:
                ...
                # 💡 及时释放大对象：清除任务组和闭包结果，防止在接下来的 interval 休眠期内成为幽灵内存
                tasks = None
