...
def _fallback_mock_macro() -> dict:
    return {
        ...
        "data": [
            {"date": "2026-05-27T18:00:00Z", "country": "US", "event": "Fed Interest Rate Decision", "impact": "high", "previous": 4.50, "estimate": 4.50, "actual": None},  # noqa: E501
            {"date": "2026-05-29T03:00:00Z", "country": "JP", "event": "BOJ Core CPI YoY", "impact": "high", "previous": 2.1, "estimate": 2.3, "actual": None}  # noqa: E501
        ]
    }
    ...
    async with _macro_locks[cache_key]:
        ...
        try:
            ...
            for row in events:
                ...
                high_impact_keywords = ["rate", "cpi", "gdp", "payroll", "employment", "nfp", "fed", "ecb", "boj", "fomc", "pmi", "ism", "claims", "利率", "决议", "非农", "失业", "通胀", "国内生产总值", "pce", "lpr", "mlf", "pboc", "降息", "降准", "准备金"]  # noqa: E501
                is_core_event = any(k in event_name.lower() for k in high_impact_keywords)  # noqa: E501
                if is_core_event: impact = "high"  # noqa: E701

                # 💡 移除后端的 hard-filter，把所有级别的数据交给前端，让前端的 UI 筛选按钮真正生效  # noqa: E501
                compressed_events.append({"date": iso_time, "country": str(row.get("country", "Global")), "event": event_name, "impact": impact, "previous": str(row.get("previous", "")), "estimate": str(row.get("estimate", "")), "actual": str(row.get("actual", ""))})  # noqa: E501
            ...
            if res.get("source") == "fred":
                result["message"] = "💡 宏观日历已降级至 FRED 数据源。受接口限制，暂不提供预期值(Estimate)与实际公布值(Actual)。"  # noqa: E501
            ...
            if compressed_events:
                try:
                    ...
                    events_info = "\n".join([f"- {e['date'][:10]}: [{e['country']}] {e['event']}" for e in upcoming_events])  # noqa: E501
                    ...
                    if cached_ai:
                        result["ai_deduction"] = cached_ai.decode('utf-8') if isinstance(cached_ai, bytes) else cached_ai  # noqa: E501
                    else:
...
async def _fetch_earnings_calendar_data(days_ahead: int, force_refresh: bool = False) -> dict:  # noqa: E501
    ...
    async with _macro_locks[cache_key]:
        ...
        try:
            ...
            result = {"status": "success", "data": earnings_list, "source": res.get("source")}  # noqa: E501

            # 💡 新增：大模型财报前瞻推演
            if earnings_list:
                try:
                    upcoming = earnings_list[:3] # 挑选近期发布财报的3家明星公司
                    info_str = "\n".join([f"- {e.get('date')}: {e.get('symbol')} (预期 EPS: {e.get('epsEstimate', 'N/A')})" for e in upcoming])  # noqa: E501
...
def get_sentiment_history(limit: int = Query(200, le=2000, description="获取历史数据点数量"), db: Session = Depends(get_db)):  # noqa: E501
...
async def _fetch_capital_flows() -> tuple[list, bool]:
    ...
    try:
        ...
        results = await asyncio.gather(south_task, csi300_task, spy_task, qqq_task, soxx_task, tlt_task, kweb_task, return_exceptions=True)  # noqa: E501
        ...
        flows.extend([
            {"market": "CN", "label": "A股核心", "amount": csi_amount, "unit": "亿人民币", "dir": csi_dir, "desc": csi_desc, "sparkDirs": [1, 1, 1, 1, -1, 1, 1, 1]},  # noqa: E501
            {"market": "US", "label": "美股大盘", "amount": spy_amount, "unit": spy_unit, "dir": spy_dir, "desc": spy_desc, "sparkDirs": [1, 1, 1, -1, 1, 1, 1, 1]},  # noqa: E501
            {"market": "US", "label": "美股科技", "amount": qqq_amount, "unit": qqq_unit, "dir": qqq_dir, "desc": qqq_desc, "sparkDirs": [-1, 1, 1, 1, 1, 1, -1, 1]},  # noqa: E501
            {"market": "US", "label": "半导体", "amount": soxx_amount, "unit": soxx_unit, "dir": soxx_dir, "desc": soxx_desc, "sparkDirs": [1, -1, 1, 1, 1, -1, 1, 1]},  # noqa: E501
            {"market": "US", "label": "美债避险", "amount": tlt_amount, "unit": tlt_unit, "dir": tlt_dir, "desc": tlt_desc, "sparkDirs": [-1, -1, -1, 1, -1, -1, -1, -1]},  # noqa: E501
            {"market": "CN", "label": "中概互联", "amount": kweb_amount, "unit": kweb_unit, "dir": kweb_dir, "desc": kweb_desc, "sparkDirs": [1, -1, 1, -1, 1, -1, 1, 1]},  # noqa: E501
        ])
...
async def get_capital_flow():
    ...
    try:
        ...
        async with _macro_locks[cache_key]:
            cached = await redis_client.get(cache_key)
            if cached: return json.loads(cached)  # noqa: E701
...
async def get_data_center_dashboard(force_refresh: bool = Query(False, description="强制绕过缓存拉取最新数据")):  # noqa: E501
    """聚合大盘看板所需的所有核心数据"""
    ...
    try:
        ...
        async with _macro_locks[cache_key]:
            ...
            economic_events = events_res.get("data", []) if isinstance(events_res, dict) and events_res.get("status") in ("success", "warning") else []  # noqa: E501
            economic_events_msg = events_res.get("message", "") if isinstance(events_res, dict) and events_res.get("status") in ("success", "warning") else ""  # noqa: E501
            economic_events_deduction = events_res.get("ai_deduction", "") if isinstance(events_res, dict) and events_res.get("status") in ("success", "warning") else ""  # noqa: E501
            news_items = news_res.get("data", []) if isinstance(news_res, dict) and news_res.get("status") in ("success", "warning") else []  # noqa: E501
            earnings_calendar = earnings_res.get("data", []) if isinstance(earnings_res, dict) and earnings_res.get("status") in ("success", "warning") else []  # noqa: E501
            earnings_calendar_deduction = earnings_res.get("ai_deduction", "") if isinstance(earnings_res, dict) and earnings_res.get("status") in ("success", "warning") else ""  # noqa: E501
...
async def _fetch_macro_assets_data():
    ...
    async def fetch_single_asset(config):
        symbol = config["symbol"]; name = config["name"]; yf_code = config["yf"]  # noqa: E702
        try:
            ...
            if cached_data:
                ...
                if records and len(records) > 0:
                    ...
                    for r in records:
                        ...
                        o_val = r.get("Open")
                        if c_val is None:
                            c_val = next((v for k, v in r.items() if str(k).startswith("('Close'")), None)  # noqa: E501
                        if o_val is None:
                            o_val = next((v for k, v in r.items() if str(k).startswith("('Open'")), None)  # noqa: E501
                    ...
                    if len(closes) > 0:
                        ...
                        prev_close = closes[-2] if len(closes) > 1 else (float(open_vals[-1]) if open_vals else last_close)  # noqa: E501
                        change_pct = ((last_close - prev_close) / prev_close) * 100 if prev_close else 0.0  # noqa: E501
                        return {"symbol": symbol, "name": name, "value": round(last_close, 2), "change": round(change_pct, 2), "sparkline": closes}  # noqa: E501
        except Exception as e:
            print(f"⚠️ [Macro] 从 Redis 解析 {symbol} 失败: {e}")
        return {"symbol": symbol, "name": name, "value": 0.0, "change": 0.0, "sparkline": [0, 0]}  # noqa: E501
...
@router.get("/assets")
async def get_macro_assets(force_refresh: bool = Query(False, description="强制绕过缓存拉取最新数据")):  # noqa: E501
    """获取大类资产与宏观风险雷达数据"""
    ...
    try:
        ...
        async with _macro_locks[cache_key]:
            ...
            def _chg(s): a = with_assets.get(s); return float(a["change"]) if a and a.get("change") is not None else None  # noqa: E501, E702
            def _norm_pct(pct, neutral=0.0, scale=2.0, inverse=False):
                if pct is None: return 50  # noqa: E701
                adjusted = -(pct - neutral) / scale if inverse else (pct - neutral) / scale  # noqa: E501
                import math; raw = 1.0 / (1.0 + math.exp(-adjusted)); return round(raw * 100, 1)  # noqa: E501, E702, I001
            def _s(s): a = with_assets.get(s); return float(a["value"]) if a and a.get("value") else None  # noqa: E501, E702

            usdjpy_chg = _chg("JPY=X"); vix_chg = _chg("VIX")  # noqa: E702
            _liq_scores = [s for s in [_norm_pct(usdjpy_chg, inverse=True), _norm_pct(vix_chg, inverse=True)] if s is not None]  # noqa: E501
            liq_raw = sum(_liq_scores) / len(_liq_scores) if _liq_scores else 50
            vix_abs = _s("VIX"); vola = round(max(0, min(100, 100 - (vix_abs - 10) * 2.5)), 1) if vix_abs else 50  # noqa: E501, E702
            eq_chgs = [c for c in [_chg("SPX"), _chg("IXIC"), _chg("HSI"), _chg("HSTECH"), _chg("N225")] if c is not None]  # noqa: E501
            ...
            tnx_chg = _chg("TNX"); bond = _norm_pct(tnx_chg, inverse=True) if tnx_chg is not None else 50  # noqa: E501, E702
            dxy_chg = _chg("DX-Y"); fx = _norm_pct(dxy_chg, inverse=True) if dxy_chg is not None else 50  # noqa: E501, E702
            ...
            try:
                ...
                if cpc_cache:
                    ...
                    if cpc_records and len(cpc_records) > 0:
                        ...
                        if c_val is None:
                            c_val = next((v for k, v in cpc_records[-1].items() if str(k).startswith("('Close'")), None)  # noqa: E501
                        if c_val:
            ...
            sentiment_indicators = {"pc_ratio": {"value": cpc_val, "status": pc_status}, "credit_spread": {"value": credit_spread, "status": cs_status}}  # noqa: E501

            radar_data = [
                {"axis": "流动性", "current": liq_raw, "benchmark": 60, "desc": "反映全球资金充裕度与风险偏好。"},  # noqa: E501
                {"axis": "波动率", "current": vola, "benchmark": 55, "desc": "反映市场恐慌与不确定性。"},  # noqa: E501
                {"axis": "权益", "current": equity, "benchmark": 60, "desc": "全球核心股市多头动能。"},  # noqa: E501
                {"axis": "商品", "current": commodity, "benchmark": 55, "desc": "大宗商品活跃度与通胀预期。"},  # noqa: E501
                {"axis": "债券", "current": bond, "benchmark": 50, "desc": "无风险利率与货币政策环境。"},  # noqa: E501
                {"axis": "汇率", "current": fx, "benchmark": 50, "desc": "非美资产汇率压力。"},  # noqa: E501
                {"axis": "中概强度", "current": cn_strength, "benchmark": 55, "desc": "中国海外核心资产动量。"},  # noqa: E501
                {"axis": "数字货币", "current": crypto, "benchmark": 50, "desc": "加密资产投机情绪。"},  # noqa: E501
            ]
...
async def websocket_macro_calendar(websocket: WebSocket):
    ...
    async def listen_redis():
        async for message in pubsub.listen():
            if message["type"] == "message":
                data = json.loads(message["data"]); data["channel"] = message["channel"]  # noqa: E702
                await websocket.send_json(data)
            ...
            for event in result["data"]:
                if event.get("date"):
                    try:
                        ...
                        if d == current_date: today_events.append(event)  # noqa: E701
                    except Exception: pass  # noqa: E701
        await websocket.send_json({"type": "macro_alert", "message": f"今日共 {len(today_events)} 个高影响事件", "events": today_events})  # noqa: E501
