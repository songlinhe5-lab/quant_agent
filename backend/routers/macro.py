...
def _fallback_mock_macro() -> dict:
    return {
        ...
        "data": [
            {"date": "2026-05-27T18:00:00Z", "country": "US", "event": "Fed Interest Rate Decision", "impact": "high", "previous": 4.50, "estimate": 4.50, "actual": None},
            {"date": "2026-05-29T03:00:00Z", "country": "JP", "event": "BOJ Core CPI YoY", "impact": "high", "previous": 2.1, "estimate": 2.3, "actual": None}
        ]
    }


async def _fetch_macro_calendar_data(days_ahead: int, force_refresh: bool = False) -> dict:
    ...
    async with _macro_locks[cache_key]:
        ...
        try:
            res = await akshare_service.get_economic_calendar(days_ahead, skip_cache=force_refresh)
            if res.get("status") == "error" or not res.get("data"):
                print(f"⚠️ [Macro] 金十数据降级失败或为空，继续安全降级 (FRED)...")
                res = await fred_service.get_economic_calendar(days_ahead, skip_cache=force_refresh)
                if res.get("status") == "error" or not res.get("data"):
                    print(f"⚠️ [Macro] FRED 降级失败，使用离线 Mock 数据")
            ...
            for row in events:
                ...
                # 💡 时区修复：FRED 数据是美东时间，需要借助 zoneinfo (自动处理夏令时) 转换为真实的 UTC 发给前端
                if is_fred and raw_time and len(raw_time) == 19:
                    try:
                        dt_ny = datetime.strptime(raw_time, "%Y-%m-%d %H:%M:%S")
                        if zoneinfo:
                            dt_ny = dt_ny.replace(tzinfo=zoneinfo.ZoneInfo("America/New_York"))
                            iso_time = dt_ny.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
                        else:
                    ...
                    try:
                        ...
                        if zoneinfo:
                            dt_cn = dt_cn.replace(tzinfo=zoneinfo.ZoneInfo("Asia/Shanghai"))
                            iso_time = dt_cn.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
                        else:
                            iso_time = raw_time.replace(" ", "T") + "+08:00"
                    except Exception:
                        iso_time = raw_time.replace(" ", "T") + "Z"
                else:
                    iso_time = raw_time.replace(" ", "T") + "Z" if raw_time else today.strftime("%Y-%m-%dT%H:%M:%SZ")

                impact = str(row.get("impact", "low")).lower()
                # 💡 增加中文核心指标及中国央行(PBOC)专项识别，确保 LPR/MLF 及降准降息数据能被正确打上高危红色预警标签
                high_impact_keywords = ["rate", "cpi", "gdp", "payroll", "employment", "nfp", "fed", "ecb", "boj", "fomc", "pmi", "ism", "claims", "利率", "决议", "非农", "失业", "通胀", "国内生产总值", "pce", "lpr", "mlf", "pboc", "降息", "降准", "准备金"]
                is_core_event = any(k in event_name.lower() for k in high_impact_keywords)
                if is_core_event: impact = "high"
                    
                # 💡 移除后端的 hard-filter，把所有级别的数据交给前端，让前端的 UI 筛选按钮真正生效
                compressed_events.append({"date": iso_time, "country": str(row.get("country", "Global")), "event": event_name, "impact": impact, "previous": str(row.get("previous", "")), "estimate": str(row.get("estimate", "")), "actual": str(row.get("actual", ""))})
            ...
            if res.get("source") == "fred":
                result["message"] = "💡 宏观日历已降级至 FRED 数据源。受接口限制，暂不提供预期值(Estimate)与实际公布值(Actual)。"
            elif res.get("source") == "jin10":
                result["message"] = "💡 宏观日历已平滑降级至金十数据 (Jin10)，支持完整的预期值与中文事件解析。"
            ...
            if compressed_events:
                try:
                    ...
                    events_info = "\n".join([f"- {e['date'][:10]}: [{e['country']}] {e['event']}" for e in upcoming_events])

                    # 💡 利用哈希值对大模型推演进行独立长效缓存，防止 force_refresh=true 时反复调用 LLM 造成长达 8 秒的延迟
                    ...
                    if cached_ai:
                        result["ai_deduction"] = cached_ai.decode('utf-8') if isinstance(cached_ai, bytes) else cached_ai
                    else:
                        prompt = f"你是顶级宏观量化分析师。以下是未来几天即将发布的全球核心宏观经济数据：\n{events_info}\n\n请对这些事件做一次前瞻性预判。挑选最核心的事件，向交易员解释该数据对当前降息预期或经济衰退的影响，以及如果数据异常走高/走低，可能对大盘资产产生怎样的冲击？字数严格控制在150字以内，直接输出精炼的推演结论，无需多余客套话。"
                        ...
                        if content:
                            ...
                            await redis_client.setex(ai_cache_key, 86400 * 3, ai_deduction) # 缓存 3 天
...
async def _fetch_earnings_calendar_data(days_ahead: int, force_refresh: bool = False) -> dict:
    """带缓存的大模型财报日历前瞻推演包装器"""
    ...
    async with _macro_locks[cache_key]:
        ...
        try:
            res = await finnhub_service.get_earnings_calendar(days_ahead, skip_cache=force_refresh)
            if res.get("status") != "success":
                return res
                
            earnings_list = res.get("data", [])
            result = {"status": "success", "data": earnings_list, "source": res.get("source")}
            
            # 💡 新增：大模型财报前瞻推演
            if earnings_list:
                try:
                    upcoming = earnings_list[:3] # 挑选近期发布财报的3家明星公司
                    info_str = "\n".join([f"- {e.get('date')}: {e.get('symbol')} (预期 EPS: {e.get('epsEstimate', 'N/A')})" for e in upcoming])
                    ...
                    if cached_ai:
                        ...
                        prompt = f"你是顶级美股分析师。以下是未来几天即将发布财报的核心明星公司：\n{info_str}\n\n请对这几份财报做一次前瞻推演。重点挑选最知名的一两家，预测其财报超预期或不及预期可能对同板块或纳斯达克指数带来的联动影响。字数严格控制在150字以内，语言犀利、直接，无需多余客套话。"
                        ...
                        content = resp.choices[0].message.content
                        if content:
                            ai_deduction = re.sub(r'^```[a-zA-Z]*\s*|\s*```$', '', content.strip()).strip()
                            result["ai_deduction"] = ai_deduction
                            await redis_client.setex(ai_cache_key, 86400 * 3, ai_deduction) # 缓存 3 天
                except Exception as llm_e:
...
async def get_macro_calendar(
    days_ahead: int = Query(7, ge=1, le=30, description="获取未来 N 天内的高影响宏观经济事件")
):
    """获取全球核心经济体的宏观日历数据"""
    try:
...
def get_sentiment_history(limit: int = Query(200, le=2000, description="获取历史数据点数量"), db: Session = Depends(get_db)):
    ...
    try:
        records = db.query(models.SentimentRecord).order_by(models.SentimentRecord.timestamp.desc()).limit(limit).all()
        data = []
...
async def _fetch_capital_flows() -> tuple[list, bool]:
    ...
    try:
        ...
        # 💡 从后台实时引擎直接读取资金流缓存，避免每次用户请求都被 Futu 的串行限流阻塞 4 秒！
        ...
        results = await asyncio.gather(south_task, csi300_task, spy_task, qqq_task, soxx_task, tlt_task, kweb_task, return_exceptions=True)
        ...
        if south_res.get("status") in ("success", "warning"):
            ...
            flows.append({
                "market": "HK", "label": "港股南向",
                "amount": sd.get("net_inflow", 0), "unit": "亿港元",
        ...
        csi_amount, csi_dir, csi_desc, csi_unit = _parse_futu_flow(csi300_res, 8.7, "沪深300ETF主力净流", "亿人民币")
        spy_amount, spy_dir, spy_desc, spy_unit = _parse_futu_flow(spy_res, 2.1, "标普500ETF主力净流", "亿美元")
        qqq_amount, qqq_dir, qqq_desc, qqq_unit = _parse_futu_flow(qqq_res, 3.5, "纳指科技ETF主力净流", "亿美元")
        soxx_amount, soxx_dir, soxx_desc, soxx_unit = _parse_futu_flow(soxx_res, 1.5, "半导体ETF主力净流", "亿美元")
        tlt_amount, tlt_dir, tlt_desc, tlt_unit = _parse_futu_flow(tlt_res, -1.8, "20年期美债ETF主力净流", "亿美元")
        kweb_amount, kweb_dir, kweb_desc, kweb_unit = _parse_futu_flow(kweb_res, 1.2, "中概互联ETF主力净流", "亿美元")

        flows.extend([
            {"market": "CN", "label": "A股核心", "amount": csi_amount, "unit": "亿人民币", "dir": csi_dir, "desc": csi_desc, "sparkDirs": [1, 1, 1, 1, -1, 1, 1, 1]},
            {"market": "US", "label": "美股大盘", "amount": spy_amount, "unit": spy_unit, "dir": spy_dir, "desc": spy_desc, "sparkDirs": [1, 1, 1, -1, 1, 1, 1, 1]},
            {"market": "US", "label": "美股科技", "amount": qqq_amount, "unit": qqq_unit, "dir": qqq_dir, "desc": qqq_desc, "sparkDirs": [-1, 1, 1, 1, 1, 1, -1, 1]},
            {"market": "US", "label": "半导体", "amount": soxx_amount, "unit": soxx_unit, "dir": soxx_dir, "desc": soxx_desc, "sparkDirs": [1, -1, 1, 1, 1, -1, 1, 1]},
            {"market": "US", "label": "美债避险", "amount": tlt_amount, "unit": tlt_unit, "dir": tlt_dir, "desc": tlt_desc, "sparkDirs": [-1, -1, -1, 1, -1, -1, -1, -1]},
            {"market": "CN", "label": "中概互联", "amount": kweb_amount, "unit": kweb_unit, "dir": kweb_dir, "desc": kweb_desc, "sparkDirs": [1, -1, 1, -1, 1, -1, 1, 1]},
        ])
...
async def _fetch_macro_news_from_stream(limit: int = 50) -> list:
    ...
    try:
        ...
        if members:
            return [json.loads(m) for m in members if isinstance(m, (str, bytes, bytearray))]
    except Exception as e:
...
async def get_macro_news(
    category: str = Query("general", description="新闻分类: general, forex, crypto, merger"),
...
async def websocket_live_news(websocket: WebSocket):
    ...
    try:
        ...
        # 💡 等待任务真正取消并归还 Redis 控制权，防止触发并发读写 RuntimeError 导致 close 被跳过
        if pending:
...
async def get_data_center_dashboard(force_refresh: bool = Query(False, description="强制绕过缓存拉取最新数据")):
    """聚合大盘看板所需的所有核心数据"""
    ...
    try:
        ...
        async with _macro_locks[cache_key]:
            ...
            (
                ...
                _fetch_earnings_calendar_data(days_ahead=7, force_refresh=force_refresh),
            ...
            if isinstance(assets_radar_res, dict) and assets_radar_res.get("status") == "success":
                radar_data = assets_radar_res.get("data", {}).get("radarData", [])
                macro_assets = assets_radar_res.get("data", {}).get("macroAssets", [])
                sentiment_indicators = assets_radar_res.get("data", {}).get("sentimentIndicators", {})
                
            # 💡 容错修复：允许包含警告信息的兜底 Mock 数据流向前端展示
            economic_events = events_res.get("data", []) if isinstance(events_res, dict) and events_res.get("status") in ("success", "warning") else []
            economic_events_msg = events_res.get("message", "") if isinstance(events_res, dict) and events_res.get("status") in ("success", "warning") else ""
            economic_events_deduction = events_res.get("ai_deduction", "") if isinstance(events_res, dict) and events_res.get("status") in ("success", "warning") else ""
            news_items = news_res.get("data", []) if isinstance(news_res, dict) and news_res.get("status") in ("success", "warning") else []
            earnings_calendar = earnings_res.get("data", []) if isinstance(earnings_res, dict) and earnings_res.get("status") in ("success", "warning") else []
            earnings_calendar_deduction = earnings_res.get("ai_deduction", "") if isinstance(earnings_res, dict) and earnings_res.get("status") in ("success", "warning") else ""
...
async def _fetch_macro_assets_data():
    """从 Redis 缓存极速拉取 12 个核心宏观指标（数据由后台 YF 守护进程负责离散化更新）"""
    assets_config = [
    ...
    async def fetch_single_asset(config):
        symbol = config["symbol"]; name = config["name"]; yf_code = config["yf"]
        try:
            ...
            if cached_data:
                ...
                if records and len(records) > 0:
                    # 提取收盘价序列供 sparkline 使用 (兼容老版本缓存中存在的 MultiIndex 字符串键)
                    closes = []
                    open_vals = []
                    for r in records:
                        c_val = r.get("Close")
                        ...
                        if o_val is None:
                            o_val = next((v for k, v in r.items() if str(k).startswith("('Open'")), None)
                    ...
                    if len(closes) > 0:
                        ...
                        prev_close = closes[-2] if len(closes) > 1 else (float(open_vals[-1]) if open_vals else last_close)
                        change_pct = ((last_close - prev_close) / prev_close) * 100 if prev_close else 0.0
                        return {"symbol": symbol, "name": name, "value": round(last_close, 2), "change": round(change_pct, 2), "sparkline": closes}
        except Exception as e:
            print(f"⚠️ [Macro] 从 Redis 解析 {symbol} 失败: {e}")
        return {"symbol": symbol, "name": name, "value": 0.0, "change": 0.0, "sparkline": [0, 0]}
...
@router.get("/assets")
async def get_macro_assets(force_refresh: bool = Query(False, description="强制绕过缓存拉取最新数据")):
    """获取大类资产与宏观风险雷达数据"""
    ...
    try:
        ...
        async with _macro_locks[cache_key]:
            ...
            def _chg(s): a = with_assets.get(s); return float(a["change"]) if a and a.get("change") is not None else None
            def _norm_pct(pct, neutral=0.0, scale=2.0, inverse=False):
                if pct is None: return 50
                adjusted = -(pct - neutral) / scale if inverse else (pct - neutral) / scale
                import math; raw = 1.0 / (1.0 + math.exp(-adjusted)); return round(raw * 100, 1)
            def _s(s): a = with_assets.get(s); return float(a["value"]) if a and a.get("value") else None

            usdjpy_chg = _chg("JPY=X"); vix_chg = _chg("VIX")
            _liq_scores = [s for s in [_norm_pct(usdjpy_chg, inverse=True), _norm_pct(vix_chg, inverse=True)] if s is not None]
            liq_raw = sum(_liq_scores) / len(_liq_scores) if _liq_scores else 50
            vix_abs = _s("VIX"); vola = round(max(0, min(100, 100 - (vix_abs - 10) * 2.5)), 1) if vix_abs else 50
            eq_chgs = [c for c in [_chg("SPX"), _chg("IXIC"), _chg("HSI"), _chg("HSTECH"), _chg("N225")] if c is not None]
            ...
            crypto = _norm_pct(sum(crypto_chgs) / len(crypto_chgs), scale=4.0) if crypto_chgs else 50
            cm_chgs = [c for c in [_chg("XAU"), _chg("WTI")] if c is not None]
            commodity = _norm_pct(sum(cm_chgs) / len(cm_chgs)) if cm_chgs else 50
            tnx_chg = _chg("TNX"); bond = _norm_pct(tnx_chg, inverse=True) if tnx_chg is not None else 50
            dxy_chg = _chg("DX-Y"); fx = _norm_pct(dxy_chg, inverse=True) if dxy_chg is not None else 50
            ...
            try:
                ...
                if cpc_cache:
                    ...
                    if cpc_records and len(cpc_records) > 0:
                        ...
                        if c_val is None:
                            c_val = next((v for k, v in cpc_records[-1].items() if str(k).startswith("('Close'")), None)
                        if c_val:
            ...
            sentiment_indicators = {"pc_ratio": {"value": cpc_val, "status": pc_status}, "credit_spread": {"value": credit_spread, "status": cs_status}}

            radar_data = [
                {"axis": "流动性", "current": liq_raw, "benchmark": 60, "desc": "反映全球资金充裕度与风险偏好。"},
                {"axis": "波动率", "current": vola, "benchmark": 55, "desc": "反映市场恐慌与不确定性。"},
                {"axis": "权益", "current": equity, "benchmark": 60, "desc": "全球核心股市多头动能。"},
                {"axis": "商品", "current": commodity, "benchmark": 55, "desc": "大宗商品活跃度与通胀预期。"},
                {"axis": "债券", "current": bond, "benchmark": 50, "desc": "无风险利率与货币政策环境。"},
                {"axis": "汇率", "current": fx, "benchmark": 50, "desc": "非美资产汇率压力。"},
                {"axis": "中概强度", "current": cn_strength, "benchmark": 55, "desc": "中国海外核心资产动量。"},
                {"axis": "数字货币", "current": crypto, "benchmark": 50, "desc": "加密资产投机情绪。"},
            ]
...
async def websocket_macro_calendar(websocket: WebSocket):
    ...
    async def listen_redis():
        async for message in pubsub.listen():
            if message["type"] == "message":
                data = json.loads(message["data"]); data["channel"] = message["channel"]
                await websocket.send_json(data)
            ...
            for event in result["data"]:
                if event.get("date"):
                    try:
                        ...
                        if d == current_date: today_events.append(event)
                    except Exception: pass
        await websocket.send_json({"type": "macro_alert", "message": f"今日共 {len(today_events)} 个高影响事件", "events": today_events})
