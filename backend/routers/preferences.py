...
async def update_preferences(prefs: Dict[str, Any] = Body(...), current_user: models.User = Depends(get_current_user)):
    ...
    try:
        ...
        # 3. 🎯 核心联动：如果前端修改了 yfinance 的降级开关，需要同步写入系统全局限流 Key
...
async def update_news_tags_rules(rules: Dict[str, str] = Body(...), current_user: models.User = Depends(get_current_user)):
    """更新动态新闻打标规则"""
    cache_key = "quant:settings:news_tags_rules"
    import re
    try:
        # 风控校验：确保传入的每一个正则表达式都是合法可编译的，防止 Finnhub 后台守护进程因此崩溃
        for tag, pattern in rules.items():
            try:
                re.compile(pattern)
            except Exception:
                raise HTTPException(status_code=400, detail=f"标签 '{tag}' 的正则表达式不合法: {pattern}")
...
async def batch_update_watchlist(req: WatchlistBatchRequest, current_user: models.User = Depends(get_current_user)):
    ...
    try:
        ...
        return {"status": "success", "message": f"成功将 {success_count} 只标的{action_str}核心监控池！"}
    except Exception as e:
