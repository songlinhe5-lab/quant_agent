...
from backend.core.backtest_engine import DivergenceResonanceStrategy, run_dynamic_sandbox_backtest
...
async def run_backtest(req: BacktestRequest):
    ...
    if req.data_source == "auto" and req.interval in ["1m", "5m"]:
        parquet_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "data", "parquet_db", f"{req.ticker}_{req.interval}.parquet"))
    ...
    if df is None or df.empty:
        if req.data_source in ["auto", "futu"]:
            period_days_map = {"1mo": 22, "3mo": 65, "6mo": 130, "1y": 252, "2y": 504, "5y": 1260, "max": 2500}
            ...
            try:
                ...
                futu_res = await futu_service.get_history(req.ticker, ktype=ktype, num=num_days)
        ...
        if (df is None or df.empty) and req.data_source in ["auto", "yfinance"]:
            ...
            success, df, msg = await yf_service.fetch_yf_data(req.ticker, "history", ttl=3600, period=req.period, interval=req.interval)
    ...
    if df is not None:
        ...
        if isinstance(df.columns, pd.MultiIndex):
            print(f"   ⚠️ [警告] 发现 MultiIndex 多级列结构！如果您的策略代码使用 df['Close'] 取值，大概率会失效并导致回测结果为空。")
        print(f"   - [前2条数据预览]:\n{df.head(2)}\n")
        print(f"   - [末2条数据预览]:\n{df.tail(2)}\n")
    else:
    ...
    if req.source_code and req.class_name:
        ...
        try:
            ...
            report = await asyncio.to_thread(
                run_dynamic_sandbox_backtest, safe_code, req.class_name, req.params or {}, df, req.initial_capital, req.debug_mode
            )
        except Exception as e:
