...
async def run_backtest(req: BacktestRequest):
    ...
    if req.data_source == "auto" and req.interval in ["1m", "5m"]:
        parquet_path = os.path.abspath(
            os.path.join(
                ...
                "data",
                "parquet_db",
                f"{req.ticker}_{req.interval}.parquet",
            )
        )  # noqa: E501
    ...
    if df is None or df.empty:
        ...
        if (df is None or df.empty) and req.data_source in ["auto", "yfinance"]:
            ...
            success, df, msg = await yf_service.fetch_yf_data(
                req.ticker,
                "history",
                ttl=3600,
                period=req.period,
    ...
    if df is not None:
        ...
        if isinstance(df.columns, pd.MultiIndex):
            print(
                "   ⚠️ [警告] 发现 MultiIndex 多级列结构！如果您的策略代码使用 df['Close'] 取值，大概率会失效并导致回测结果为空。"
            )  # noqa: E501
        print(f"   - [前2条数据预览]:\n{df.head(2)}\n")
        print(f"   - [末2条数据预览]:\n{df.tail(2)}\n")
    else:
