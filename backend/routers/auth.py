...
def _create_jwt_token(data: dict, expires_delta: timedelta, token_type: Optional[str] = None):
    to_encode = data.copy()
...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None):
    return _create_jwt_token(data, expires_delta or timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES))

def create_refresh_token(data: dict, expires_delta: Optional[timedelta] = None):
    return _create_jwt_token(data, expires_delta or timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS), "refresh")
...
def verify_google_token(
    ...
    try:
        ...
        if not email:
            raise HTTPException(status_code=400, detail="Google Auth Failed: No email provided")
        ...
        if has_email_column:
            ...
            user = db.query(models.User).filter(models.User.username == email.split('@')[0]).first()
            
        if not user:
            base_username = email.split('@')[0]
            existing_user = db.query(models.User).filter(models.User.username == base_username).first()
            username = base_username if not existing_user else f"{base_username}_{str(hash(email))[-4:]}"
...
# ==========================================
@router.post("/refresh")
async def refresh_access_token(refresh_token: Optional[str] = Cookie(None), db: Session = Depends(get_db)):
