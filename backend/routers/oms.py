...
try:
    from backend.services.oms_mock_data import INITIAL_BOTS, ACTIVE_ORDERS, HISTORICAL_TRADES, ALGO_EXECUTIONS
except ImportError:
    INITIAL_BOTS = []
    ACTIVE_ORDERS = []
    HISTORICAL_TRADES = []
...
async def mock_bot_log_generator():
    ...
    while True:
        try:
            ...
            running_bots = [bot for bot in INITIAL_BOTS if bot.get("status") == "running"]
            if not running_bots:
            ...
            # 模拟生成不同类型的日志
            log_type = random.choices(["info", "success", "warn"], weights=[0.7, 0.2, 0.1], k=1)[0]
            
            if log_type == "info":
                msg = random.choice(["Scanning market for entry signals...", f"Current position size: {random.randint(100, 500)} shares.", "ATR(14) is stable, maintaining trailing stop.", "Market volume is low, holding position."])
            elif log_type == "success":
                msg = f"✅ Trade executed: BOUGHT {random.randint(1,5)*100} shares of {bot_to_log['ticker']} @ {random.uniform(150, 160):.2f}"
            else: # warn
                msg = f"⚠️ High volatility detected! VIX spiked by {random.uniform(5, 15):.1f}%. Tightening stop loss."

            log_entry = {"time": datetime.now().strftime("%H:%M:%S"), "msg": msg, "type": log_type}
...
async def execute_emergency_liquidation():
    ...
    try:
        ...
        from futu import OrderType, TrdSide, ModifyOrderOp
        
        ctx = getattr(futu_service, "trade_ctx", None)
        if not ctx:
            logger.error("🚨 [KILL SWITCH] 未检测到有效的底层交易网关上下文 (trade_ctx)，降级为仅阻断新订单流。")
            
            # 💡 本地沙箱体验补充：如果没有连接真实券商，则在内存中模拟强平并广播，完成前端视觉闭环
            ACTIVE_ORDERS.clear()
            for bot in INITIAL_BOTS:
                if bot.get("status") != "error":
                    bot["status"] = "error"
                    bot["logs"].append({"time": datetime.now().strftime("%H:%M:%S"), "msg": "🚨 物理熔断触发，进程已强杀 (Mock)", "type": "warn"})
            ...
            await redis_client.publish("oms:algo_executions:update", json.dumps(ALGO_EXECUTIONS))
            return
        ...
        def cancel_all_orders():
            ...
            if ret == 0 and not data.empty:
                for _, row in data.iterrows():
                    ctx.modify_order(ModifyOrderOp.CANCEL, row['order_id'], 0, 0, trd_env=row['trd_env'])
        ...
        # 由于 Futu API 大部分为同步阻塞调用，将其包装进线程池中运行，避免阻塞 FastAPI 事件循环
...
async def trigger_kill_switch(background_tasks: BackgroundTasks, req: KillSwitchReq):
    ...
    try:
        ...
        return {"status": "success", "message": "Kill switch engaged. All positions are being closed."}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to broadcast kill switch signal: {str(e)}")
...
async def cancel_order(order_id: str, req: CancelOrderReq):
    ...
    try:
        await redis_client.publish("oms:order_cancel", json.dumps({"order_id": order_id}))
...
async def modify_order(order_id: str, req: ModifyOrderReq):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Modification dispatch failed: {str(e)}")
...
async def websocket_oms_updates(websocket: WebSocket):
    ...
    channels = ["oms:bots:update", "oms:orders:update", "oms:trades:new", "oms:bot_log:stream", "oms:algo_executions:update"]
        ...
        async for message in pubsub.listen():
            if message["type"] == "message":
                ...
                if channel == "oms:bots:update":
                    ...
                    await websocket.send_json({"type": "active_orders_update", "data": data})
                    ...
                    await websocket.send_json({"type": "algo_executions_update", "data": data})
