        ...
        while True:
            ...
            try:
                ...
                if action == "subscribe":
                    ...
                    await websocket.send_text(json.dumps({
                        "code": 0, "msg": "ok",
                        "data": {"subscribed": new_tickers, "already_subscribed": [t for t in req_tickers if t not in new_tickers]},  # noqa: E501
    ...
    if res.get("status") == "error":
        ...
        if success and info and info.get('regularMarketPrice') is not None:
            ...
            return {
                "status": "success",
                "data": {
                    "ticker": ticker, "last_price": info.get("regularMarketPrice"), "open": info.get("regularMarketOpen"),  # noqa: E501
                    "high": info.get("regularMarketDayHigh"), "low": info.get("regularMarketDayLow"), "prev_close": info.get("previousClose"),  # noqa: E501
                    "volume": volume, "turnover": volume * info.get("regularMarketPrice", 0), "change_val": change, "change_pct": change_pct,  # noqa: E501
                    "amplitude": ((info.get("regularMarketDayHigh", 0) - info.get("regularMarketDayLow", 0)) / info.get("previousClose", 1) * 100 if info.get("previousClose") else 0),  # noqa: E501
...
async def sync_kline_warehouse(req: SyncKlineRequest):
    ...
    try:
        ...
        if success:
            return {"status": "success", "message": f"{req.ticker} ({req.interval}) 历史数据已成功同步并安全落库！可以继续回测。"}  # noqa: E501
        else:
...
async def get_history(ticker: str, ktype: str = "K_DAY", num: int = 60):
    ...
    if res.get("status") == "error":
        ...
        success, df_data, msg = await yf_service.fetch_yf_data(yf_ticker, "history", ttl=60, period=period, interval=yf_interval)  # noqa: E501
    ...
    if res.get("status") == "error":
        ...
        try:
            ...
            def fetch_yf_options():
                ...
                dates = tk.options
                if not dates:
                    return {"status": "error", "message": f"{yf_ticker} 没有可用的期权链数据"}  # noqa: E501
                ...
                return {"status": "success", "expiration_date": target_date, "count": len(compressed), "options": compressed, "source": "yfinance_fallback", "message": "已通过 YFinance 返回截断后的期权链兜底数据。"}  # noqa: E501

            yf_res = await asyncio.to_thread(fetch_yf_options)
            if yf_res.get("status") == "success": return yf_res  # noqa: E701
            raise HTTPException(status_code=400, detail=f"Futu: {res.get('message')}, YFinance: {yf_res.get('message')}")  # noqa: E501
        except Exception as e:
            if isinstance(e, HTTPException): raise e  # noqa: E701
            raise HTTPException(status_code=400, detail=f"Futu: {res.get('message')}, YFinance 兜底异常: {str(e)}")  # noqa: E501
    return res
        ...
        async with asyncio.timeout(5.0):
            async with _news_locks[cache_key]:
                ...
                if res.get("status") == "success" and res.get("data"):
                    ...
                    result = {"status": "success", "count": len(formatted_news), "data": formatted_news, "source": "finnhub"}  # noqa: E501
                    ttl = 900 + random.randint(10, 60)
                    await redis_client.setex(cache_key, ttl, json.dumps(result))
                    return result

                # 如果没有数据，返回空数组
                return {"status": "success", "count": 0, "data": [], "source": "empty", "message": "暂无个股新闻数据"}  # noqa: E501
    except TimeoutError:
        print(f"⚠️ [Market News] 等待 {ticker} 的锁或请求 Finnhub 超时 (5秒)")
        return {"status": "success", "count": 0, "data": [], "source": "empty", "message": "获取新闻超时，暂无数据"}  # noqa: E501
    except Exception as e:
        err_msg = str(e).strip() or type(e).__name__
        print(f"⚠️ [Market News] {ticker} 的 Finnhub 数据请求异常: {err_msg}")
        return {"status": "success", "count": 0, "data": [], "source": "error", "message": f"个股新闻源受限: {err_msg}"}  # noqa: E501
...
async def get_fundamental(ticker: str):
    ...
    fred_macro_map = {
        ...
        "DX-Y": "DTWEXBGS", "DX-Y.NYB": "DTWEXBGS", "WTI": "DCOILWTICO", "CL=F": "DCOILWTICO",  # noqa: E501
        "XAU": "GOLDAMGBD228NLBM", "GC=F": "GOLDAMGBD228NLBM", "BTC": "CBBTCUSD", "BTC-USD": "CBBTCUSD",  # noqa: E501
        "N225": "NIKKEI225", "^N225": "NIKKEI225", "EURUSD=X": "DEXUSEU", "GBPUSD=X": "DEXUSUK",  # noqa: E501
    ...
    for key, fred_id in fred_macro_map.items():
        if key == upper_ticker or key in upper_ticker:
            ...
            if res.get("status") == "success":
                return {"status": "success", "message": f"[{ticker}] 属于宏观大类资产，已自动为您路由至 FRED 数据库获取其最新指标序列。", "data": {"fred_series_id": fred_id, "recent_observations": res.get("data")}}  # noqa: E501

    index_indicators = ["HSI", "DJI", "800000", "800700", "SSEC", "CSI300"]
    if any(idx == upper_ticker or f".{idx}" in upper_ticker or f"{idx}." in upper_ticker for idx in index_indicators) or "BK" in upper_ticker:  # noqa: E501
        return {"status": "warning", "message": f"[{ticker}] 属于大盘或板块指数。指数没有个股基本面。请改用 get_broker_market_data 工具获取成交额与行情。"}  # noqa: E501
    ...
    if futu_res.get("status") == "success" and futu_res.get("data"):
        ...
        if not yf_success:
            raise HTTPException(status_code=400, detail=f"Futu: {futu_res.get('message')}, YFinance: {yf_msg}")  # noqa: E501
        ...
        if yf_info:
            ...
            if yf_info.get("quoteType") == "ETF":
                return {
                    ...
                    "data": {
                        "ticker": ticker, "company_name": yf_info.get("shortName", yf_info.get("longName")),  # noqa: E501
                        "fund_family": yf_info.get("fundFamily"), "total_assets": yf_info.get("totalAssets"),  # noqa: E501
            ...
            yf_results = {
                "ticker": ticker, "company_name": yf_info.get("shortName", ""), "trailing_PE": yf_info.get("trailingPE"),  # noqa: E501
                "forward_PE": yf_info.get("forwardPE"), "PEG_ratio": yf_info.get("pegRatio"), "price_to_book": yf_info.get("priceToBook"),  # noqa: E501
