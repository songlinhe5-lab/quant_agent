...
import re
import time
from fastapi import APIRouter, WebSocketDisconnect, HTTPException, WebSocket
...
async def get_services_health():
    ...
    # 1. Futu OpenD
    f_status = "healthy" if getattr(futu_service, "status", "") == "CONNECTED" else "disconnected"
    ...
    if res.get("status") == "error":
        ...
        if success and info and info.get('regularMarketPrice') is not None:
            ...
            return {
                "status": "success",
                "data": {
                    "ticker": ticker, "last_price": info.get("regularMarketPrice"), "open": info.get("regularMarketOpen"),
                    "high": info.get("regularMarketDayHigh"), "low": info.get("regularMarketDayLow"), "prev_close": info.get("previousClose"),
                    "volume": volume, "turnover": volume * info.get("regularMarketPrice", 0), "change_val": change, "change_pct": change_pct,
                    "amplitude": ((info.get("regularMarketDayHigh", 0) - info.get("regularMarketDayLow", 0)) / info.get("previousClose", 1) * 100 if info.get("previousClose") else 0),
                    "volume_str": f"{volume / 1_000_000:.2f}M" if volume > 1_000_000 else f"{volume / 1_000:.2f}K",
                },
                "source": "yfinance_fallback"
            }
        else:
            raise HTTPException(status_code=400, detail=f"Futu: {res.get('message')}, YFinance: {msg}")
    return res
...
async def sync_kline_warehouse(req: SyncKlineRequest):
    ...
    try:
        print(f"📦 [Market API] 收到前端手动数据同步请求: {req.ticker} ({req.interval})")
        # 调用本地数仓的更新方法 (自动执行增量追加或全量降级拉取)
        success = await kline_warehouse.update_ticker(req.ticker, ktype=ktype, force_full=req.force_full)
        
        if success:
            return {"status": "success", "message": f"{req.ticker} ({req.interval}) 历史数据已成功同步并安全落库！可以继续回测。"}
        else:
            raise HTTPException(status_code=500, detail="数据同步失败，API额度可能已耗尽或标的退市。")
    except Exception as e:
...
async def get_history(ticker: str, ktype: str = "K_DAY", num: int = 60):
    ...
    if res.get("status") == "error":
        ...
        # 💡 架构增强：针对 A 股日 K 线，优先使用 AKShare (规避 YFinance A股前复权异常问题)
        is_a_share = ticker.startswith("SH.") or ticker.startswith("SZ.")
        ...
        period = "7d" if yf_interval in ["1m", "5m"] else "1mo" if yf_interval in ["15m", "30m", "60m"] else "1y"
        
        success, df_data, msg = await yf_service.fetch_yf_data(yf_ticker, "history", ttl=60, period=period, interval=yf_interval)
        ...
        if success and df_data is not None and not df_data.empty:
            ...
            # 💡 性能修复：彻底抛弃巨慢的 iterrows()，使用 Pandas 高效向量化转换推入线程池
            def _format_history(df):
                df_formatted = pd.DataFrame({
                    "time": pd.Series(df.index).astype(str).str.split('+').str[0].str.replace('T', ' ').values,
...
async def get_option_chain(ticker: str, expiration_date: str = ""):
    ...
    if res.get("status") == "error":
        msg = res.get("message", "")
        if "原生不支持" not in msg:
            print(f"⚠️ [Option Chain] 券商数据获取 {ticker} 期权链失败 ({msg})，尝试降级雅虎财经...")
        ...
        try:
            ...
            def fetch_yf_options():
                ...
                dates = tk.options
                if not dates:
                    return {"status": "error", "message": f"{yf_ticker} 没有可用的期权链数据"}
                ...
                return {"status": "success", "expiration_date": target_date, "count": len(compressed), "options": compressed, "source": "yfinance_fallback", "message": "已通过 YFinance 返回截断后的期权链兜底数据。"}
            
            yf_res = await asyncio.to_thread(fetch_yf_options)
            if yf_res.get("status") == "success": return yf_res
            raise HTTPException(status_code=400, detail=f"Futu: {res.get('message')}, YFinance: {yf_res.get('message')}")
        except Exception as e:
            if isinstance(e, HTTPException): raise e
            raise HTTPException(status_code=400, detail=f"Futu: {res.get('message')}, YFinance 兜底异常: {str(e)}")
    return res
        ...
        try:
            ...
            if len(df) >= 30: # 确保有足够天数计算中长期均线等
                ...
                if res.get("status") == "success":
                    return res
                print(f"⚠️ [Tech Indicators] 券商数据指标计算返回异常: {res.get('message')}，尝试降级...")
        except Exception as e:
            print(f"⚠️ [Tech Indicators] 券商数据计算异常: {e}，尝试降级雅虎财经...")
    else:
        msg = futu_res.get("message", "")
        if "原生不支持" not in msg:
            print(f"⚠️ [Tech Indicators] 券商历史数据获取受阻 ({msg})，尝试降级雅虎财经...")
            
    # 2. 降级走雅虎财经
    yf_ticker = _to_yf_ticker(ticker)
    res = await yf_service.get_tech_indicators(ticker=yf_ticker, lookback_days=lookback_days)
    if res.get("status") == "error":
        raise HTTPException(status_code=400, detail=res.get("message"))
    return res
        ...
        async with asyncio.timeout(5.0):
            async with _news_locks[cache_key]:
                ...
                if res.get("status") == "success" and res.get("data"):
                    data = res.get("data", [])
                    data = sorted(data, key=lambda x: x.get("datetime", 0), reverse=True)[:limit]
                    ...
                    result = {"status": "success", "count": len(formatted_news), "data": formatted_news, "source": "finnhub"}
                    ttl = 900 + random.randint(10, 60)
                    await redis_client.setex(cache_key, ttl, json.dumps(result))
                    return result
                
                # 如果没有数据，返回空数组
                return {"status": "success", "count": 0, "data": [], "source": "empty", "message": "暂无个股新闻数据"}
    except TimeoutError:
        print(f"⚠️ [Market News] 等待 {ticker} 的锁或请求 Finnhub 超时 (5秒)")
        return {"status": "success", "count": 0, "data": [], "source": "empty", "message": "获取新闻超时，暂无数据"}
    except Exception as e:
        err_msg = str(e).strip() or type(e).__name__
        print(f"⚠️ [Market News] {ticker} 的 Finnhub 数据请求异常: {err_msg}")
        return {"status": "success", "count": 0, "data": [], "source": "error", "message": f"个股新闻源受限: {err_msg}"}
...
async def get_fundamental(ticker: str):
    ...
    fred_macro_map = {
        "SPX": "SP500", "^GSPC": "SP500", "IXIC": "NASDAQCOM", "^IXIC": "NASDAQCOM",
        "TNX": "DGS10", "^TNX": "DGS10", "VIX": "VIXCLS", "^VIX": "VIXCLS",
        "DX-Y": "DTWEXBGS", "DX-Y.NYB": "DTWEXBGS", "WTI": "DCOILWTICO", "CL=F": "DCOILWTICO",
        "XAU": "GOLDAMGBD228NLBM", "GC=F": "GOLDAMGBD228NLBM", "BTC": "CBBTCUSD", "BTC-USD": "CBBTCUSD",
        "N225": "NIKKEI225", "^N225": "NIKKEI225", "EURUSD=X": "DEXUSEU", "GBPUSD=X": "DEXUSUK", 
        "JPY=X": "DEXJPUS", "CNH=X": "DEXCHUS"
    }
    
    # 1. 智能拦截：如果是宏观资产/指数，自动无缝路由给 fred_service 获取其特有的“基本面” (宏观序列)
    for key, fred_id in fred_macro_map.items():
        if key == upper_ticker or key in upper_ticker:
            ...
            if res.get("status") == "success":
                return {"status": "success", "message": f"[{ticker}] 属于宏观大类资产，已自动为您路由至 FRED 数据库获取其最新指标序列。", "data": {"fred_series_id": fred_id, "recent_observations": res.get("data")}}

    index_indicators = ["HSI", "DJI", "800000", "800700", "SSEC", "CSI300"]
    if any(idx == upper_ticker or f".{idx}" in upper_ticker or f"{idx}." in upper_ticker for idx in index_indicators) or "BK" in upper_ticker:
        return {"status": "warning", "message": f"[{ticker}] 属于大盘或板块指数。指数没有个股基本面。请改用 get_broker_market_data 工具获取成交额与行情。"}

    # 💡 优先获取 Futu 的高质量数据，失败时再使用 YFinance 兜底，避免同时请求双数据源浪费资源
    # 组装最终结果
    final_data = {}
    ...
    if futu_res.get("status") == "success" and futu_res.get("data"):
        ...
        yf_success, yf_info, yf_msg = await yf_service.fetch_yf_data(yf_ticker, "info", ttl=43200, persist=True)
        if not yf_success:
            raise HTTPException(status_code=400, detail=f"Futu: {futu_res.get('message')}, YFinance: {yf_msg}")
        ...
        if yf_info:
            ...
            if yf_info.get("quoteType") == "ETF":
                return {
                    "status": "success",
                    "message": f"[{ticker}] 属于 ETF 基金，没有个股维度的 PE/PB/ROE 等估值指标。",
                    "data": {
                        "ticker": ticker, "company_name": yf_info.get("shortName", yf_info.get("longName")),
                        "fund_family": yf_info.get("fundFamily"), "total_assets": yf_info.get("totalAssets"),
                        "nav_price": yf_info.get("navPrice"),
                        "yield": f"{yf_info.get('yield', 0) * 100:.2f}%" if yf_info.get('yield') else None,
                        "beta": yf_info.get("beta"),
                    }
                }

            yf_results = {
                "ticker": ticker, "company_name": yf_info.get("shortName", ""), "trailing_PE": yf_info.get("trailingPE"),
                "forward_PE": yf_info.get("forwardPE"), "PEG_ratio": yf_info.get("pegRatio"), "price_to_book": yf_info.get("priceToBook"),
                "ROE": f"{yf_info.get('returnOnEquity', 0) * 100:.2f}%" if yf_info.get('returnOnEquity') else None,
                "short_ratio": yf_info.get("shortRatio"), "beta": yf_info.get("beta")
            }
...
async def get_insider_marquee(limit: int = 10):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取内幕交易跑马灯数据失败: {str(e)}")
