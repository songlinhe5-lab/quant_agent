    ...
    try:
        ...
        if user_message.attachments:
            for att in user_message.attachments:
                if att.type.startswith("image/"):
                    ...
                    # PDF 解析降级逻辑：在后端分离 base64 数据头，后续可接入 pdfplumber 提取纯文本
                    if "," in att.url:
                        header, base64_data = att.url.split(",", 1)
                    else:
                        base64_data = att.url
                    
                    # 💡 防御 Prompt 注入：净化用户控制的文件名，防止恶意闭合符伪造系统级别的核心指令
                    safe_name = att.name.replace("[", "【").replace("]", "】").replace("\n", " ")
                    
                    llm_content.append({
                        "type": "text",
                        "text": f"[系统提示：用户上传了 PDF 文件 {safe_name}，Base64 数据长度: {len(base64_data)} 字节]"
                    })
        
        # 构建最终发给 LLM SDK 的 messages
        llm_messages = [
            {"role": "system", "content": "你是一个专业的量化投研 AI 助手。"},
        ...
        async def generate_response():
            ...
            thoughts = [
                ...
                "⚠️ 发现参数偏离设定，正在自动纠错重试 (第 1 次)...\n拦截原因: 不支持该维度或周期 [filters->0->field]",
            ...
            # 💡 思考完毕后，开始输出给用户的正式回复
            reply_text = f"已收到指令。经过分析，为您筛选出以下结果..."
