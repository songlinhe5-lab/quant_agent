...
class TestPreferencesAPI(unittest.TestCase):
    ...
    def test_create_and_update_preferences(self):
        ...
        res_post_1 = self.client.post("/api/preferences/me", json=payload_1, headers=self.headers)
        self.assertEqual(res_post_1.status_code, 200)
        
        payload_2 = {"macro_symbols": ["TSLA", "NVDA"]}
        res_post_2 = self.client.post("/api/preferences/me", json=payload_2, headers=self.headers)
