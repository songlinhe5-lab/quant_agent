...
from backend.services.iv_history_service import get_iv_history, record_iv_snapshot


@pytest.mark.asyncio
async def test_record_iv_snapshot_rejects_invalid_iv():
    """负/零/None IV 不得落库 (零幻觉: 防止污染历史序列)"""
    with patch("backend.services.iv_history_service.SessionLocal") as mock_session:
...
async def test_record_iv_snapshot_valid_writes_db_and_cache():
    ...
    with (
        patch("backend.services.iv_history_service.SessionLocal", return_value=fake_db),
        patch("backend.services.iv_history_service.redis_client") as mock_redis,
    ):
...
async def test_get_iv_history_redis_priority():
    ...
    with (
        patch("backend.services.iv_history_service.redis_client") as mock_redis,
        patch("backend.services.iv_history_service.SessionLocal") as mock_session,
    ):
