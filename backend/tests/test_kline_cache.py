...
class TestCacheTier:
    def test_tier_values(self):
        from backend.services.datalake.kline_cache import CacheTier
...
class TestModuleConstants:
    def test_l1_ttl_days_value(self):
        from backend.services.datalake.kline_cache import L1_TTL_DAYS

        assert L1_TTL_DAYS == 5

    def test_l1_ttl_seconds_value(self):
        from backend.services.datalake.kline_cache import L1_TTL_DAYS, L1_TTL_SECONDS

        assert L1_TTL_SECONDS == L1_TTL_DAYS * 86400

    def test_l2_retention_days_value(self):
        from backend.services.datalake.kline_cache import L2_RETENTION_DAYS

        assert L2_RETENTION_DAYS == 365

    def test_l1_key_prefix(self):
        from backend.services.datalake.kline_cache import L1_KEY_PREFIX
...
class TestKlineCacheEngineGet:
    ...
    def test_get_kline_routes_to_l1_for_short_days(self):
        from backend.services.datalake.kline_cache import KlineCacheEngine
    ...
    def test_get_kline_explicit_tier_overrides_auto(self):
        from backend.services.datalake.kline_cache import CacheTier, KlineCacheEngine
...
class TestGetKlineCacheEngine:
    ...
    def test_singleton_is_kline_cache_engine(self):
        from backend.services.datalake.kline_cache import KlineCacheEngine, get_kline_cache_engine
