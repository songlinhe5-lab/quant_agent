...
# 导入核心组件和各种守护进程服务
from backend.core.database import engine, async_engine
from backend.core.redis_client import redis_client, redis_batch_writer
...
async def worker_heartbeat_daemon() -> None:
    ...
    while True:
        try:
            ...
            active_uuids = sorted([
                k.decode('utf-8').split(":")[-1] if isinstance(k, bytes) else k.split(":")[-1] 
            ...
            if WORKER_UUID in active_uuids:
                ...
                # 💡 动态覆写环境变量，全局任何调用 is_my_shard 的地方都会瞬间自动转移阵地！
                if str(new_worker_id) != os.getenv("WORKER_ID") or str(new_worker_total) != os.getenv("WORKER_TOTAL"):
                    os.environ["WORKER_ID"] = str(new_worker_id)
                    os.environ["WORKER_TOTAL"] = str(new_worker_total)
                    print(f"🔄 [HA 自动容灾] 探测到集群节点变动！当前节点重新定级: Rank {new_worker_id} / Total {new_worker_total}")
...
async def main():
    ...
    all_quote_tickers = ["US.AAPL", "HK.00700", "US.TSLA", "US.SPY", "US.QQQ", "US.NVDA", "US.MSFT", "US.AMZN", "US.META", "US.GOOGL"]
    tasks.append(asyncio.create_task(
        # 💡 这里直接传入所有 ticker，只要 publisher.run_daemon 内部每次循环调用 is_my_shard 即可实现动态跟随
    ...
    asyncio.create_task(notification_service.send_alert("✅ [Quant Worker] 独立数据生产节点已成功连线启动！"))
