...
def calculate_adx(df: pd.DataFrame, period: int = 14) -> dict:
    ...
    return {
        ...
        "di_diff": float(abs(plus_di.iloc[-1] - minus_di.iloc[-1])) if not np.isnan(plus_di.iloc[-1] - minus_di.iloc[-1]) else None,
    }
...
def calculate_vwma(df: pd.DataFrame, period: int = 20) -> float:
    ...
    vwma = (typical_price * df["volume"]).rolling(window=period).sum() / \
