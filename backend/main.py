...
try:
    ...
    with engine.begin() as conn:
        if is_pg:
            ...
            conn.execute(text("CREATE INDEX IF NOT EXISTS trgm_idx_ticker_symbol ON tickers USING gin (symbol gin_trgm_ops);"))  # noqa: E501
            conn.execute(text("CREATE INDEX IF NOT EXISTS trgm_idx_ticker_name ON tickers USING gin (name gin_trgm_ops);"))  # noqa: E501
...
async def lifespan(app: FastAPI): # type: ignore
    ...
    asyncio.create_task(finnhub_service._insider_transactions_marquee_daemon()) # 启动高管内幕交易跑马灯守护进程  # noqa: E501
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    logger.error(f"[UnhandledException] {request.method} {request.url.path} trace_id={trace_id} error={exc}", exc_info=True)  # noqa: E501
    body = {
...
async def response_envelope_middleware(request: Request, call_next):
    ...
    envelope = {
        ...
        "msg": "ok" if 200 <= response.status_code < 300 else (data.get("message", "error") if isinstance(data, dict) else "error"),  # noqa: E501
        "data": data,
        "ts": int(time.time() * 1000),
    }
...
def verify_metrics_auth(credentials: HTTPBasicCredentials = Depends(metrics_security)):
    ...
    if not (correct_username and correct_password):
        raise HTTPException(
            status_code=401, detail="Unauthorized metrics access", headers={"WWW-Authenticate": "Basic"}  # noqa: E501
        )
...
def metrics(username: str = Depends(verify_metrics_auth)):
    ...
    return Response(content=prometheus_client.generate_latest(), media_type=prometheus_client.CONTENT_TYPE_LATEST)  # noqa: E501
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    if not request.url.path.startswith("/assets") and request.url.path not in ["/", "/monitor", "/health"]:  # noqa: E501
        ...
        try:
            ...
            if current_requests > RATE_LIMIT:
                return JSONResponse(
                    status_code=429,
                    content={"status": "error", "message": f"请求过于频繁，限制为 {RATE_LIMIT}次/{RATE_WINDOW}秒。"}  # noqa: E501
                )
        except Exception as e:
...
async def get_current_user(
    ...
    try:
        ...
        if not username: raise HTTPException(status_code=401, detail="Token 载荷非法 (缺失 sub)")  # noqa: E501, E701
        return username
    except JWTError:
...
async def get_financial_report(ticker: str, chunk_index: int = 0):
    ...
    if not matched_files:
        return {"status": "error", "message": f"未在财报目录下找到包含 {safe_ticker} 的财报文件。"}  # noqa: E501
    ...
    try:
        ...
        if chunk_index < 0 or chunk_index >= len(chunks):
            return {"status": "error", "message": f"chunk_index 越界。有效范围: 0 到 {len(chunks)-1}"}  # noqa: E501

        return {"status": "success", "file_path": target_file, "total_chunks": len(chunks), "current_chunk_index": chunk_index, "content": chunks[chunk_index], "message": f"成功读取财报文件第 {chunk_index + 1}/{len(chunks)} 部分。"}  # noqa: E501
    except Exception as e:
...
async def toggle_yfinance(payload: YFinanceToggle):
    ...
    return {"status": "success", "message": f"YFinance 兜底已{'开启' if payload.enabled else '关闭'}"}  # noqa: E501
...
async def mcp_health_check(session_id: Optional[str] = None):
    ...
    return {"status": "success", "message": "MCP endpoint is online", "session_id": session_id}  # noqa: E501
...
async def root():
    """默认根路由"""
    return {"status": "success", "message": "Quant Agent 主网关已启动。请访问 /docs 查看接口文档。"}  # noqa: E501

# ==========================================
# --- AI Agent Chat API (SSE) ---
# ==========================================
STATIC_SUGGESTIONS = [
    {"title": "今日宏观风向", "prompt": "提取今天全球核心经济体的宏观大事件，并给出你的风险判断。"},  # noqa: E501
    {"title": "回测绩效归因", "prompt": "我的策略夏普比率 1.5，但最大回撤 25%，请分析可能的原因及改进建议。"},  # noqa: E501
    {"title": "量化代码 Debug", "prompt": "我有一个报错：RuntimeWarning: divide by zero encountered in scalar divide，在 Numpy 计算夏普比率时，如何安全处理？"},  # noqa: E501
    {"title": "交易心理建设", "prompt": "连续亏损导致心态失衡，作为量化交易员，该如何科学地执行熔断并调整心态？"},  # noqa: E501
    {"title": "期权策略套利", "prompt": "当前 VIX 较低，推荐一个适合中性震荡行情的期权卖方策略（如 Iron Condor）。"}  # noqa: E501
]

# 💡 动态组合词库：13(资产) * 10(指标) * 8(主题) * 7(动作) * 5(模板) = 可产生超过 36,000+ 种不重复的灵感  # noqa: E501
DYN_ASSETS = ["AAPL(苹果)", "TSLA(特斯拉)", "NVDA(英伟达)", "MSFT(微软)", "0700.HK(腾讯)", "09988.HK(阿里)", "BTC(比特币)", "ETH(以太坊)", "SPY(标普500)", "QQQ(纳指ETF)", "GLD(黄金ETF)", "USO(原油ETF)", "TLT(长牛美债)"]  # noqa: E501
DYN_INDICATORS = ["双均线(MA)", "指数移动平均(EMA)", "MACD", "RSI", "布林带(BOLL)", "真实波幅(ATR)", "KDJ", "VWAP", "动量因子(Momentum)", "夏普比率(Sharpe)"]  # noqa: E501
DYN_THEMES = ["基本面", "技术面", "资金面", "情绪面", "宏观政策", "期权隐含波动率(IV)", "量化统计套利", "跨市场对冲"]  # noqa: E501
DYN_ACTIONS = ["写一个实盘策略框架", "分析最新的走势", "诊断可能存在的黑天鹅风险", "给出投资组合建议", "编写量化特征提取代码", "分析支撑位和压力位", "评估目前的估值水平"]  # noqa: E501
...
async def chat_endpoint(request: ChatRequest, username: str = Depends(get_current_user)):  # noqa: E501
    ...
    async def generate_response():
        try:
            async for chunk in current_agent.chat_stream_async(user_message, attachments=None): # 暂时禁用图片识别，不传递 attachments  # noqa: E501
...
async def get_sessions(user_id: Optional[int] = None, q: Optional[str] = None, username: str = Depends(get_current_user)):  # noqa: E501
...
async def mcp_sse(request: Request):
    ...
    async def sse_generator():
        ...
        try:
            while True:
                try:
                    msg = await asyncio.wait_for(pubsub.get_message(ignore_subscribe_messages=True, timeout=15.0), timeout=15.0)  # noqa: E501
