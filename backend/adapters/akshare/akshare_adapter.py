...
class AkShareAdapter(DataSourcePort):
    ...
    def fetch(self, action: str, params: dict) -> DataSourceResult:
        ...
        if action not in self.capabilities:
            return DataSourceResult.error(
                f"Unsupported action: {action}", source=self.name
            )
    ...
    def _record_request(self):
        ...
        if self._request_count >= self.DEFAULT_RATE_LIMIT:
            ...
            logger.warning(f"[AkShareAdapter] Rate limit reached, backing off for 60s")
    ...
    def _fetch_stock_history(self, params: dict) -> dict:
        ...
        try:
            ...
            if ticker.startswith("SH.") or ticker.startswith("SZ."):
                symbol = ticker.replace(".", "")
                df = ak.stock_zh_a_hist(symbol=symbol, period="daily", start_date=params.get("start_date"), end_date=params.get("end_date"))
            elif "." in ticker:
                code = ticker.split(".")[0]
                market = "hk"
                df = ak.stock_hk_hist(symbol=code, period="daily", start_date=params.get("start_date"), end_date=params.get("end_date"))
            else:
                return {"success": False, "message": "Unsupported ticker format"}
